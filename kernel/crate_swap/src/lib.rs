...
use mod_mgmt::{
    crate_name_from_path, replace_containing_crate_name, write_relocation, CrateNamespace,
    IntoCrateObjectFile, NamespaceDir, StrongSectionRef, WeakDependent,
};
use path::Path;
...
pub type StateTransferFunction =
    fn(&Arc<CrateNamespace>, &CrateNamespace) -> Result<(), &'static str>;
...
pub fn swap_crates(
    ...
    let (namespace_of_new_crates, is_optimized) = {
        #[cfg(not(loscd_eval))]
        {
            ...
            if let Some(previously_cached_crates) =
                UNLOADED_CRATE_CACHE.lock().remove(&swap_requests)
            {
                warn!(
                    "Using optimized swap routine to swap in previously cached crates: {:?}",
                    previously_cached_crates.crate_names(true)
                );
    ...
    let (mut future_swap_requests, cached_crates) = if cache_old_crates {
        ...
        (
            SwapRequestList::new(),
            CrateNamespace::new(String::new(), this_namespace.dir().clone(), None),
        )
    };
    ...
    for req in &swap_requests {
        let SwapRequest {
            old_crate_name,
            old_namespace,
            new_crate_object_file,
            new_namespace: _new_ns,
        ...
        let new_crate_name =
            crate_name_from_path(&Path::new(new_crate_object_file.lock().get_name())).to_string();
        ...
        let old_crate_ref = match old_crate_name
            .as_deref()
            .and_then(|ocn| CrateNamespace::get_crate_and_namespace(old_namespace, ocn))
        {
            Some((ocr, _ns)) => {
        ...
        let new_crate_ref = if is_optimized {
            debug!(
                "swap_crates(): OPTIMIZED: looking for new crate {:?} in cache",
                new_crate_name
            );
            namespace_of_new_crates
                .get_crate(&new_crate_name)
                .ok_or_else(|| "BUG: swap_crates(): Couldn't get new crate from optimized cache")?
        } else {
            #[cfg(not(loscd_eval))]
            debug!(
                "looking for newly-loaded crate {:?} in temp namespace",
                new_crate_name
            );
        ...
        // scope the lock on the `new_crate_ref`
        {
            ...
            for old_sec in old_crate.data_sections_iter() {
                ...
                let prefix = if crates_have_same_name {
                    ...
                    if let Some(s) = replace_containing_crate_name(
                        old_sec_name_without_hash,
                        &old_crate_name_without_hash,
                        &new_crate_name_without_hash,
                    ) {
                        Cow::from(s)
                    } else {
                        Cow::from(old_sec_name_without_hash)
                ...
                let new_dest_sec = {
                    let mut iter = new_crate
                        .data_sections_iter()
                        .filter(|sec| sec.name.starts_with(&*prefix));
                    iter.next()
                ...
                #[cfg(not(loscd_eval))]
                debug!(
                    "swap_crates(): copying .data or .bss section from old {:?} to new {:?}",
                    &*old_sec, new_dest_sec
                );
            ...
            for old_sec in old_crate.global_sections_iter() {
                #[cfg(not(loscd_eval))]
                debug!(
                    "swap_crates(): looking for old_sec_name: {:?}",
                    old_sec.name
                );

                let old_sec_ns = this_namespace
                    .get_symbol_and_namespace(&old_sec.name)
                    .map(|(_weak_sec, ns)| ns)
                    .ok_or_else(|| {
                        error!(
                ...
                #[cfg(not(loscd_eval))]
                debug!(
                    "swap_crates(): old_sec_name: {:?}, old_sec: {:?}",
                    old_sec.name, old_sec
                );
                let old_sec_name_without_hash = old_sec.name_without_hash();
                    ...
                    .enumerate()
                {
                    ...
                    let new_source_sec = if let Some(ref nsr) = new_sec {
                        ...
                        let nsr =
                            find_corresponding_new_section(&mut new_crate.reexported_symbols)?;
                    ...
                    // then we need to temporarily remap them as writable here so we can fix up the target_sec's new relocation entry.
                    {
                        ...
                        #[cfg(loscd_eval)]
                        {
                            ...
                            hpet_total_rewriting_relocations +=
                                end_rewriting_relocations - start_rewriting_relocations;
                        }
                    ...
                    for mut strong_dep in target_sec.inner.write().sections_i_depend_on.iter_mut() {
                        if Arc::ptr_eq(&strong_dep.section, &old_sec)
                    ...
                    #[cfg(loscd_eval)]
                    {
                        ...
                        hpet_total_fixing_dependencies +=
                            end_fixing_dependencies - start_fixing_dependencies;
                    }
    ...
    for symbol in state_transfer_functions {
        let state_transfer_fn_sec = namespace_of_new_crates
            .get_symbol_or_load(&symbol, Some(this_namespace), kernel_mmi_ref, verbose_log)
            .upgrade()
            // as a backup, search fuzzily to accommodate state transfer function symbol names without full hashes
            .or_else(|| {
                namespace_of_new_crates
                    .get_symbol_starting_with(&symbol)
                    .upgrade()
            })
    ...
    if swap_requests.len() != new_crate_names.len()
        && swap_requests.len() != old_crates_are_loaded.len()
    {
    ...
    for ((req, new_crate_name), is_old_crate_loaded) in swap_requests
        .iter()
        .zip(new_crate_names.iter())
        .zip(old_crates_are_loaded.iter())
    {
        if !is_old_crate_loaded {
            continue;
        }
        let SwapRequest {
            old_crate_name,
            old_namespace,
            new_crate_object_file: _,
            new_namespace,
            reexport_new_symbols_as_old,
        } = req;
        ...
        if let Some(old_crate_ref) = old_namespace.crate_tree().lock().remove_str(old_crate_name) {
            {
                ...
                #[cfg(not(loscd_eval))]
                info!(
                    "  Removed old crate {:?} ({:?}) from namespace {}",
                    old_crate_name,
                    &*old_crate,
                    old_namespace.name()
                );
                ...
                if !reexport_new_symbols_as_old {
                    ...
                    for old_sec in old_crate.global_sections_iter() {
                        if old_ns_symbol_map.remove_str(&old_sec.name).is_none() {
                            ...
                            return Err(
                                "couldn't find old symbol {:?} in the old crate's namespace",
                            );
                        }
                    }
                }
            ...
            if cache_old_crates {
                ...
                cached_crates
                    .crate_tree()
                    .lock()
                    .insert_str(old_crate_name, old_crate_ref);
            }
            ...
            error!(
                "swap_crates(): couldn't remove old crate {} from old namespace {}!",
                old_crate_name,
                old_namespace.name()
            );
            continue;
        ...
        .zip(old_crates_are_loaded.iter())
    {
        ...
        #[cfg(not(loscd_eval))]
        debug!(
            "swap_crates(): adding new crate {:?} to namespace {}",
            new_crate_ref,
            req.new_namespace.name()
        );

        req.new_namespace
            .add_symbols(new_crate_ref.lock_as_ref().sections.values(), verbose_log);
        req.new_namespace
            .crate_tree()
            .lock()
            .insert_str(new_crate_name, new_crate_ref.clone());
    }
    ...
    namespace_of_new_crates.for_each_crate(true, |new_crate_name, new_crate_ref| {
        if !new_crate_ref.is_shared() {
            ...
            //        based on which directory its object file
            {
                ...
                if objfile_path.components().skip(1).next()
                    == Some(mod_mgmt::CrateType::Kernel.default_namespace_name())
                {
                    let new_target_ns = this_namespace
                        .recursive_namespace()
                        .unwrap_or(this_namespace);
                    #[cfg(not(loscd_eval))]
                    warn!(
                        "temp fix: changing target_ns from {} to {}, for crate {:?}",
                        this_namespace.name(),
                        new_target_ns.name(),
                        new_crate_ref
                    );
            ...
            #[cfg(not(loscd_eval))]
            {
                if this_namespace.get_crate(new_crate_name).is_none() {
                    error!(
                        "BUG: shared crate {} was not already in the current (backup) namespace",
                        new_crate_name
                    );
                }
                // else {
    ...
    for req in swap_requests.iter() {
        let SwapRequest {
            ...
            new_crate_object_file,
            new_namespace,
            reexport_new_symbols_as_old: _,
        } = req;

        let source_dir_ref = new_crate_object_file
            .lock()
            .get_parent_dir()
            .ok_or("BUG: new_crate_object_file has no parent directory")?;
        ...
        if let Some((mut replaced_old_crate_file, original_source_dir)) =
            move_file(new_crate_object_file, dest_dir_ref)?
        {
    ...
    if cache_old_crates {
        #[cfg(not(loscd_eval))]
        {
            ...
            UNLOADED_CRATE_CACHE
                .lock()
                .insert(future_swap_requests, cached_crates);
        }
    }
...
fn move_file(
    file: &FileRef,
    dest_dir: &DirRef,
) -> Result<Option<(FileOrDir, DirRef)>, &'static str> {
    let parent = file
        .lock()
        .get_parent_dir()
        .ok_or("couldn't get file's parent directory")?;
    ...
    let mut removed_file = parent
        .lock()
        .remove(&FileOrDir::File(Arc::clone(file)))
        .ok_or("Couldn't remove file from its parent directory")?;
    ...
    match res {
        Ok(replaced_file) => {
            #[cfg(not(any(loscd_eval, downtime_eval)))]
            debug!(
                "swap_crates::move_file(): moved file {:?} ({:?}) from {:?} to {:?}",
                file.try_lock().map(|f| f.get_name()),
                removed_file.get_name(),
                parent.try_lock().map(|p| p.get_name()),
                dest_dir.try_lock().map(|d| d.get_name())
            );
    ...
    pub fn new(
        ...
        let (old_crate_full_name, real_old_namespace) = match old_crate_name {
            ...
            Some(ocn) => {
                ...
                let mut matching_crates =
                    CrateNamespace::get_crates_starting_with(&old_namespace, ocn);
                if matching_crates.len() == 1 {
                    ...
                    let mut matching_files =
                        CrateNamespace::get_crate_object_files_starting_with(&old_namespace, ocn);
                    if matching_files.len() == 1 {
                        ...
                        let old_crate_full_name =
                            crate_name_from_path(&old_crate_file_path).to_string();
                        (Some(old_crate_full_name), real_old_namespace)
                    } else {
                        ...
                        let matches_vec = if !matching_crates.is_empty() {
                            matching_crates
                                .into_iter()
                                .map(|(c_name, _c_ref, ns)| (c_name, Arc::clone(ns)))
                                .collect::<Vec<_>>()
        ...
        if !Arc::ptr_eq(&old_namespace, real_old_namespace) {
            #[cfg(not(downtime_eval))]
            trace!(
                "SwapRequest::new(): changing old namespace from {:?} to {:?}",
                old_namespace.name(),
                real_old_namespace.name()
            );
        }
        ...
        let mut new_namespace = new_namespace.unwrap_or_else(|| {
            #[cfg(not(downtime_eval))]
            trace!(
                "SwapRequest::new(): new namespace was None, using old namespace {:?}",
                real_old_namespace.name()
            );
        ...
        let verified_new_crate_file = match new_crate_object_file {
            ...
            IntoCrateObjectFile::Prefix(prefix) => {
                let (new_crate_file, real_new_namespace) = {
                    let mut matching_files = CrateNamespace::get_crate_object_files_starting_with(
                        &new_namespace,
                        &prefix,
                    );
                    if matching_files.len() == 1 {
                        ...
                        return Err(InvalidSwapRequest::NewCratePrefixNotFound(
                            ...
                            matching_files
                                .into_iter()
                                .map(|(f, ns_ref)| (f, Arc::clone(ns_ref)))
                                .collect::<Vec<_>>(),
                        ));
                    }
                };
                if !Arc::ptr_eq(&new_namespace, real_new_namespace) {
                    #[cfg(not(downtime_eval))]
                    trace!(
                        "SwapRequest::new(): changing new namespace from {:?} to {:?}",
                        new_namespace.name(),
                        real_new_namespace.name()
                    );
...
pub enum InvalidSwapRequest {
    ...
    NewCratePrefixNotFound(
        String,
        Arc<CrateNamespace>,
        Vec<(FileRef, Arc<CrateNamespace>)>,
    ),
}
