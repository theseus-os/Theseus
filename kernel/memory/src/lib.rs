...
pub fn create_contiguous_mapping(
    ...
    let mp =
        kernel_mmi
            .page_table
            .map_allocated_pages_to(allocated_pages, allocated_frames, flags)?;
...
> {
    ...
    debug!(
        "bootloader info memory: p{:#X} to p{:#X}, bootloader modules: p{:#X} to p{:#X}",
        boot_info_paddr_start, boot_info_paddr_end, modules_start_paddr, modules_end_paddr,
    );
    debug!(
        "kernel_phys_start: p{:#X}, kernel_phys_end: p{:#X} kernel_virt_end = v{:#x}",
    ...
    let kernel_frames = FrameRange::from_phys_addr(
        kernel_phys_start,
        kernel_phys_end.value() - kernel_phys_start.value(),
    );
    let boot_modules_frames = FrameRange::from_phys_addr(
        modules_start_paddr,
        modules_end_paddr.value() - modules_start_paddr.value(),
    );
    let boot_info_frames = FrameRange::from_phys_addr(
        boot_info_paddr_start,
        boot_info_paddr_end.value() - boot_info_paddr_start.value(),
    );
    ...
    for area in boot_info
        ...
        .all_memory_areas()
    {
        let frames = FrameRange::from_phys_addr(
