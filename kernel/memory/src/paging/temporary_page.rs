...
use super::{Frame, FrameAllocator, Page, VirtualAddress};
...
impl TemporaryPage {
    ...
    pub fn map_table_frame(
        ...
        let mut page =
            Page::containing_address(VirtualAddress::new_canonical(TEMPORARY_PAGE_VIRT_ADDR));
        while page_table.translate_page(page).is_some() {
            // this never happens
            warn!(
                "temporary page {:?} is already mapped, trying the next lowest Page",
                page
            );
            page -= 1;
        }
        ...
        self.mapped_page = Some(try!(page_table.map_to(
            page,
            frame,
            flags,
            &mut self.allocator
        ...
        let table: &mut Table<Level1> = try!(
            try!(self.mapped_page.as_mut().ok_or("mapped page error")).as_type_mut(0) // no offset
        );
        Ok(table)
    }
}
