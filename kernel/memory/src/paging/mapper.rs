...
use paging::table::{Level4, Table, P4};
use paging::{get_current_p4, PageRange};
use type_name;
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
use x86_64;
use {
    AllocatedPages, Frame, FrameAllocator, FrameRange, Page, PhysicalAddress, VirtualAddress,
    BROADCAST_TLB_SHOOTDOWN_FUNC, FRAME_ALLOCATOR,
};
...
impl Mapper {
    ...
    pub fn dump_pte(&self, virtual_address: VirtualAddress) {
        ...
        if let Some(_pte) = p1.map(|p1| &p1[page.p1_index()]) {
            debug!(
                ...
                p3.map(|p3| &p3[page.p3_index()])
                    .map(|p3_entry| p3_entry.value())
                    .unwrap_or(0x0),
                p3.map(|p3| &p3[page.p3_index()])
                    .map(|p3_entry| p3_entry.flags()),
                p2.map(|p2| &p2[page.p2_index()])
                    .map(|p2_entry| p2_entry.value())
                    .unwrap_or(0x0),
                p2.map(|p2| &p2[page.p2_index()])
                    .map(|p2_entry| p2_entry.flags()),
                p1.map(|p1| &p1[page.p1_index()])
                    .map(|p1_entry| p1_entry.value())
                    .unwrap_or(0x0), // _pet.value()
                p1.map(|p1| &p1[page.p1_index()])
                    .map(|p1_entry| p1_entry.flags()), // _pte.flags()
            );
        } else {
            debug!(
                "Error: couldn't get PTE entry for vaddr: {:#X}. Has it been mapped?",
                virtual_address
            );
        }
    }
            ...
            p3.and_then(|p3| {
                ...
                if let Some(start_frame) = p3_entry.pointed_frame() {
                    if p3_entry.flags().contains(EntryFlags::HUGE_PAGE) {
                        ...
                        assert!(
                            start_frame.number % (ENTRIES_PER_PAGE_TABLE * ENTRIES_PER_PAGE_TABLE)
                                == 0
                        );
                        return Some(Frame {
                            number: start_frame.number
                                + page.p2_index() * ENTRIES_PER_PAGE_TABLE
                                + page.p1_index(),
                        });
                    }
                }
        ...
        A: FrameAllocator,
    {
        ...
        for page in pages.clone() {
            let frame = try!(allocator
                .allocate_frame()
                .ok_or("Mapper::internal_map(): couldn't allocate new frame, out of memory!"));

            let p3 = self
                .p4_mut()
                .next_table_create(page.p4_index(), top_level_flags, allocator);
            ...
            if !p1[page.p1_index()].is_unused() {
                error!(
                    "Mapper::internal_map(): page {:#x} -> frame {:#X}, page was already in use!",
                    page.start_address(),
                    frame.start_address()
                );
            ...
            p1[page.p1_index()].set(
                frame,
                flags | EntryFlags::PRESENT | EntryFlags::ACCESSEDARM | EntryFlags::INNER_SHARE,
            );
        }
        ...
        A: FrameAllocator,
    {
        ...
        if pages_count != frames_count {
            error!(
                "map_to_internal(): page count {} must equal frame count {}!",
                pages_count, frames_count
            );
        ...
        for (page, frame) in pages.clone().into_iter().zip(frames) {
            ...
            if !p1[page.p1_index()].is_unused() {
                error!(
                    "map_to() page {:#x} -> frame {:#X}, page was already in use!",
                    page.start_address(),
                    frame.start_address()
                );
                return Err("page was already in use");
    ...
    where
        A: FrameAllocator,
    {
        self.internal_map_to(
            PageRange::new(page, page),
            FrameRange::new(frame.clone(), frame),
            flags,
            allocator,
        )
    }
    ...
    pub fn map_allocated_pages_to<A>(
        &mut self,
...
impl MappedPages {
    ...
    pub fn merge(
        mappings: Vec<MappedPages>,
    ) -> Result<MappedPages, (&'static str, Vec<MappedPages>)> {
        if mappings.len() <= 1 {
            return Err((
                "cannot merge one or fewer mappings, nothing to do",
                mappings,
            ));
        };
        ...
        let first_mapping = mappings.get(0).map(|first| {
            (
                first.page_table_p4.clone(),
                first.flags,
                first.allocated.is_some(),
                first.pages.clone(),
            )
        });
        ...
        for mp in &mappings[1..] {
            ...
            if mp.flags != flags {
                error!(
                    "MappedPages::merge(): mappings had different flags: {:?} vs. {:?}",
                    mp.flags, flags
                );
    ...
    pub fn deep_copy<A: FrameAllocator>(
        ...
        let new_pages =
            allocate_pages(self.size_in_pages()).ok_or_else(|| "Couldn't allocate_pages()")?;
        ...
        A: FrameAllocator,
    {
        ...
        for page in self.pages.clone() {
            ...
            let _frame = try!(p1[page.p1_index()]
                .pointed_frame()
            ...
            #[cfg(any(target_arch = "aarch64"))]
            aarch64::instructions::tlb::flush(aarch64::VirtualAddress(
    ...
    pub fn as_type<T>(&self, offset: usize) -> Result<&T, &'static str> {
        ...
        let t: &T = unsafe { mem::transmute(self.pages.start_address() + offset) };
    ...
    pub fn as_type_mut<T>(&mut self, offset: usize) -> Result<&mut T, &'static str> {
        ...
        // SAFE: we guarantee the size and lifetime are within that of this MappedPages object
        let t: &mut T = unsafe { mem::transmute(self.pages.start_address() + offset) };
    ...
    pub fn as_slice<T>(&self, byte_offset: usize, length: usize) -> Result<&[T], &'static str> {
        ...
        if end > self.size_in_bytes() {
            ...
            return Err(
                "requested slice length and offset would not fit within the MappedPages bounds",
            );
        }
        ...
        let slc: &[T] = unsafe {
            slice::from_raw_parts(
                (self.pages.start_address().value() + byte_offset) as *const T,
                length,
            )
        };
    ...
    pub fn as_slice_mut<T>(
        ...
        let slc: &mut [T] = unsafe {
            slice::from_raw_parts_mut(
                (self.pages.start_address().value() + byte_offset) as *mut T,
                length,
            )
        };
    ...
    pub fn as_func<'a, F>(
        ...
        *space = self.pages.start_address().value() + offset;
        ...
        // SAFE: we guarantee the size and lifetime are within that of this MappedPages object
        let t: &'a F = unsafe { mem::transmute(space) };
