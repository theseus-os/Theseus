...
use super::{tlb_flush_virt_addr, EntryFlags};
use core::{
    fmt::{self, Write},
    mem,
    ops::Deref,
...
use PhysicalAddress;
use VirtualAddress;
use BROADCAST_TLB_SHOOTDOWN_FUNC;
...
impl Mapper {
            ...
            p3.and_then(|p3| {
                ...
                if let Some(start_frame) = p3_entry.pointed_frame() {
                    if p3_entry.flags().is_huge() {
                        ...
                        assert!(
                            start_frame.number()
                                % (ENTRIES_PER_PAGE_TABLE * ENTRIES_PER_PAGE_TABLE)
                                == 0
                        );
    ...
    pub fn map_allocated_pages(
        ...
        for page in pages.deref().clone() {
            ...
            if !p1[page.p1_index()].is_unused() {
                error!(
                    "map_allocated_pages(): page {:#X} -> frame {:#X}, page was already in use!",
                    page.start_address(),
                    frame.start_address()
                );
...
impl Mapper {
    ...
    /// Consumes the given `AllocatedPages` and returns a `MappedPages` object which contains those `AllocatedPages`.
    #[doc(hidden)]
    pub unsafe fn map_to_non_exclusive(
        mapper: &mut Self,
        pages: AllocatedPages,
        frames: FrameRange,
        flags: EntryFlags,
        ...
        if pages_count != frames_count {
            error!(
                "map_to_non_exclusive(): pages {:?} count {} must equal frames {:?} count {}!",
                pages, pages_count, frames, frames_count
            );
            ...
            .zip(frames.deref().clone().into_iter())
        {
            ...
            if !p1[page.p1_index()].is_unused() {
                error!(
                    "map_to_non_exclusive(): page {:#X} -> frame {:#X}, page was already in use!",
                    page.start_address(),
                    frame.start_address()
                );
...
impl MappedPages {
    ...
    pub fn merge(&mut self, mut mp: MappedPages) -> Result<(), (&'static str, MappedPages)> {
        if mp.page_table_p4 != self.page_table_p4 {
            ...
            return Err((
                "failed to merge MappedPages that were mapped into different page tables",
                mp,
            ));
        }
        if mp.flags != self.flags {
            ...
            return Err((
                "failed to merge MappedPages that were mapped with different flags",
                mp,
            ));
        }
        ...
        if let Err(orig) = self.pages.merge(second_alloc_pages_owned) {
            ...
            return Err((
                "failed to merge MappedPages that weren't virtually contiguous",
                mp,
            ));
        }
    ...
    pub fn unmap_into_parts(
        ...
        match self.unmap(active_table_mapper) {
            ...
            Err(e) => {
                error!(
                    "MappedPages::unmap_into_parts(): failed to unmap {:?}, error: {}",
                    self, e
                );
                return Err(self);
            }
    ...
    pub fn as_type<T: FromBytes>(&self, offset: usize) -> Result<&T, &'static str> {
        ...
        let t: &T = unsafe { &*((self.pages.start_address().value() + offset) as *const T) };
    ...
    pub fn as_type_mut<T: FromBytes>(&mut self, offset: usize) -> Result<&mut T, &'static str> {
        ...
        // SAFE: we guarantee the size and lifetime are within that of this MappedPages object
        let t: &mut T = unsafe { &mut *((self.pages.start_address().value() + offset) as *mut T) };
