...
pub use self::{mapper::*, temporary_page::TemporaryPage};
...
impl PageTable {
    ...
    pub fn new_table(
...

        {
            ...
            table.zero();
            table[RECURSIVE_P4_INDEX]
                .set_entry(p4_frame, EntryFlags::PRESENT | EntryFlags::WRITABLE);
        }

        let (_temp_page, inited_new_p4_frame) =
            temporary_page.unmap_into_parts(current_page_table)?;

        Ok(PageTable {
            mapper: Mapper::with_p4_frame(p4_frame),
            p4_table: inited_new_p4_frame.ok_or(
                "BUG: PageTable::new_table(): failed to take back unmapped Frame for p4_table",
            )?,
        })
    }
        ...
        F: FnOnce(&mut Mapper) -> Result<(), &'static str>,
    {
        ...
        p4_table[RECURSIVE_P4_INDEX]
            .set_entry(active_p4_frame, EntryFlags::PRESENT | EntryFlags::WRITABLE);
        tlb_flush_all();
        ...
        self.p4_table = p4_frame
            .ok_or("BUG: PageTable::with(): failed to take back unmapped Frame for p4_table")?;
...
pub fn init(
    ...
    &'static str,
> {
    let (aggregated_section_memory_bounds, _sections_memory_bounds) =
        find_section_memory_bounds(boot_info)?;
    debug!(
        "{:X?}\n{:X?}",
        aggregated_section_memory_bounds, _sections_memory_bounds
    );

    // bootstrap a PageTable from the currently-loaded page table
    let current_active_p4 = frame_allocator::allocate_frames_at(
    ...
    let boot_info_start_vaddr = VirtualAddress::new(boot_info.start_address())
        .ok_or("boot_info start virtual address was invalid")?;
    ...
    debug!(
        "multiboot vaddr: {:#X}, multiboot paddr: {:#X}, size: {:#X}\n",
        boot_info_start_vaddr, boot_info_start_paddr, boot_info_size
    );

    let new_p4_frame =
        frame_allocator::allocate_frames(1).ok_or("couldn't allocate frame for new page table")?;
    ...
    page_table.with(&mut new_table, TemporaryPage::new(), |mapper| {
        ...
        let text_pages = page_allocator::allocate_pages_by_bytes_at(
            text_start_virt,
            text_end_virt.value() - text_start_virt.value(),
        )?;
        let text_frames = frame_allocator::allocate_frames_by_bytes_at(
            text_start_phys,
            text_end_phys.value() - text_start_phys.value(),
        )?;
        let text_pages_identity = page_allocator::allocate_pages_by_bytes_at(
            text_start_virt - KERNEL_OFFSET,
            text_end_virt.value() - text_start_virt.value(),
        )?;
        let text_frames_identity = text_frames.deref().clone();
        text_mapped_pages =
            Some(mapper.map_allocated_pages_to(text_pages, text_frames, text_flags)?);
        identity_mapped_pages[index] = Some(unsafe {
            Mapper::map_to_non_exclusive(
                mapper,
                text_pages_identity,
                text_frames_identity,
                text_flags,
            )?
        });
        index += 1;

        let rodata_pages = page_allocator::allocate_pages_by_bytes_at(
            rodata_start_virt,
            rodata_end_virt.value() - rodata_start_virt.value(),
        )?;
        let rodata_frames = frame_allocator::allocate_frames_by_bytes_at(
            rodata_start_phys,
            rodata_end_phys.value() - rodata_start_phys.value(),
        )?;
        let rodata_pages_identity = page_allocator::allocate_pages_by_bytes_at(
            rodata_start_virt - KERNEL_OFFSET,
            rodata_end_virt.value() - rodata_start_virt.value(),
        )?;
        let rodata_frames_identity = rodata_frames.deref().clone();
        rodata_mapped_pages =
            Some(mapper.map_allocated_pages_to(rodata_pages, rodata_frames, rodata_flags)?);
        identity_mapped_pages[index] = Some(unsafe {
            Mapper::map_to_non_exclusive(
                mapper,
                rodata_pages_identity,
                rodata_frames_identity,
                rodata_flags,
            )?
        });
        index += 1;

        let data_pages = page_allocator::allocate_pages_by_bytes_at(
            data_start_virt,
            data_end_virt.value() - data_start_virt.value(),
        )?;
        let data_frames = frame_allocator::allocate_frames_by_bytes_at(
            data_start_phys,
            data_end_phys.value() - data_start_phys.value(),
        )?;
        let data_pages_identity = page_allocator::allocate_pages_by_bytes_at(
            data_start_virt - KERNEL_OFFSET,
            data_end_virt.value() - data_start_virt.value(),
        )?;
        let data_frames_identity = data_frames.deref().clone();
        data_mapped_pages =
            Some(mapper.map_allocated_pages_to(data_pages, data_frames, data_flags)?);
        identity_mapped_pages[index] = Some(unsafe {
            Mapper::map_to_non_exclusive(
                mapper,
                data_pages_identity,
                data_frames_identity,
                data_flags,
            )?
        });
        index += 1;
        ...
        let stack_pages = page_allocator::allocate_pages_by_bytes_at(
            stack_start_virt,
            (stack_end_virt - stack_start_virt).value(),
        )?;
        let start_of_stack_pages = *stack_pages.start() + 1;
        let (stack_guard_page, stack_allocated_pages) =
            stack_pages.split(start_of_stack_pages).map_err(|_ap| {
                "BUG: initial stack's allocated pages were not split correctly after guard page"
            })?;
        let stack_start_frame = Frame::containing_address(stack_start_phys) + 1; // skip 1st frame, which corresponds to the guard page
        let stack_allocated_frames = frame_allocator::allocate_frames_at(
            stack_start_frame.start_address(),
            stack_allocated_pages.size_in_pages(),
        )?;
        ...
        let vga_display_pages = page_allocator::allocate_pages_by_bytes_at(
            vga_virt_addr_identity + KERNEL_OFFSET,
            vga_size_in_bytes,
        )?;
        let vga_display_frames =
            frame_allocator::allocate_frames_by_bytes_at(vga_phys_addr, vga_size_in_bytes)?;
        let vga_display_pages_identity =
            page_allocator::allocate_pages_by_bytes_at(vga_virt_addr_identity, vga_size_in_bytes)?;
        ...
        identity_mapped_pages[index] = Some(unsafe {
            Mapper::map_to_non_exclusive(
                mapper,
                vga_display_pages_identity,
                vga_display_frames_identity,
                vga_flags,
            )?
        });
        index += 1;
        ...
        let boot_info_pages =
            page_allocator::allocate_pages_by_bytes_at(boot_info_start_vaddr, boot_info_size)?;
        let boot_info_frames =
            frame_allocator::allocate_frames_by_bytes_at(boot_info_start_paddr, boot_info_size)?;
        ...
        debug!(
            ...
            &identity_mapped_pages[..index]
        );
        debug!(
            "higher_half_mapped_pages: {:?}",
            &higher_half_mapped_pages[..index]
        );
    ...
    let boot_info_mapped_pages =
        boot_info_mapped_pages.ok_or("Couldn't map boot_info pages section")?;
    let stack_page_group = stack_page_group.ok_or("Couldn't map .stack section")?;

    debug!(
        "switching from old page table {:?} to new page table {:?}",
