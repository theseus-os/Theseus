use super::super::{
    EntryFlags, Frame, FrameAllocator, Page, PhysicalAddress, VirtualAddress, VirtualMemoryArea,
    BROADCAST_TLB_SHOOTDOWN_FUNC,
};
...
impl MapperSpillful {
    pub fn new() -> MapperSpillful {
        ...
        } // cannot panic, we know P4 is valid.
    }
        ...
        A: FrameAllocator,
    {
        ...
        for page in PageRange::from_virt_addr(vaddr, size).clone() {
            let frame = allocator
                .allocate_frame()
                .ok_or("MapperSpillful::map() -- out of memory trying to alloc frame")?;
            let mut p3 =
                self.p4_mut()
            ...
            if !p1[page.p1_index()].is_unused() {
                error!(
                    "MapperSpillful::map() page {:#x} -> frame {:#X}, page was already in use!",
                    page.start_address(),
                    frame.start_address()
                );
        ...
        VMAS.lock()
            .push(VirtualMemoryArea::new(vaddr, size, flags, ""));
        Ok(())
    }
    ...
    pub fn remap(
        ...
        for page in pages {
            ...
            let frame = try!(p1[page.p1_index()]
                .pointed_frame()
