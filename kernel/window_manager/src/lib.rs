...
pub fn init() -> Result<(Queue<Event>, Queue<Event>), &'static str> {
    ...
    TaskBuilder::new(window_manager_loop, (key_consumer, mouse_consumer))
...
fn keyboard_handle_application(key_input: KeyEvent) -> Result<(), &'static str> {
        ...
        && key_input.action == KeyAction::Pressed
    {
        ...
        TaskBuilder::<fn(Vec<alloc::string::String>) -> isize, Vec<alloc::string::String>, isize>::new_application(path, Some(new_app_namespace))?
            .name(format!("shell"))
            .spawn()?;
