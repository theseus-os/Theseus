...
extern crate window;
extern crate window_list_primitive;
...
pub use window_list_primitive::{
    SCREEN_BACKGROUND_COLOR, WINDOW_ACTIVE_COLOR, 
    WINDOW_INACTIVE_COLOR, WINDOW_MARGIN, WINDOW_PADDING, WindowListPrimitive, 
};
...
lazy_static! {
    ...
    pub static ref WINDOWLIST: Mutex<WindowListPrimitive<WindowProfilePrimitive>> = Mutex::new(
        WindowListPrimitive{
...
pub struct WindowPrimitive<Buffer: FrameBuffer> {
    /// The profile object of the window.
    pub profile: Arc<Mutex<WindowProfilePrimitive>>,
...
pub fn new_window(
    ...
    width: usize,
    height: usize,
) -> Result<WindowPrimitive<FrameBufferRGB>, &'static str> {
    ...
    let window: WindowPrimitive<FrameBufferRGB> = WindowPrimitive {
        profile: profile_ref,
        consumer: consumer,
...
pub fn new_default_window() -> Result<WindowPrimitive<FrameBufferRGB>, &'static str> {
...
pub struct WindowProfilePrimitive {
...
/*impl<Buffer: FrameBuffer> Drop for WindowPrimitive<Buffer> {
    fn drop(&mut self) {
        let mut window_list_primitive = window_manager.lock();

        // Switches to a new active window and sets
        // the active pointer field of the window allocator to the new active window
        match window_list_primitive.delete(&self.inner) {
            Ok(_) => {}
