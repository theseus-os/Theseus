...
use irq_safety::{interrupts_enabled, MutexIrqSafe};
...
pub static RUNQUEUE_REMOVAL_FUNCTION: spin::Once<fn(&TaskRef, u8) -> Result<(), &'static str>> =
    spin::Once::new();
...
impl Task {
    ...
    pub fn new(
        ...
        let curr_task = get_my_current_task()
            .ok_or("Task::new(): couldn't get current task (not yet initialized)")?;
    ...
    fn set_as_current_task(&self) {
        ...
        if let Some(ref tld) = self.inner.lock().task_local_data {
            ...
            error!(
                "BUG: failed to set current task, it had no TaskLocalData. {:?}",
                self
            );
        }
    }
    ...
    pub fn task_switch(&self, next: &Task, apic_id: u8) {
        ...
        if false {
            ...
            if Arc::ptr_eq(prev_mmi, next_mmi) {
                ...
                prev_mmi_locked
                    .page_table
                    .switch(&next_mmi_locked.page_table);
            }
        }
        ...
        if self.has_exited() {
            ...
            next.inner.lock().drop_after_task_switch = self
                .inner
                .lock()
                .task_local_data
                .take()
                .map(|tld_box| tld_box as Box<dyn Any + Send>);
        }
...
pub fn bootstrap_task(
    ...
    let default_namespace = mod_mgmt::get_initial_kernel_namespace()
        .ok_or(
            "The initial kernel CrateNamespace must be initialized before the tasking subsystem.",
        )?
        .clone();
    ...
    if get_my_current_task().is_none() {
        ...
        return Err(
            "BUG: bootstrap_task(): failed to properly set the new idle task as the current task",
        );
    }
