...
impl TxDescriptor for LegacyTxDescriptor {
    ...
    fn send(&mut self, transmit_buffer_addr: PhysicalAddress, transmit_buffer_length: u16) {
        ...
        self.cmd
            .write(TX_CMD_EOP | TX_CMD_IFCS | TX_CMD_RPS | TX_CMD_RS);
        self.status.write(0);
    }
...
impl fmt::Debug for LegacyTxDescriptor {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "{{addr: {:#X}, length: {}, cso: {}, cmd: {}, status: {}, css: {}, special: {}}}",
            self.phys_addr.read(),
            self.length.read(),
            self.cso.read(),
            self.cmd.read(),
...
impl fmt::Debug for LegacyRxDescriptor {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "{{addr: {:#X}, length: {}, checksum: {}, status: {}, errors: {}, special: {}}}",
            self.phys_addr.read(),
            self.length.read(),
            self.checksum.read(),
            self.status.read(),
...
impl RxDescriptor for AdvancedRxDescriptor {
    fn init(&mut self, packet_buffer_address: PhysicalAddress) {
        self.packet_buffer_address
            .write(packet_buffer_address.value() as u64);
...
impl fmt::Debug for AdvancedRxDescriptor {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "{{Packet buffer address: {:#X}, Packet header address: {:#X}}}",
...
impl TxDescriptor for AdvancedTxDescriptor {
    ...
    fn send(&mut self, transmit_buffer_addr: PhysicalAddress, transmit_buffer_length: u16) {
        self.packet_buffer_address
            .write(transmit_buffer_addr.value() as u64);
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("AdvancedTxDescriptor")
            ...
            .field(
                ...
                &self.paylen_popts_cc_idx_sta.read(),
            )
            .finish()
    }
}
