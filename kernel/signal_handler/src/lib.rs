...
pub fn register_signal_handler(signal: Signal, handler: Box<dyn SignalHandler>) -> Result<(), ()> {
...
pub fn take_signal_handler(signal: Signal) -> Option<Box<dyn SignalHandler>> {
    SIGNAL_HANDLERS.with(|sig_handlers| sig_handlers[signal as usize].borrow_mut().take())
}
