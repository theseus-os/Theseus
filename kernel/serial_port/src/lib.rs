...
pub use serial_port_basic::{
    take_serial_port as take_serial_port_basic, SerialPort as SerialPortBasic, SerialPortAddress,
...
fn interrupt_number_handler(serial_port_address: &SerialPortAddress) -> (u8, HandlerFunc) {
    match serial_port_address {
        SerialPortAddress::COM1 | SerialPortAddress::COM3 => {
            (IRQ_BASE_OFFSET + 0x04, com1_com3_interrupt_handler)
        }
        SerialPortAddress::COM2 | SerialPortAddress::COM4 => {
            (IRQ_BASE_OFFSET + 0x03, com2_com4_interrupt_handler)
        }
    }
}
...
impl SerialPort {
    ...
    pub fn register_interrupt_handler(
        ...
        let registration_result = deferred_interrupt_tasks::register_interrupt_handler(
            ...
            serial_port,
            Some(format!(
                "serial_port_deferred_task_irq_{:#X}",
                interrupt_number
            )),
        );
        ...
        match registration_result {
            Ok(deferred_task) => {
                ...
                info!(
                    "Registered interrupt handler at IRQ {:#X} for serial port {:#X}.",
                ...
                match SerialPortAddress::try_from(base_port) {
                    ...
                    Err(_) => warn!(
                        "Registering interrupt handler for unknown serial port at {:#X}",
                        base_port
                    ),
                };
            }
            Err(InterruptRegistrationError::IrqInUse {
                irq,
                existing_handler_address,
            }) => {
                if existing_handler_address != interrupt_handler as u64 {
                    error!(
                        "Failed to register interrupt handler at IRQ {:#X} for serial port {:#X}. \
    ...
    pub fn set_data_sender(&mut self, sender: Sender<DataChunk>) -> Result<(), ()> {
...
fn serial_port_receive_deferred(serial_port: &Arc<MutexIrqSafe<SerialPort>>) -> Result<(), ()> {
    ...
    if let Err(e) = send_result {
        error!(
            "Failed to send data received for serial port at {:#X}: {:?}.",
            base_port, e.1
        );
    }
    ...
    if input_was_ignored {
        if let Some(sender) = NEW_CONNECTION_NOTIFIER.get() {
            ...
            if let Ok(serial_port_address) = SerialPortAddress::try_from(base_port) {
                ...
                error!(
                    "Error: base port {:#X} was not a known serial port address.",
                    base_port
                );
            }
        } else {
