...
use memory::{VirtualAddress, PhysicalAddress, MappedPages, Page, Frame, EntryFlags, MemoryManagementInfo, FRAME_ALLOCATOR, Stack};
    ...
    for madt_entry in madt_iter.clone() {
        if let MadtEntry::LocalApic(lapic_entry) = madt_entry { 
            if lapic_entry.apic_id == me {
                ...
                let ap_stack = kernel_mmi_ref.lock().alloc_stack(4).ok_or("could not allocate AP stack!")?;
