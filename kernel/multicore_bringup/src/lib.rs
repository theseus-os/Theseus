...
use apic::{get_bsp_id, get_lapics, get_my_apic_id, has_x2apic, LocalApic};
use core::{ops::DerefMut, sync::atomic::Ordering};
use irq_safety::MutexIrqSafe;
use kernel_config::memory::{KERNEL_STACK_SIZE_IN_PAGES, PAGE_SHIFT, PAGE_SIZE};
use madt::{find_nmi_entry_for_processor, Madt, MadtEntry, MadtLocalApic};
use memory::{EntryFlags, MappedPages, MemoryManagementInfo, PhysicalAddress, VirtualAddress};
...
pub fn handle_ap_cores(
    ...
    let mut ap_startup_mapped_pages: MappedPages; // must be held throughout APs being booted up
    {
        ...
        page_table
            .translate(ap_start_realmode_begin)
            .ok_or("handle_ap_cores(): couldn't translate ap_start_realmode address")?;
        ...
        let trampoline_frame =
            memory::allocate_frames_at(PhysicalAddress::new_canonical(TRAMPOLINE), 1)
                .map_err(|_e| "handle_ap_cores(): failed to allocate trampoline frame")?;
        let trampoline_page = memory::allocate_pages_at(
            VirtualAddress::new_canonical(TRAMPOLINE),
            trampoline_frame.size_in_frames(),
        )
        .map_err(|_e| "handle_ap_cores(): failed to allocate trampoline page")?;
        let ap_startup_frames = memory::allocate_frames_by_bytes_at(
            PhysicalAddress::new_canonical(AP_STARTUP),
            ap_startup_size_in_bytes,
        )
        .map_err(|_e| "handle_ap_cores(): failed to allocate AP startup frames")?;
        let ap_startup_pages = memory::allocate_pages_at(
            VirtualAddress::new_canonical(AP_STARTUP),
            ap_startup_frames.size_in_frames(),
        )
    ...
    // Copy the AP startup code (from the kernel's text section pages) into the AP_STARTUP physical address entry point.
    {
        ...
        let kernel_text_pages_ref = mod_mgmt::get_initial_kernel_namespace()
            ...
            .and_then(|namespace| {
                namespace
                    .get_crate("nano_core")
                    .ok_or("BUG: couldn't get the 'nano_core' crate")
            })
            .and_then(|nano_core_crate| {
                nano_core_crate
                    .lock_as_ref()
                    .text_pages
                    .clone()
                    .ok_or("BUG: nano_core crate had no text pages")
            })?;
        ...
        let dest_slice: &mut [u8] =
            ap_startup_mapped_pages.as_slice_mut(0, ap_startup_size_in_bytes)?;
    ...
    for madt_entry in madt_iter.clone() {
        if let MadtEntry::LocalApic(lapic_entry) = madt_entry {
            if lapic_entry.apic_id == me {
                ...
                let (nmi_lint, nmi_flags) =
                    find_nmi_entry_for_processor(lapic_entry.processor, madt_iter.clone());
    ...
    {
        let graphic_info =
            trampoline_mapped_pages.as_type::<GraphicInfo>(GRAPHIC_INFO_OFFSET_FROM_TRAMPOLINE)?;
    ...
    while num_known_cores < expected_cores {
        ...
        if iter == 100000 {
            trace!(
                "BSP is waiting for APs to boot ({} of {})",
                num_known_cores,
                expected_cores
            );
            iter = 0;
...
fn bring_up_ap(
    ...
    ap_trampoline_data
        .ap_stack_end
        .write(ap_stack.top_unusable());
    ap_trampoline_data
        .ap_code
        .write(VirtualAddress::new_canonical(kstart_ap as usize));
    ...
    info!(
        "Bringing up AP, proc: {} apic_id: {}",
        new_lapic.processor, new_lapic.apic_id
    );
    ...
    debug!("waiting 10 ms...");
    pit_clock::pit_wait(10000).unwrap_or_else(|_e| {
        error!("bring_up_ap(): failed to pit_wait 10 ms. Error: {:?}", _e);
    });
    debug!("done waiting.");
    ...
    pit_clock::pit_wait(300).unwrap_or_else(|_e| {
        error!("bring_up_ap(): failed to pit_wait 300 us. Error {:?}", _e);
    });
    pit_clock::pit_wait(200).unwrap_or_else(|_e| {
        error!("bring_up_ap(): failed to pit_wait 200 us. Error {:?}", _e);
    });
