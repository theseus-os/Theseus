...
impl<KL> KnownLength for Box<KL>
where
    KL: KnownLength + ?Sized,
{
    fn len(&self) -> usize {
        (**self).len()
    }
}
impl<KL> KnownLength for &KL
where
    KL: KnownLength + ?Sized,
{
    fn len(&self) -> usize {
...
impl<KL> KnownLength for &mut KL
where
    KL: KnownLength + ?Sized,
{
    fn len(&self) -> usize {
...
impl<R> From<R> for ByteReaderWrapper<R>
where
    R: BlockReader,
{
...
impl<R> ByteReader for ByteReaderWrapper<R>
where
    R: BlockReader,
{
    fn read_at(&mut self, buffer: &mut [u8], offset: usize) -> Result<usize, IoError> {
        ...
        for transfer in transfers.iter().flatten() {
            let BlockByteTransfer {
                byte_range_absolute,
                block_range,
                bytes_in_block_range,
            } = transfer;
            ...
            if bytes_in_block_range.start % self.block_size() == 0
                && bytes_in_block_range.end % self.block_size() == 0
            {
            ...
            // Otherwise, we transfer a single block into a temp buffer and copy a sub-range of those bytes into `buffer`.
                ...
                buffer[buffer_range]
                    .copy_from_slice(&tmp_block_bytes[bytes_in_block_range.clone()]);
            }
        }
...
impl<R> BlockIo for ByteReaderWrapper<R>
where
    R: BlockReader,
{
    delegate! { to self.0 { fn block_size(&self) -> usize; } }
}
impl<R> KnownLength for ByteReaderWrapper<R>
where
    R: KnownLength + BlockReader,
{
    delegate! { to self.0 { fn len(&self) -> usize; } }
}
impl<R> BlockReader for ByteReaderWrapper<R>
where
    R: BlockReader,
{
...
impl<RW> From<RW> for ByteReaderWriterWrapper<RW>
where
...
impl<RW> ByteReader for ByteReaderWriterWrapper<RW>
where
    RW: BlockReader + BlockWriter,
{
    fn read_at(&mut self, buffer: &mut [u8], offset: usize) -> Result<usize, IoError> {
...
impl<RW> ByteWriter for ByteReaderWriterWrapper<RW>
where
    RW: BlockReader + BlockWriter,
{
    fn write_at(&mut self, buffer: &[u8], offset: usize) -> Result<usize, IoError> {
        ...
        for transfer in transfers.iter().flatten() {
            ...
            // 3. Write that whole block back to the underlying writer.
                ...
                let _blocks_read = self.read_blocks(&mut tmp_block_bytes, block_range.start)?;
                tmp_block_bytes[bytes_in_block_range.clone()]
                    .copy_from_slice(&buffer[buffer_range]);
...
impl<RW> BlockIo for ByteReaderWriterWrapper<RW>
where
    RW: BlockReader + BlockWriter,
{
    delegate! { to self.0 { fn block_size(&self) -> usize; } }
}
impl<RW> KnownLength for ByteReaderWriterWrapper<RW>
where
    RW: KnownLength + BlockReader + BlockWriter,
{
    delegate! { to self.0 { fn len(&self) -> usize; } }
}
impl<RW> BlockReader for ByteReaderWriterWrapper<RW>
where
    RW: BlockReader + BlockWriter,
{
    delegate! { to self.0 { fn read_blocks(&mut self, buffer: &mut [u8], block_offset: usize) -> Result<usize, IoError>; } }
}
impl<RW> BlockWriter for ByteReaderWriterWrapper<RW>
where
    RW: BlockReader + BlockWriter,
{
    delegate! { to self.0 {
...
impl<RW> From<RW> for ByteWriterWrapper<RW>
where
    RW: BlockReader + BlockWriter,
{
    fn from(block_reader_writer: RW) -> Self {
...
impl<RW> ByteWriter for ByteWriterWrapper<RW>
where
    RW: BlockReader + BlockWriter,
{
    delegate! { to self.0 { fn write_at(&mut self, buffer: &[u8], offset: usize) -> Result<usize, IoError>; } }
    fn flush(&mut self) -> Result<(), IoError> {
        ByteWriter::flush(&mut self.0)
    }
}
impl<RW> BlockIo for ByteWriterWrapper<RW>
where
    RW: BlockReader + BlockWriter,
{
    delegate! { to self.0 { fn block_size(&self) -> usize; } }
}
impl<RW> KnownLength for ByteWriterWrapper<RW>
where
    RW: KnownLength + BlockReader + BlockWriter,
{
    delegate! { to self.0 { fn len(&self) -> usize; } }
}
impl<RW> BlockWriter for ByteWriterWrapper<RW>
where
    RW: BlockReader + BlockWriter,
{
    delegate! { to self.0 { fn write_blocks(&mut self, buffer: &[u8], block_offset: usize) -> Result<usize, IoError>; } }
    fn flush(&mut self) -> Result<(), IoError> {
        BlockWriter::flush(&mut self.0)
    }
}
...
impl<IO> core2::io::Read for ReaderWriter<IO>
where
    IO: ByteReader,
{
    fn read(&mut self, buf: &mut [u8]) -> core2::io::Result<usize> {
        let bytes_read = self
            .io
            .read_at(buf, self.offset as usize)
            .map_err(Into::<core2::io::Error>::into)?;
        self.offset += bytes_read as u64;
        Ok(bytes_read)
    }
}
impl<IO> core2::io::Write for ReaderWriter<IO>
where
    IO: ByteWriter,
{
    fn write(&mut self, buf: &[u8]) -> core2::io::Result<usize> {
        let bytes_written = self
            .io
            .write_at(buf, self.offset as usize)
            .map_err(Into::<core2::io::Error>::into)?;
...
impl<IO> Seek for ReaderWriter<IO>
where
    IO: KnownLength,
{
...
impl<IO> BlockIo for Reader<IO>
where
    IO: BlockIo,
{
    delegate! { to self.0 { fn block_size(&self) -> usize; } }
}
impl<IO> KnownLength for Reader<IO>
where
    IO: KnownLength,
{
    delegate! { to self.0 { fn len(&self) -> usize; } }
}
impl<IO> BlockReader for Reader<IO>
where
    IO: BlockReader,
{
    delegate! { to self.0 { fn read_blocks(&mut self, buffer: &mut [u8], block_offset: usize) -> Result<usize, IoError>; } }
}
impl<IO> ByteReader for Reader<IO>
where
    IO: ByteReader,
{
    delegate! { to self.0 { fn read_at(&mut self, buffer: &mut [u8], offset: usize) -> Result<usize, IoError>; } }
}
impl<IO> core2::io::Read for Reader<IO>
where
    IO: ByteReader,
{
    delegate! { to self.0 { fn read(&mut self, buf: &mut [u8]) -> core2::io::Result<usize>; } }
}
impl<IO> Seek for Reader<IO>
where
    IO: KnownLength,
{
...
impl<IO> BlockIo for Writer<IO>
where
    IO: BlockIo,
{
    delegate! { to self.0 { fn block_size(&self) -> usize; } }
}
impl<IO> KnownLength for Writer<IO>
where
    IO: KnownLength,
{
    delegate! { to self.0 { fn len(&self) -> usize; } }
}
impl<IO> BlockWriter for Writer<IO>
where
    IO: BlockWriter,
{
    delegate! { to self.0 {
...
impl<IO> ByteWriter for Writer<IO>
where
    IO: ByteWriter,
{
    delegate! { to self.0 {
...
impl<IO> core2::io::Write for Writer<IO>
where
    IO: ByteWriter,
{
    delegate! { to self.0 { fn write(&mut self, buf: &[u8]) -> core2::io::Result<usize>; } }
    fn flush(&mut self) -> core2::io::Result<()> {
        core2::io::Write::flush(&mut self.0)
    }
}
impl<IO> Seek for Writer<IO>
where
    IO: KnownLength,
{
    delegate! { to self.0 { fn seek(&mut self, position: SeekFrom) -> core2::io::Result<u64>; } }
    ...
    while curr_byte < byte_range.end {
        ...
        // Otherwise, if the curr_byte is NOT block-aligned, then we can only do a single-block transfer.
            ...
            transfers[transfer_idx] = Some(BlockByteTransfer {
                ...
                bytes_in_block_range: offset_into_curr_block
                    ..(offset_into_curr_block + (end_byte - curr_byte)),
            });
            transfer_idx += 1;
            curr_byte = end_byte;
        }
