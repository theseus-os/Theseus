...
pub fn acpi_table_handler(
    ...
    match &signature {
        ...
        rsdt::RSDT_SIGNATURE | rsdt::XSDT_SIGNATURE => {
            rsdt::handle(acpi_tables, signature, length, phys_addr)
        }
        ...
        _ => {
            warn!(
                "Skipping unsupported ACPI table {:?}",
                core::str::from_utf8(&signature).unwrap_or("Unknown Signature")
            );
            Ok(())
