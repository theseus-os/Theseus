...
use alloc::boxed::Box;
use memory::{
    allocate_frames_by_bytes_at, allocate_pages, EntryFlags, MappedPages, PageTable,
    PhysicalAddress,
};
use owning_ref::BoxRefMut;
use sdt::{GenericAddressStructure, Sdt};
...
impl HpetAcpiTable {
    ...
    pub fn init_hpet(
    ...
    ) -> Result<&'static RwLock<BoxRefMut<MappedPages, Hpet>>, &'static str> {
        ...
        let pages =
            allocate_pages(frames.size_in_frames()).ok_or("Couldn't allocate pages for HPET")?;
        ...
        let mut hpet = BoxRefMut::new(Box::new(hpet_mp))
            .try_map_mut(|mp| mp.as_type_mut::<Hpet>(phys_addr.frame_offset()))?;
