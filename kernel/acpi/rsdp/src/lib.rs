...
use memory::{
    ...
    PageTable, PhysicalAddress, PteFlags, Page4K,
};
...
impl Rsdp {
    ...
    pub fn get_rsdp(page_table: &mut PageTable) -> Result<BorrowedMappedPages<Rsdp>, &'static str> {
        ...
        let frames_to_search = allocate_frames_by_bytes_at::<Page4K>(PhysicalAddress::new_canonical(RSDP_SEARCH_START), size)
    ...
    pub fn from_address(
        ...
        let pages = allocate_pages_by_bytes(size).ok_or("couldn't allocate pages")?;
        let frames = allocate_frames_by_bytes_at::<Page4K>(address, size)
