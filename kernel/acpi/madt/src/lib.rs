...
use apic::{LocalApic, get_bootstrap_core_id, LapicInitError, get_my_core_id};
use sdt::Sdt;
...
fn handle_bsp_lapic_entry(madt_iter: MadtIter, page_table: &mut PageTable) -> Result<(), &'static str> {
    ...
    for madt_entry in madt_iter.clone() {
        if let MadtEntry::LocalApic(lapic_entry) = madt_entry { 
            ...
            assert!(get_my_core_id() == lapic_entry.apic_id);
    ...
    let bsp_id = get_bootstrap_core_id().ok_or("handle_bsp_lapic_entry(): Couldn't find BSP LocalApic in Madt!")?;
