...
use crate_metadata::{write_relocation, RelocationEntry, StrongCrateRef, StrongSectionRef};
use fs_node::WeakFileRef;
use gimli::{
    read::{
        DebugAbbrev, DebugInfo, DebugLine, DebugLoc, DebugPubNames, DebugPubTypes, DebugRanges,
        DebugStr, Reader,
    },
    EndianSlice, NativeEndian, RawRngListEntry, SectionId,
};
use goblin::elf::reloc::*;
use hashbrown::{HashMap, HashSet};
use memory::{allocate_pages_by_bytes, EntryFlags, MappedPages, MmiRef, VirtualAddress};
use mod_mgmt::{find_symbol_table, CrateNamespace};
...
impl DebugSections {
    ...
    pub fn debug_loc(&self) -> Option<DebugLoc<EndianSlice<NativeEndian>>> {
        self.debug_loc
            .as_ref()
            .map(|loc| DebugLoc::new(&loc.0, NativeEndian))
    }
    ...
    fn handle_node<R: Reader>(
        ...
        if tag == gimli::DW_TAG_lexical_block {
            ...
            let (starting_vaddr, ending_vaddr) = {
                if let Some(gimli::AttributeValue::RangeListsRef(ranges_offset)) =
                    entry.attr_value(gimli::DW_AT_ranges)?
                {
                    let range_lists_offset = context
                        .dwarf
                        .ranges_offset_from_raw(&context.unit, ranges_offset);
                    let mut raw_ranges = context
                        .dwarf
                        .raw_ranges(&context.unit, range_lists_offset)?;
                    debug!(
                        "{:indent$}--Lexical Block range list:",
                        "",
                        indent = ((depth) * 2)
                    );
                    ...
                    while let Some(r) = raw_ranges.next()? {
                        ...
                        if first_range.is_none() {
                            first_range = Some((begin, end));
                        }
                    }
                    first_range
                        .map(|(begin, end)| (begin as usize, end as usize))
                        .expect("range list iter was empty")
                } else if let (Some(low_pc), Some(high_pc)) = (
                    ...
                    let starting_vaddr = match low_pc {
                        ...
                        unsupported => panic!(
                            "unsupported AttributeValue type for low_pc: {:?}",
                            unsupported
                        ),
                    };
                    let size_of_lexical_block = match high_pc {
                        ...
                        unsupported => panic!(
                            "unsupported AttributeValue type for high_pc: {:?}",
                            unsupported
                        ),
                    };
            ...
            debug!(
                "{:indent$}--Lexical Block ranges from {:#X} to {:#X} (size {:#X} bytes)",
                "",
                starting_vaddr,
                ending_vaddr,
                ending_vaddr - starting_vaddr,
                indent = ((depth) * 2)
            );
            ...
            if enclosing_range.contains(&instruction_pointer.value()) {
                warn!(
                    "{:indent$}  --> Lexical block contains instr ptr!",
                    "",
                    indent = ((depth) * 2)
                );
        ...
        // Those are the variables that we want to drop.
            ...
            let variable_name = entry
                ...
                .expect("Variable DIE didn't have a DW_AT_name attribute")
                .string_value(context.debug_str_sec)
                .expect("Couldn't convert variable name attribute value to string")
                .to_string()
                .map(|s| String::from(s))?;
            debug!(
                "{:indent$}Variable {:?}",
            ...
            if let Some(loc_attr) = entry.attr(gimli::DW_AT_location)? {
                match loc_attr.value() {
                    gimli::AttributeValue::LocationListsRef(loc_lists_offset) => {
                        let mut locations =
                            context.dwarf.locations(&context.unit, loc_lists_offset)?;
                        while let Some(location) = locations.next()? {
                            ...
                            warn!(
                                "{:indent$}Variable {:?} MAY NEED HANDLING FOR LOCATION {:X?}",
                                "",
                                variable_name,
                                location,
                                indent = ((depth + 1) * 2)
                    ...
                    unsupported => {
                        panic!("Unsupported DW_AT_location attr value: {:?}", unsupported)
                    }
                }
            } else {
            ...
            if let Some(location) = latest_variable_location {
                ...
                let _type_signature = match entry.attr_value(gimli::DW_AT_type)? {
                    ...
                    unexpected => {
                        warn!(
                            "{:indent$}unexpected DW_AT_type attribute value: {:X?}",
                            "",
                            unexpected,
                            indent = ((depth) * 2)
                        );
        ...
        while let Some(child_subtree) = children.next()? {
            if let Some(offset) = self.handle_node(
                instruction_pointer,
                enclosing_range.clone(),
                depth + 1,
                child_subtree,
                context,
            )? {
    ...
    fn handle_subprogram_node<R: Reader>(
        ...
        if tag == gimli::constants::DW_TAG_subprogram {
            let _subprogram_name = entry.attr(gimli::DW_AT_name)?.and_then(|attr| {
                attr.string_value(context.debug_str_sec)
                    .and_then(|s| s.to_string().ok().map(|s| String::from(s)))
            });
            let _subprogram_linkage_name =
                entry.attr(gimli::DW_AT_linkage_name)?.and_then(|attr| {
                    attr.string_value(context.debug_str_sec)
                        .and_then(|s| s.to_string().ok().map(|s| String::from(s)))
                });
            ...
            let starting_vaddr = match entry.attr_value(gimli::DW_AT_low_pc)? {
                ...
                Some(unsupported) => panic!(
                    "unsupported AttributeValue type for low_pc: {:?}",
                    unsupported
                ),
                _ => {
                    debug!(
                        "{:indent$}--Subprogram {:?}({:?}) did not have attribute DW_AT_low_pc",
                        "",
                        _subprogram_name,
                        _subprogram_linkage_name,
                        indent = (depth * 2)
                    );
                    return Ok(None);
                }
            ...
            let size_of_subprogram = match entry.attr_value(gimli::DW_AT_high_pc)? {
                ...
                _ => {
                    debug!(
                        "{:indent$}--Subprogram {:?}({:?}) did not have attribute DW_AT_high_pc",
                        "",
                        _subprogram_name,
                        _subprogram_linkage_name,
                        indent = (depth * 2)
            ...
            debug!(
                "{:indent$}--Subprogram {:?}({:?}) ranges from {:#X} to {:#X} (size {:#X} bytes)",
                "",
                _subprogram_name,
                _subprogram_linkage_name,
                starting_vaddr,
            ...
            if instruction_pointer.value() >= starting_vaddr
                && instruction_pointer.value() < ending_vaddr
            {
                warn!(
                    "{:indent$}--Found matching subprogram at {:?}",
                    "",
                    entry.offset(),
                    indent = ((depth) * 2)
                );
        ...
        while let Some(child_subtree) = children.next()? {
            if let Some(offset) =
                self.handle_subprogram_node(instruction_pointer, depth + 1, child_subtree, context)?
            {
                return Ok(Some(offset));
            }
        }
    ...
    ///
    /// Otherwise, an error is returned upon failure, e.g., a problem parsing the debug sections.
    pub fn find_subprogram_containing(
        &self,
        instruction_pointer: VirtualAddress,
    ) -> gimli::Result<Option<gimli::DebugInfoOffset>> {
        ...
        while let Some(uh) = units.next()? {
            ...
            if let Some(_offset) =
                self.handle_subprogram_node(instruction_pointer, 0, node, &context)?
            {
...
impl DebugSymbols {
    ...
    pub fn load(
        ...
        let (mut debug_sections_mp, _debug_sections_vaddr_range) =
            allocate_debug_section_pages(&elf_file, &kernel_mmi_ref)?;
        debug!(
            "debug sections spans {:#X} to {:#X}  (size: {:#X} bytes)",
        ...
        for (shndx, sec) in elf_file.section_iter().enumerate() {
            ...
            if Ok(SectionId::DebugStr.name()) == sec_name {
                ...
                debug_abbrev = Some(DebugSection {
                    shndx,
                    virt_addr,
                    mp_offset,
                    size,
                ...
                debug_ranges = Some(DebugSection {
                    shndx,
                    virt_addr,
                    mp_offset,
                    size,
                ...
                debug_pubnames = Some(DebugSection {
                    shndx,
                    virt_addr,
                    mp_offset,
                    size,
                ...
                debug_pubtypes = Some(DebugSection {
                    shndx,
                    virt_addr,
                    mp_offset,
                    size,
        ...
        if true {
            debug!(
                "Section .debug_str loaded from {:#X} to {:#X} (size {:#X} bytes)",
                debug_str.virt_addr,
                debug_str.virt_addr + debug_str.size,
                debug_str.size
            );
            if let Some(ref loc) = debug_loc {
                debug!(
                    "Section .debug_loc loaded from {:#X} to {:#X} (size {:#X} bytes)",
                    loc.virt_addr,
                    loc.virt_addr + loc.size,
                    loc.size
                );
            } else {
            ...
            debug!(
                "Section .debug_abbrev loaded from {:#X} to {:#X} (size {:#X} bytes)",
                debug_abbrev.virt_addr,
                debug_abbrev.virt_addr + debug_abbrev.size,
                debug_abbrev.size
            );
            debug!(
                "Section .debug_info loaded from {:#X} to {:#X} (size {:#X} bytes)",
                debug_info.virt_addr,
                debug_info.virt_addr + debug_info.size,
                debug_info.size
            );
            debug!(
                "Section .debug_ranges loaded from {:#X} to {:#X} (size {:#X} bytes)",
                debug_ranges.virt_addr,
                debug_ranges.virt_addr + debug_ranges.size,
                debug_ranges.size
            );
            debug!(
                "Section .debug_pubnames loaded from {:#X} to {:#X} (size {:#X} bytes)",
                debug_pubnames.virt_addr,
                debug_pubnames.virt_addr + debug_pubnames.size,
                debug_pubnames.size
            );
            debug!(
                "Section .debug_pubtypes loaded from {:#X} to {:#X} (size {:#X} bytes)",
                debug_pubtypes.virt_addr,
                debug_pubtypes.virt_addr + debug_pubtypes.size,
                debug_pubtypes.size
            );
            debug!(
                "Section .debug_line loaded from {:#X} to {:#X} (size {:#X} bytes)",
                debug_line.virt_addr,
                debug_line.virt_addr + debug_line.size,
                debug_line.size
            );
        }
        ...
        for sec in elf_file
            .section_iter()
            .filter(|sec| sec.get_type() == Ok(ShType::Rela))
        {
            ...
            for rela_entry in rela_array {
                if false {
                    trace!(
                        "      Rela64 offset: {:#X}, addend: {:#X}, symtab_index: {}, type: {:#X}",
                ...
                let (source_sec_vaddr, source_sec_dep) = match shndx_map
                    .get(&source_sec_shndx)
                    .map(|s| (s.virt_addr, None))
                    .or_else(|| {
                        loaded_crate
                            .lock_as_ref()
                            .sections
                            .get(&source_sec_shndx)
                            .map(|sec| (sec.address_range.start, Some(sec.clone())))
                    }) {
                    ...
                    None => {
                        if let Ok(source_sec_name) = source_sec_entry.get_name(&elf_file) {
                            ...
                            warn!(
                                "Looking for foreign relocation source section {:?}",
                                demangled
                            );
                ...
                if !source_and_target_in_same_crate {
                    warn!(
                        "Found foreign dependency on source section {:?}",
                        source_sec_dep
                    );
        ...
        let create_debug_section_slice =
            |debug_sec: DebugSection| -> Result<DebugSectionSlice, &'static str> {
        ...
        let loaded = DebugSections {
            ...
            debug_loc: if let Some(loc) = debug_loc {
                Some(create_debug_section_slice(loc)?)
            } else {
                None
            },
        ...
        *self = Self::Loaded(loaded);
        match self {
            Self::Loaded(d) => Ok(d),
            Self::Unloaded(_) => Err(
                "BUG: unreachable: debug sections were loaded but DebugSymbols enum was wrong type",
            ),
        }
    }
...
fn allocate_debug_section_pages(
    elf_file: &ElfFile,
    kernel_mmi_ref: &MmiRef,
) -> Result<(MappedPages, Range<VirtualAddress>), &'static str> {
    let mut ro_bytes = 0;
    ...
    let mp = kernel_mmi_ref
        .lock()
        .page_table
        .map_allocated_pages(allocated_pages, EntryFlags::PRESENT | EntryFlags::WRITABLE)?;
...
fn write_relocation_debug(
    ...
    match relocation_entry.typ {
        R_X86_64_32 => {
            ...
            if verbose_log {
                trace!("                    target_ptr: {:#X}, source_val: {:#X} (ignoring source_sec_vaddr {:#X})", target_ref as *mut _ as usize, source_val, source_sec_vaddr);
            }
