...
impl IoApic {
    ...
    pub fn new(
        ...
        let frame = allocate_frames_at(phys_addr, 1)
            .map_err(|_e| "Couldn't allocate physical frame for IOAPIC")?;
        ...
        let ioapic_regs = BoxRefMut::new(Box::new(ioapic_mapped_page))
            .try_map_mut(|mp| mp.as_type_mut::<IoApicRegisters>(0))?;
        let ioapic = IoApic {
            regs: ioapic_regs,
            id: id,
        ...
        debug!(
            "Created new IoApic, id: {}, gsi_base: {}, phys_addr: {:#X}",
            id, gsi_base, phys_addr
        );
    ...
    pub fn handles_irq(&self, irq_num: u32) -> bool {
        (irq_num >= self.gsi_base) && (irq_num < (self.gsi_base + INTERRUPT_ENTRIES_PER_IOAPIC))
    }
