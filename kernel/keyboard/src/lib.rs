...
use keycodes_ascii::{KeyAction, KeyEvent, KeyboardModifiers, Keycode, KEY_RELEASED_OFFSET};
use mpmc::Queue;
use ps2::{init_ps2_port1, keyboard_detect, keyboard_led, test_ps2_port1, KeyboardType};
use spin::Once;
...
pub fn init(keyboard_queue_producer: Queue<Event>) {
    ...
    match keyboard_detect() {
        ...
        Ok(s) => match s {
            KeyboardType::AncientATKeyboard => {
                info!("Ancient AT Keyboard with translator enabled in the PS/2 Controller")
            }
            KeyboardType::MF2Keyboard => info!("MF2Keyboard"),
            KeyboardType::MF2KeyboardWithPSControllerTranslator => {
                info!("MF2 Keyboard with translator enabled in PS/2 Controller")
            }
        },
    }
...
pub fn handle_keyboard_input(scan_code: u8, extended: bool) -> Result<(), &'static str> {
    ...
    match scan_code {
        ...
        x if x == Keycode::Alt as u8 + KEY_RELEASED_OFFSET => {
            modifiers.remove(KeyboardModifiers::ALT);
        }
        x if x == Keycode::LeftShift as u8 + KEY_RELEASED_OFFSET => {
            modifiers.remove(KeyboardModifiers::SHIFT_LEFT);
        }
        x if x == Keycode::RightShift as u8 + KEY_RELEASED_OFFSET => {
            modifiers.remove(KeyboardModifiers::SHIFT_RIGHT);
        }
        x if x == Keycode::SuperKeyLeft as u8 + KEY_RELEASED_OFFSET => {
            modifiers.remove(KeyboardModifiers::SUPER_KEY_LEFT);
        }
        x if x == Keycode::SuperKeyRight as u8 + KEY_RELEASED_OFFSET => {
    ...
    match scan_code {
        x => {
            ...
            match keycode {
                Some(keycode) => {
                    let event = Event::new_keyboard_event(KeyEvent::new(
                        keycode,
                        action,
                        modifiers.clone(),
                    ));
                    if let Some(producer) = KEYBOARD_PRODUCER.get() {
                        producer
                            .push(event)
                            .map_err(|_e| "keyboard input queue is full")
                    } else {
