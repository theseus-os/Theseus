...
pub fn init(end_vaddr_of_low_designated_region: VirtualAddress) -> Result<(), &'static str> {
    ...
    let designated_low_end = DESIGNATED_PAGES_LOW_END
        .call_once(|| Page::containing_address(end_vaddr_of_low_designated_region));
    ...
    let initial_free_chunks = [
        ...
        Some(Chunk {
            pages: PageRange::new(designated_low_end + 1, DESIGNATED_PAGES_HIGH_START - 1),
        }),
        Some(Chunk {
            pages: PageRange::new(
                ...
                Page::containing_address(VirtualAddress::new_canonical(
                    KERNEL_TEXT_START - ADDRESSABILITY_PER_P4_ENTRY - 1,
                )),
            ),
        }),
    ...
    fn from(alloc_err: AllocationError) -> &'static str {
        match alloc_err {
            AllocationError::AddressNotFree(..) => {
                "address was in use or outside of this page allocator's range"
            }
...
fn find_any_chunk<'list>(
    ...
    warn!(
        "PageAllocator: unlikely scenario: non-designated chunks are all allocated, \
		  falling back to allocating {} pages from designated regions!",
        num_pages
    );
    match list.0 {
        Inner::Array(ref mut arr) => {
...
pub fn allocate_pages_by_bytes_at(
    ...
    allocate_pages_by_bytes_deferred(Some(vaddr), num_bytes).map(|(ap, _action)| ap)
}
...
pub fn allocate_pages_at(
    ...
    allocate_pages_deferred(Some(vaddr), num_pages).map(|(ap, _action)| ap)
}
