...
macro_rules! implement_page_frame {
    ($TypeName:ident, $desc:literal, $prefix:literal, $address:ident) => {
        paste! { // using the paste crate's macro for easy concatenation
            ...
            #[doc = "A version of `" $TypeName "` that encodes its [`PageSize`]
                with internal enum variants."]
            #[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord)]
            pub enum [<$TypeName Sized>] {
                Normal4K($TypeName<Page4K>),
                Huge2M(  $TypeName<Page2M>),
                Huge1G(  $TypeName<Page1G>),
            }
            impl From<$TypeName<Page4K>> for [<$TypeName Sized>] {
                fn from(chunk: $TypeName<Page4K>) -> Self {
                    Self::Normal4K(chunk)
                }
            }
            impl From<$TypeName<Page2M>> for [<$TypeName Sized>] {
                fn from(chunk: $TypeName<Page2M>) -> Self {
                    Self::Huge2M(chunk)
                }
            }
            impl From<$TypeName<Page1G>> for [<$TypeName Sized>] {
                fn from(chunk: $TypeName<Page1G>) -> Self {
                    Self::Huge1G(chunk)
                }
            }
            impl [<$TypeName Sized>] {
                pub const fn number(&self) -> usize {
                    chunk_sized_expr!(Self, self, .number)
                }
                pub const fn start_address(&self) -> $address {
                    chunk_sized_expr!(Self, self, .start_address())
                }
                #[doc = "Converts this `" [<$TypeName Sized>] "` into an identical
                    4K-sized chunk with the same internal number."]
                pub const fn [<into_4k_ $TypeName:lower>](self) -> $TypeName<Page4K> {
                    // To make this a const fn, we cannot use the implementations of `Into`.
                    match self {
                        Self::Normal4K(c) => c,
                        Self::Huge2M(c)   => $TypeName::<Page4K> { number: c.number, size: PhantomData },
                        Self::Huge1G(c)   => $TypeName::<Page4K> { number: c.number, size: PhantomData },
                    }
                }
            }            
        }
