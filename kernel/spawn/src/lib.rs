...
fn remove_current_task_from_runqueue(current_task: &TaskRef) {
    ...
    #[cfg(not(rq_eval))] {
        if let Err(e) = runqueue::get_runqueue(multicore::current_cpu())
...
pub fn create_idle_task() -> Result<JoinableTaskRef, &'static str> {
    let apic_id = multicore::current_cpu();
...
fn idle_task_entry(_apic_id: u8) {
    info!("Entered idle task loop on core {}: {:?}", multicore::current_cpu(), task::get_my_current_task());
    loop {
