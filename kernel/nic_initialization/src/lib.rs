...
use memory::{
    ...
    get_kernel_mmi_ref, EntryFlags, MappedPages, PhysicalAddress,
};
...
pub fn allocate_device_register_memory(
    dev: &PciDevice,
...
pub fn allocate_memory(
    ...
    let kernel_mmi_ref =
        get_kernel_mmi_ref().ok_or("NicInit::mem_map(): KERNEL_MMI was not yet initialized!")?;
    let mut kernel_mmi = kernel_mmi_ref.lock();
    let nic_mapped_page =
        kernel_mmi
            .page_table
            .map_allocated_pages_to(pages_nic, frames_nic, NIC_MAPPING_FLAGS)?;
...
pub fn init_rx_queue<T: RxDescriptor, S: RxQueueRegisters>(
    ...
    let (rx_descs_mapped_pages, rx_descs_starting_phys_addr) =
        create_contiguous_mapping(size_in_bytes_of_all_rx_descs_per_queue, NIC_MAPPING_FLAGS)?;
    ...
    for rd in rx_descs.iter_mut() {
        ...
        let rx_buf = rx_buffer_pool
            ...
            .or_else(|_e| {
                create_contiguous_mapping(buffer_size, NIC_MAPPING_FLAGS).map(
                    |(buf_mapped, buf_paddr)| {
                        ReceiveBuffer::new(
                            buf_mapped,
                            buf_paddr,
                            buffer_size as u16,
...
pub fn init_tx_queue<T: TxDescriptor, S: TxQueueRegisters>(
    num_desc: usize,
    txq_regs: &mut S,
    ...
    let (tx_descs_mapped_pages, tx_descs_starting_phys_addr) =
        create_contiguous_mapping(size_in_bytes_of_all_tx_descs, NIC_MAPPING_FLAGS)?;
