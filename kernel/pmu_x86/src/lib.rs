...
use raw_cpuid::*;
use spin::Once;
use x86_64::{registers::model_specific::Msr, structures::idt::InterruptStackFrame, VirtAddr};
...
static CORES_SAMPLING: [AtomicU64; WORDS_IN_BITMAP] = [
    AtomicU64::new(0),
    AtomicU64::new(0),
    AtomicU64::new(0),
    AtomicU64::new(0),
...
static RESULTS_READY: [AtomicU64; WORDS_IN_BITMAP] = [
    AtomicU64::new(0),
    AtomicU64::new(0),
    AtomicU64::new(0),
    AtomicU64::new(0),
...
fn init_registers() {
    unsafe {
        ...
        Msr::new(IA32_PERF_GLOBAL_CTRL)
            .write(ENABLE_FIXED_PERFORMANCE_COUNTERS | ENABLE_GENERAL_PERFORMANCE_COUNTERS);
    }
}
...
fn free_counter(core_id: u8, counter: u8) {
    let pmcs = get_pmcs_available_for_core(core_id)
        .expect("Trying to free a PMU counter when the PMU is not initialized");

    pmcs.fetch_update(Ordering::SeqCst, Ordering::SeqCst, |mut x| {
...
fn check_pmu_availability() -> Result<(), &'static str> {
    ...
    if !CORES_INITIALIZED.lock().contains(&core_id) {
        if *PMU_VERSION >= MIN_PMU_VERSION {
            error!(
                "PMU version {} is available. It still needs to be initialized on this core",
                *PMU_VERSION
            );
...
pub fn retrieve_samples() -> Result<SampleResults, &'static str> {
    ...
    let mut samples = sampling_info.get_mut(&my_core_id).ok_or(
        "pmu_x86::retrieve_samples: could not retrieve sampling information for this core",
    )?;
    ...
    instruction_pointers.extend(
        samples
            .ip_list
            .iter()
            .map(|va| memory::VirtualAddress::new_canonical(va.as_u64() as usize)),
    );

    Ok(SampleResults {
        instruction_pointers,
        task_ids: samples.task_id_list.clone(),
    })
}
...
pub fn print_samples(sample_results: &SampleResults) {
    ...
    for results in sample_results
        .instruction_pointers
        .iter()
        .zip(sample_results.task_ids.iter())
    {
...
pub fn find_function_names_from_samples(
    ...
    let taskref = task::get_my_current_task().ok_or(
        "pmu_x86::get_function_names_from_samples: Could not get reference to current task",
    )?;
    ...
    for ip in sample_results.instruction_pointers.iter() {
        let (section_ref, _offset) = namespace
            .get_section_containing_address(*ip, false)
...
pub fn handle_sample(stack_frame: &InterruptStackFrame) -> Result<bool, &'static str> {
    ...
    if let Some(my_apic) = apic::get_my_apic() {
        ...
        error!(
            "Error in Performance Monitoring! Reference to the local APIC could not be retrieved."
        );
    }
