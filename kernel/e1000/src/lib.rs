...
use alloc::boxed::Box;
use memory::{get_kernel_mmi_ref,FRAME_ALLOCATOR, VirtualAddress, PhysicalAddress, Frame, PageTable, EntryFlags, FrameAllocator, allocate_pages, MappedPages, FrameIter, PhysicalMemoryArea};
...
impl NetworkCard for Nic {
    ...
    fn send_packet(&mut self, packet_vaddr: VirtualAddress, size_in_bytes: u16) -> Result<(), &'static str> {
        ...
        self.tx_descs[self.tx_cur as usize].addr = packet_paddr as u64;
...
impl Nic{
    ...
    fn read_mac_address_from_nic(&mut self) -> Result<(), &'static str> {
...
pub fn init_nic(e1000_pci: &PciDevice) -> Result<(), &'static str>{
    ...
    e1000_nc.read_mac_address_from_nic()?;
    //e1000_nc.clear_multicast();
    //e1000_nc.clear_statistics();for 
...
extern "x86-interrupt" fn e1000_handler(_stack_frame: &mut ExceptionStackFrame) {
    ...
    match nic.handle_interrupt() {
        Err(e) => error!("e1000_handler(): error handling interrupt: {:?}", e),
        _ => { }
    };
