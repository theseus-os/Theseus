...
use pci::{PciConfigSpaceAccessMechanism, PciDevice, PCI_INTERRUPT_LINE};
use spin::Once;
...
impl E1000Nic {
    ...
    pub fn init(
    ...
    ) -> Result<&'static MutexIrqSafe<E1000Nic>, &'static str> {
        ...
        let (mut mapped_registers, rx_registers, tx_registers, mut mac_registers) =
            Self::map_e1000_regs(e1000_pci_dev, mem_base)?;
    ...
    > {
        ...
        let nic_rx_regs_mapped_page = allocate_memory(
            mem_base + GENERAL_REGISTERS_SIZE_BYTES,
            RX_REGISTERS_SIZE_BYTES,
        )?;
        let nic_tx_regs_mapped_page = allocate_memory(
            mem_base + GENERAL_REGISTERS_SIZE_BYTES + RX_REGISTERS_SIZE_BYTES,
            TX_REGISTERS_SIZE_BYTES,
        )?;
        let nic_mac_regs_mapped_page = allocate_memory(
            mem_base
                + GENERAL_REGISTERS_SIZE_BYTES
                + RX_REGISTERS_SIZE_BYTES
                + TX_REGISTERS_SIZE_BYTES,
            MAC_REGISTERS_SIZE_BYTES,
        )?;

        let regs = BoxRefMut::new(Box::new(nic_regs_mapped_page))
            .try_map_mut(|mp| mp.as_type_mut::<E1000Registers>(0))?;
        let rx_regs = BoxRefMut::new(Box::new(nic_rx_regs_mapped_page))
            .try_map_mut(|mp| mp.as_type_mut::<E1000RxRegisters>(0))?;
        let tx_regs = BoxRefMut::new(Box::new(nic_tx_regs_mapped_page))
            .try_map_mut(|mp| mp.as_type_mut::<E1000TxRegisters>(0))?;
        let mac_regs = BoxRefMut::new(Box::new(nic_mac_regs_mapped_page))
            .try_map_mut(|mp| mp.as_type_mut::<E1000MacRegisters>(0))?;
    ...
    > {
        ...
        let (rx_descs, rx_bufs_in_use) = init_rx_queue(
            E1000_NUM_RX_DESC as usize,
            &RX_BUFFER_POOL,
            E1000_RX_BUFFER_SIZE_IN_BYTES as usize,
            rx_regs,
        )?;
    ...
    fn handle_interrupt(&mut self) -> Result<(), &'static str> {
        ...
        if !handled {
            error!(
                "e1000::handle_interrupt(): unhandled interrupt!  status: {:#X}",
                status
            );
        }
