...
use apic::{core_count, get_my_apic, LapicIpiDestination, LocalApic};
...
pub fn handle_tlb_shootdown_ipi(pages_to_invalidate: PageRange) {
    ...
    for page in pages_to_invalidate {
        x86_64::instructions::tlb::flush(
            x86_64::VirtAddr::new(page.start_address().value() as u64),
        );
    }
...
pub fn send_tlb_shootdown_ipi(my_lapic: &mut LocalApic, pages_to_invalidate: PageRange) {
    ...
    loop {
        match TLB_SHOOTDOWN_IPI_LOCK.compare_exchange_weak(
            old_lock_val,
            true,
            Ordering::AcqRel,
            Ordering::Relaxed,
