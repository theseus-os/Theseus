...
use memory_structs::{
    AggregatedSectionMemoryBounds, PhysicalAddress, SectionMemoryBounds, VirtualAddress,
};
...
pub fn get_modules_address(boot_info: &BootInformation) -> (PhysicalAddress, PhysicalAddress) {
    ...
    (
        PhysicalAddress::new_canonical(mod_min),
        PhysicalAddress::new_canonical(mod_max),
    )
}
...
> {
    ...
    for section in elf_sections_tag.sections() {
        ...
        if section.size() == 0 || !section.is_allocated() || section.name().starts_with(".debug") {
            continue;
        }
        ...
        let start_phys_addr = PhysicalAddress::new(start_phys_addr)
            .ok_or("section had invalid starting physical address")?;
        let start_virt_addr = VirtualAddress::new(start_virt_addr)
            .ok_or("section had invalid ending physical address")?;
        ...
        let static_str_name = match section.name() {
            ...
            _ => {
                error!(
                    "Section {} at {:#X}, size {:#X} was not an expected section",
                    section.name(),
                    section.start_address(),
                    section.size()
                );
        ...
        debug!(
            "     will map kernel section {:?} as {:?} at vaddr: {:#X}, size {:#X} bytes",
            section.name(),
            static_str_name,
            start_virt_addr,
            section.size()
...
pub fn get_vga_mem_addr() -> Result<(PhysicalAddress, usize, EntryFlags), &'static str> {
    ...
    Ok((
        PhysicalAddress::new(VGA_DISPLAY_PHYS_START)
            .ok_or("invalid VGA starting physical address")?,
        vga_size_in_bytes,
        vga_display_flags,
    ))
}
...
pub fn get_p4() -> PhysicalAddress {
    PhysicalAddress::new_canonical(Cr3::read_raw().0.start_address().as_u64() as usize)
}
