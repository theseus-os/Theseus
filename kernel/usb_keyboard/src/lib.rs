...
use usb_uhci::{box_dev_req,box_config_desc,box_device_desc,box_inter_desc,box_endpoint_desc,map};
...
pub fn init(active_table: &mut ActivePageTable, device:& UsbDevice)-> Result<(),&'static str>{
    ...
    let k_buffer = box_keyboard_buffer(active_table,data_buffer_pointer)?;
...
pub fn box_keyboard_buffer(active_table: &mut ActivePageTable, frame_base: PhysicalAddress)
    ...
    let buffer: BoxRefMut<MappedPages, [ReadOnly<u8>;8]>  = BoxRefMut::new(Box::new(map(active_table,frame_base)?))
        .try_map_mut(|mp| mp.as_type_mut::<[ReadOnly<u8>;8]>(0))?;
