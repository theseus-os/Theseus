...
use spin::Once;
use x86_64::structures::idt::{HandlerFunc, InterruptDescriptorTable, InterruptStackFrame}; //, CONFIG_RTC_FREQUENCY_HZ};
                                                                                           // use rtc;
use apic::{InterruptChip, INTERRUPT_CHIP};
...
pub fn init(
    ...
    // preserve any handlers that were already registered to this `IDT` during early boot and device init.
    {
        ...
        let double_fault_options = new_idt
            .double_fault
            .set_handler_fn(exceptions_early::double_fault_handler);
        unsafe {
        ...
        for (_idx, new_entry) in new_idt.slice_mut(32..=255).iter_mut().enumerate() {
            if new_entry.handler_addr().as_u64() != 0 {
                debug!(
                    "Preserved early registered interrupt handler for IRQ {:#X} at address {:#X}",
                    _idx + IRQ_BASE_OFFSET as usize,
                    new_entry.handler_addr(),
                );
            } else {
...
fn set_handlers(idt: &mut InterruptDescriptorTable) {
    ...
    idt[apic::APIC_SPURIOUS_INTERRUPT_VECTOR as usize]
...
pub fn register_interrupt(interrupt_num: u8, func: HandlerFunc) -> Result<(), u64> {
    ...
    if existing_handler_addr == 0 || existing_handler_addr == unimplemented_interrupt_handler as u64
    {
        ...
        trace!(
            "register_interrupt: the requested interrupt IRQ {} was already in use",
            interrupt_num
        );
    ...
    if indicator & 0x01 == 0x01 {
        ...
        if indicator & 0x20 != 0x20 {
            ...
            if scan_code == 0xE0 {
                ...
                // a regular scancode, go ahead and handle it
                ...
                if scan_code != 0 {
                    ...
                    if let Err(e) = keyboard::handle_keyboard_input(scan_code, extended) {
                        error!(
                            "ps2_keyboard_handler: error handling PS2_PORT input: {:?}",
                            e
                        );
                    }
                }
