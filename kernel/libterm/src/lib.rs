...
impl Terminal {
    ...
    fn display_text(&mut self) -> Result<(), &'static str> {
        ...
        let area_to_render = self
            .text_display
            .display(coord, self.window.framebuffer_mut().deref_mut())?;
...
impl Terminal {
    ...
    pub fn get_event(&mut self) -> Option<Event> {
        match self.window.handle_event() {
            ...
            Err(_e) => {
                error!(
                    "Terminal::get_event(): error in the window's event handler: {:?}.",
                    _e
                );
    ...
    pub fn resize(&mut self, new_position: Rectangle) -> Result<(), &'static str> {
        self.text_display
            .set_size(new_position.width(), new_position.height());
