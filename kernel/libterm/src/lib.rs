...
//! * Managing the scrollback buffer, a string of characters that should be
//!   printed to the screen.
//! * Determining which parts of that buffer should be displayed and using the
...
use draw::{Coordinates, Drawable, Rectangle, Settings, Text};
...
pub struct Terminal {
    ...
    pub window: Window,
    /// The terminal's scrollback buffer which stores a string to be displayed
    /// by the text display
    scrollback_buffer: String,
    /// Indicates whether the text display is displaying the last part of the
    /// scrollback buffer slice
    is_scroll_end: bool,
    /// The starting index of the scrollback buffer string slice that is
    /// currently being displayed on the text display
...
impl Terminal {
    /// Gets the width and height of the text displayable in number of
    /// characters.
    ...
    /// This function takes in the end index of some index in the scrollback
    /// buffer and calculates the starting index of the scrollback buffer so
    /// that a slice containing the starting and ending index would perfectly
    /// fit inside the dimensions of text display.
    /// If the text display's first line will display a continuation of a
    /// syntactical line in the scrollback buffer, this function calculates
    /// the starting index so that when displayed on the text display, it
    /// preserves that line so that it looks the same as if the whole
    /// physical line is displayed on the buffer
    ///
    /// Return: starting index of the string and the cursor position(with
    /// respect to position on the screen, not in the scrollback buffer) in that
    /// order
    fn calc_start_idx(&self, end_idx: usize) -> (usize, usize) {
        let (buffer_width, buffer_height) = self.get_text_dimensions();
        let mut start_idx = end_idx;
        // Grabs a max-size slice of the scrollback buffer (usually does not totally fit
        // because of newlines)
        ...
        if let Some(slice) = result {
            ...
            // Obtains a vector of indices of newlines in the slice in the REVERSE order
            // that they occur
            ...
            if new_line_indices[0].0 != slice.len() - 1 {
                ...
                last_line_chars = (slice.len() - 1 - new_line_indices[0].0) % buffer_width;
            // fix: account for more than one line
            } else {
                start_idx -= 1;
                total_lines += 1;
            }

            // covers everything *up to* the characters between the beginning of the slice
            // and the first new line character
            for i in 0..new_line_indices.len() - 1 {
                ...
                let num_lines = if (num_chars - 1) % buffer_width != 0 || (num_chars - 1) == 0 {
                    ...
                    (num_chars - 1) / buffer_width
                }; // using (num_chars -1) because that's the number of characters that actually
                   // show up on the screen
                if num_chars > start_idx {
                    // prevents subtraction overflow
            ...
            // tracks the characters between the beginning of the slice and the first new
            // line character
            let first_chars = new_line_indices[new_line_indices.len() - 1].0;
            let first_chars_lines = first_chars / buffer_width + 1;

            // covers the case where the text inside the new_lines_indices array overflow
            // the text buffer
            if total_lines > buffer_height {
            ...
            // covers the case where the text between the last newline and the
            // end of the slice overflow the text buffer
            } else if first_chars_lines + total_lines > buffer_height {
                let diff = buffer_height - total_lines;
                total_lines += diff;
                start_idx -= diff * buffer_width;
            // covers the case where the text between the last newline and the
            // end of the slice exactly fits the text buffer
            } else if first_chars_lines + total_lines == buffer_height {
                total_lines += first_chars_lines;
                start_idx -= first_chars;
            // covers the case where the slice fits within the text buffer (i.e.
            // there is not enough output to fill the screen)
            } else {
                return (0, total_lines * buffer_width + last_line_chars); // In
                                                                          // the
                                                                          // case
                                                                          // that
                                                                          // an end
            ...
            // If the previous loop overcounted, this cuts off the excess string from
            // string. Happens when there are many charcters between newlines at the
            ...
            (0, 0) /* WARNING: should change to Option<> rather than
                    * returning (0, 0) */
        }
    }

    /// This function takes in the start index of some index in the scrollback
    /// buffer and calculates the end index of the scrollback buffer so that
    /// a slice containing the starting and ending index would perfectly fit
    /// inside the dimensions of text display.
    fn calc_end_idx(&self, start_idx: usize) -> Result<usize, ScrollError> {
        ...
        if let Some(slice) = result {
            let mut total_lines = 0;
            // Obtains a vector of the indices of the slice where newlines occur in
            // ascending order
            let new_line_indices: Vec<(usize, &str)> = slice.match_indices('\n').collect();
            // if there are no new lines in the slice
            if new_line_indices.is_empty() {
                // indicates that the text is just one continuous string with no newlines and
                // will therefore fill the buffer completely
            ...
            let mut counter = 0;
            // Covers the case where the start idx argument corresponds to a string that
            ...
            // covers the case where the text inside the new_line_indices array overflows
            // the text buffer capacity
            if total_lines > buffer_height {
                let num_chars = new_line_indices[counter].0 - new_line_indices[counter - 1].0;
                end_idx -= num_chars;
                end_idx += buffer_width;
            // covers the case where the characters between the last newline and
            // the end of the slice overflow the text buffer capacity
            } else if total_lines + num_last_lines >= total_lines {
                let diff = buffer_height - total_lines;
                end_idx += diff * buffer_width;
            // covers the case where the entire slice exactly fits or is smaller
            // than the text buffer capacity
            } else {
            ...
            Ok(self.scrollback_buffer.len() - 1) /* WARNING: maybe should
                                                  * return Error? */
        }
    }
    ...
    fn scroll_up_one_line(&mut self) {
        ...
        if buffer_width < start_idx {
            result = self
                .scrollback_buffer
                .as_str()
                .get(start_idx - buffer_width..start_idx);
        ...
        if let Some(slice) = result {
            ...
            new_start_idx = match index {
                Some(index) => start_idx - slice_len + index, /* Moves the starting index back
                                                                * to the position of the nearest
                                                                * newline back */
                None => start_idx - slice_len, /* If no newline is found, moves the start index
                                                * back by the buffer width value */
            }; // we're moving the cursor one position to the right relative to
    ...
    fn scroll_down_one_line(&mut self) {
        ...
        // If the newly calculated end index is the bottom of the scrollback buffer,
        // recalculates the start index and returns
        ...
        let new_end_idx;
        {
            ...
            let slice_len; // specifies the length of the grabbed slice
                           // Grabs a slice (the size of the buffer width at most) of the scrollback buffer
                           // that is directly below the current slice being displayed on the text display
            if self.scrollback_buffer.len() > end_idx + buffer_width {
                ...
                result = self
                    .scrollback_buffer
                    .as_str()
                    .get(end_idx..self.scrollback_buffer.len());
            }
            // Searches the grabbed slice for a newline
            if let Some(slice) = result {
                let index = slice.find('\n');
                new_end_idx = match index {
                    Some(index) => end_idx + index + 1, /* Moves end index forward to the next
                                                          * newline */
                    None => end_idx + slice_len, /* If no newline is found, moves the end index
                                                  * forward by the buffer width value */
                };
            } else {
                return;
            }
    ...
    /// Shifts the text display up by making the previous first line the last
    /// line displayed on the text display
    fn page_up(&mut self) {
    ...
    /// Shifts the text display down by making the previous last line the first
    /// line displayed on the text display
    fn page_down(&mut self) {
        ...
        if new_end_idx == self.scrollback_buffer.len() - 1 {
            // if the user page downs near the bottom of the page so only gets a partial
            // shift
            self.is_scroll_end = true;
            return;
        }
    ...
    /// Updates the text display by taking a string index and displaying as much
    /// as it starting from the passed string index (i.e. starts from the top of
    /// the display and goes down)
    ...
    /// Updates the text display by taking a string index and displaying as much
    /// as it can going backwards from the passed string index (i.e. starts from
    /// the bottom of the display and goes up)
...
impl Terminal {
    ...
    pub fn new() -> Result<Terminal, &'static str> {
        ...
        let window = compositor::window(window_width, window_height);
        ...
        terminal.print_to_terminal(
            "Theseus Terminal Emulator\nPress Ctrl+C to quit a task\n".to_string(),
        );
        Ok(terminal)
    }

    /// Adds a string to be printed to the terminal to the terminal scrollback
    /// buffer. Note that one needs to call `refresh_display` to get things
    /// actually printed.
    ...
    pub fn refresh_display(&mut self) -> Result<(), &'static str> {
        let start_idx = self.scroll_start_idx;
        // handling display refreshing errors here so that we don't clog the main loop
        // of the terminal
        if self.is_scroll_end {
    ...
    /// * `offset_from_end`: the position to insert the character. It represents
    ///   the distance relative to the end of the whole output in the terminal
    ///   in number of characters.
    ///
    /// # Examples
    ///
    /// * `terminal.insert_char(char, 0)` will append `char` to the end of
    ///   existing text.
    /// * `terminal.insert_char(char, 5)` will insert `char` right before the
    ///   last 5 characters.
    ///
    /// After invoke this function, one must call `refresh_display` to get the
    /// updates actually showed on the screen.
    pub fn insert_char(&mut self, c: char, offset_from_end: usize) -> Result<(), &'static str> {
        let buflen = self.scrollback_buffer.len();
        if buflen < offset_from_end {
            return Err("offset_from_end is larger than length of scrollback buffer");
        }
        let insert_idx = buflen - offset_from_end;
        self.scrollback_buffer
            .insert_str(insert_idx, &c.to_string());
        Ok(())
    }
    ...
    ///
    /// # Arguments
    /// * `offset_from_end`: the position of the character to remove. It
    ///   represents the distance relative to the end of the whole output in the
    ///   terminal in number of characters. `offset_from_end == 0` is *invalid*
    ///   here.
    ///
    /// # Examples
    /// * `terminal.remove_char(1)` will remove the last character in the
    ///   screen.
    ///
    /// After invoke this function, one must call `refresh_display` to get the
    /// updates actually showed on the screen.
    pub fn remove_char(&mut self, offset_from_end: usize) -> Result<(), &'static str> {
        ...
        if offset_from_end == 0 {
            return Err("cannot remove character at offset_from_end == 0");
        }
    ...
    // TODO: This function shouldn't exist. It should either block, or return a
    // future.
    ...
    pub fn display_cursor(&mut self) -> Result<(), &'static str> {
        ...
        let (next_col, next_row) = Text::new(&self.text, Coordinates::ORIGIN)
            .next_grid_position(&*self.window.as_framebuffer());
        let (num_col, num_row) = self.get_text_dimensions();

        // return if the cursor is not in the screen
        if next_col >= num_col || next_row >= num_row {
            return Ok(());
        }

        // Get the bounding box that contains the displayed cursor.
        let bounding_box = {
            let coord = self.window.area().vertex(Vertical::Top, Horizontal::Left);
            self.cursor.display(
                coord,
                next_col,
                next_row,
    ...
    /// Gets the position of the cursor relative to the end of text in number of
    /// characters.
    ...
    /// # Arguments
    /// * `offset_from_end`: the position of the cursor relative to the end of
    ///   text in number of characters.
    /// * `underlying_char`: the ASCII code of the underlying character when the
    ///   cursor is unseen.
    ...
    /// Resizes this terminal and its underlying text display and then refreshes
    /// the window. This does not automatically redisplay the terminal
    /// cursor.
...
fn display_text(window: &mut Window, text: &str) {
    ...
    let refresh_area = Text::new(text, Coordinates::ORIGIN)
        .draw(&mut *window.as_mut_framebuffer(), &settings);
    // TODO
    log::info!("{refresh_area:?}");
    let start = time::Instant::now();
    window.blocking_refresh(refresh_area);
    log::info!("refresh took: {:?}", time::Instant::now().duration_since(start));
}
