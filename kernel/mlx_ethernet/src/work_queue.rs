...
impl WorkQueue {
    ...
    fn init(pd: u32, db_addr: PhysicalAddress, wq_size: u32, wqe_size_in_bytes: u32) -> WorkQueue {
        ...
        wq.wq_type_signature
            .write(U32::new((WQType::Cyclic as u32) << WQ_TYPE_SHIFT));
        wq.pd.write(U32::new(pd & PD_MASK));
        wq.dbr_addr_h
            .write(U32::new((db_addr.value() >> 32) as u32));
        ...
        wq.log_wq_stride_pg_sz_sz.write(U32::new(
            (log_wq_stride << WQ_STRIDE_SHIFT)
                | (log_wq_page_size << WQ_PAGE_SIZE_SHIFT)
                | log_wq_size,
        ));
        wq
    }
    ...
    pub fn init_sq(pd: u32, uar_page: u32, db_addr: PhysicalAddress, wq_size: u32) -> WorkQueue {
        ...
        let mut wq = Self::init(
            pd,
            db_addr,
            wq_size,
            core::mem::size_of::<WorkQueueEntrySend>() as u32,
        );
    ...
    pub fn init_rq(pd: u32, db_addr: PhysicalAddress, wq_size: u32) -> WorkQueue {
        Self::init(
            ...
            db_addr,
            wq_size,
            core::mem::size_of::<WorkQueueEntryReceive>() as u32,
        )
    }
...
impl ControlSegment {
    ...
    fn init(&mut self, opcode: WQEOpcode, wqe_index: u32, sqn: u32) {
        ...
        self.opcode
            .write(U32::new((wqe_index << WQE_INDEX_SHIFT) | (opcode as u32)));
        self.ds
            .write(U32::new((sqn << SQN_SHIFT) | WQE_SIZE_IN_OCTWORDS));
        self.ce_se.write(U32::new(
            (CompletionAndEventMode::CQEAlways as u32) << CE_SHIFT,
        ));
    }
...
impl EthSegment {
    ...
    pub fn init(&mut self, packet: &[u8]) {
        ...
        self.inline_headers_0.write(U32::new(
            (INLINE_HEADER_SIZE << INLINE_HEADER_SHIFT)
                | (packet[0] as u32) << 8
                | (packet[1] as u32),
        ));
        self.inline_headers_1.write(U32::new(
            (packet[2] as u32) << 24
                | (packet[3] as u32) << 16
                | (packet[4] as u32) << 8
                | packet[5] as u32,
        ));
        self.inline_headers_2.write(U32::new(
            (packet[6] as u32) << 24
                | (packet[7] as u32) << 16
                | (packet[8] as u32) << 8
                | packet[9] as u32,
        ));
        self.inline_headers_3.write(U32::new(
            (packet[10] as u32) << 24
                | (packet[11] as u32) << 16
                | (packet[12] as u32) << 8
                | packet[13] as u32,
        ));
        self.inline_headers_4.write(U32::new(
            (packet[14] as u32) << 24 | (packet[15] as u32) << 16,
        ));
    }
}
...
impl MemoryPointerDataSegment {
    ...
    pub fn init(&mut self, lkey: u32, local_address: PhysicalAddress, len: u32) {
        ...
        self.local_address_h
            .write(U32::new((local_address.value() >> 32) as u32));
        self.local_address_l
            .write(U32::new((local_address.value() & 0xFFFF_FFFF) as u32));
    }
}
