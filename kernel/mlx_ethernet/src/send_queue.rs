...
use crate::{
    ...
    work_queue::{DoorbellRecord, WorkQueueEntrySend},
    Lkey, Sqn, Tisn, CQN_MASK,
};
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("SendQueueContext")
            ...
            .field(
                ...
                &self.packet_pacing_rate_limit_index.read().get(),
            )
...
impl SendQueueContext {
    ...
    pub fn init(cqn: u32, tisn: u32) -> SendQueueContext {
        ...
        ctxt.rlky_state.write(U32::new(
            ENABLE_RLKEY | FAST_REGISTER_ENABLE | FLUSH_IN_ERROR_ENABLE | ONE_INLINE_HEADER,
        ));
    ...
    pub fn set_state(&mut self, next_state: SendQueueState) {
        ...
        self.rlky_state
            .write(U32::new(state | ((next_state as u32) << STATE_SHIFT)));
    }
...
impl SendQueue {
    ...
    pub fn create(
        ...
        let mut entries = BoxRefMut::new(Box::new(entries_mp))
            .try_map_mut(|mp| mp.as_slice_mut::<WorkQueueEntrySend>(0, num_entries))?;
        for entry in entries.iter_mut() {
            entry.init()
        }
        // map the doorbell and initialize
        let mut doorbell = BoxRefMut::new(Box::new(doorbell_mp))
            .try_map_mut(|mp| mp.as_type_mut::<DoorbellRecord>(0))?;
        *doorbell = DoorbellRecord::default();
        // map the uar and initialize
        let mut uar = BoxRefMut::new(Box::new(uar_mp))
            .try_map_mut(|mp| mp.as_type_mut::<UserAccessRegion>(0))?;
        *uar = UserAccessRegion::default();
        ...
        Ok(SendQueue {
            ...
            _tisn,
            lkey,
            uar_db: CurrentUARDoorbell::Even,
        })
    }
    ...
    fn finish_wqe_operation(&mut self) {
        ...
        self.doorbell
            .send_counter
            .write(U32::new(self.wqe_counter as u32));
