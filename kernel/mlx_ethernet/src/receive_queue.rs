...
use memory::{create_contiguous_mapping, MappedPages};
...
use crate::{
    command_queue::CommandOpcode, completion_queue::CompletionQueue,
    work_queue::WorkQueueEntryReceive, Lkey, Rqn, CQN_MASK,
};
...
impl ReceiveQueueContext {
    ...
    pub fn init(cqn: u32) -> ReceiveQueueContext {
        ...
        ctxt.rlky_state
            .write(U32::new(ENABLE_RLKEY | VLAN_STRIP_DISABLE));
    ...
    pub fn get_state(&self) -> Result<ReceiveQueueState, &'static str> {
        ...
        Ok(ReceiveQueueState::try_from(state as u8)
            .map_err(|_e| "Invalid value in the RQ state")?)
    }
...
impl ReceiveQueue {
    ...
    pub fn create(
        ...
        let mut entries = BoxRefMut::new(Box::new(entries_mp))
            .try_map_mut(|mp| mp.as_slice_mut::<WorkQueueEntryReceive>(0, num_entries))?;
    ...
    pub fn refill(&mut self) -> Result<(), &'static str> {
        ...
        for wqe in self.entries.iter_mut() {
            ...
            let rx_buf = self
                ...
                .or_else(|_e| {
                    create_contiguous_mapping(buffer_size as usize, NIC_MAPPING_FLAGS).map(
