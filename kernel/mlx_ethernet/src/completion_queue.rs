...
use crate::{
    command_queue::CommandOpcode, log_page_size, work_queue::WQEOpcode, Cqn, HW_OWNERSHIP,
    LOG_PAGE_SIZE_SHIFT, LOG_QUEUE_SIZE_MASK, LOG_QUEUE_SIZE_SHIFT, UAR_MASK,
};
...
impl CompletionQueueContext {
    ...
    pub fn init(
        ...
        let size =
            (libm::log2(cq_size as f64) as u32 & LOG_QUEUE_SIZE_MASK) << LOG_QUEUE_SIZE_SHIFT;
        ctxt.uar_log_cq_size.write(U32::new(uar | size));

        ctxt.c_eqn.write(U32::new(c_eqn as u32));

        let log_page_size =
            log_page_size(cq_size * core::mem::size_of::<CompletionQueueEntry>() as u32);
        ctxt.log_page_size
            .write(U32::new(log_page_size << LOG_PAGE_SIZE_SHIFT));
...
impl CompletionQueue {
    ...
    pub fn init(
        ...
        let mut entries = BoxRefMut::new(Box::new(entries_mp))
            .try_map_mut(|mp| mp.as_slice_mut::<CompletionQueueEntry>(0, num_entries))?;
        let mut doorbell = BoxRefMut::new(Box::new(doorbell_mp))
            .try_map_mut(|mp| mp.as_type_mut::<CompletionQueueDoorbellRecord>(0))?;
