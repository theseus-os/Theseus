...
use crate::{
    command_queue::CommandOpcode, log_page_size, Cqn, Eqn, HW_OWNERSHIP, LOG_PAGE_SIZE_SHIFT,
    LOG_QUEUE_SIZE_MASK, LOG_QUEUE_SIZE_SHIFT, UAR_MASK,
};
use alloc::boxed::Box;
...
impl EventQueueContext {
    ...
    pub(crate) fn init(uar_page: u32, eq_size: u32) -> EventQueueContext {
        ...
        let size =
            (libm::log2(eq_size as f64) as u32 & LOG_QUEUE_SIZE_MASK) << LOG_QUEUE_SIZE_SHIFT;
        ctxt.uar_log_eq_size.write(U32::new(uar | size));

        let log_eq_page_size =
            log_page_size(eq_size * core::mem::size_of::<EventQueueEntry>() as u32);
        ctxt.log_pg_size
            .write(U32::new(log_eq_page_size << LOG_PAGE_SIZE_SHIFT));
        ctxt
    }
...
impl EventQueue {
    ...
    pub fn init(mp: MappedPages, num_entries: usize, eqn: Eqn) -> Result<EventQueue, &'static str> {
        let mut entries = BoxRefMut::new(Box::new(mp))
            .try_map_mut(|mp| mp.as_slice_mut::<EventQueueEntry>(0, num_entries))?;
