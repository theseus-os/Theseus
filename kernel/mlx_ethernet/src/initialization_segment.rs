...
use command_queue::{CmdState, Command};
use core::fmt;
...
impl InitializationSegment {
    ...
    pub fn set_physical_address_of_cmdq(
        &mut self,
        ...
        self.cmdq_phy_addr_high
            .write(U32::new((cmdq_physical_addr.value() >> 32) as u32));
        let val = self.cmdq_phy_addr_low.read().get() & 0xFFF;
        self.cmdq_phy_addr_low
            .write(U32::new(cmdq_physical_addr.value() as u32 | val));
        Ok(())
    }
    ...
    pub(crate) fn post_command(&mut self, command: &Command<{ CmdState::Initialized }>) {
        ...
        self.command_doorbell_vector
            .write(U32::new(val | (1 << command.entry_num)));
    }
}
...
impl fmt::Debug for InitializationSegment {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("InitializationSegment")
            ...
            .field(
                "Command Interface Rev",
                &self.cmd_interface_rev.read().get(),
            )
            .field(
                "Command queue address high",
                &self.cmdq_phy_addr_high.read().get(),
            )
            .field(
                "Command queue address low",
                &self.cmdq_phy_addr_low.read().get(),
            )
            .field(
