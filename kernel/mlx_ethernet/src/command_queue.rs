...
use crate::{
    ...
    flow_table::{
        DestinationEntry, DestinationType, FlowContext, FlowContextAction, FlowEntryInput,
        FlowGroupInput, FlowTableContext, FlowTableType, MatchCriteriaEnable,
    },
    ...
    Cqn, Eqn, FgId, FtId, Lkey, Pd, Rqn, Sqn, Td, Tirn, Tisn,
};
    ...
    fn num_input_mailboxes(&self, num_pages: Option<usize>) -> Result<usize, CommandQueueError> {
        let num = match self {
            ...
            Self::SetFlowTableEntry => {
                ...
                let size_of_mailbox_data =
                    0x30 + 0x300 + (NUM_DEST_ENTRIES * core::mem::size_of::<DestinationEntry>());
                Self::num_mailboxes(size_of_mailbox_data)
            }
            _ => return Err(CommandQueueError::NotImplemented),
        };
...
impl MailboxBuffer {
    fn write_to_mailbox<T: FromBytes>(
        &mut self,
        data: T,
        offset: usize,
...
impl CommandQueue {
    ...
    pub fn create(
        ...
        Ok(CommandQueue {
            entries,
            available_entries: available_entries.into_boxed_slice(),
            token,
            mailbox_buffers,
        })
    }
    ...
    fn create_command(
        &mut self,
        parameters: CommandBuilder,
    ) -> Result<Command<{ CmdState::Initialized }>, CommandQueueError> {
        let entry_num = self
            .find_free_command_entry()
            .ok_or(CommandQueueError::NoCommandEntryAvailable)?;
        let num_pages = parameters
            .allocated_pages
            .as_ref()
            .and_then(|pages| Some(pages.len()));
        ...
        let mut cmdq_entry =
            CommandQueueEntry::init(parameters.opcode, parameters.opmod, self.token, num_pages)?;
        let mut input_mailbox_buffers: Box<[MailboxBuffer]> =
            self.initialize_mailboxes(parameters.opcode.num_input_mailboxes(num_pages)?)?;
        let output_mailbox_buffers: Box<[MailboxBuffer]> =
            self.initialize_mailboxes(parameters.opcode.num_output_mailboxes()?)?;
        ...
        match parameters.opcode {
            ...
            CommandOpcode::ManagePages => {
                let pages_pa = parameters
                    .allocated_pages
                    .ok_or(CommandQueueError::MissingInputPages)?;
            ...
            CommandOpcode::ModifyNicVportContext => {
                ...
                let context = input_mailbox_buffers[0]
                    .mp
                    .as_type_mut::<NicVportContext>(NIC_VPORT_CONTEXT_OFFSET)
                    .map_err(|_e| CommandQueueError::InvalidMailboxOffset)?;

                context.mtu.write(U32::new(
                    parameters.mtu.ok_or(CommandQueueError::MissingInput)? as u32,
                ));
                context
                    .allowed_list_size
                    .write(U32::new((1 << 31) | (1 << 30) | (1 << 29))); // promisc_uc | promisc_mc | promisc_all
            }
            CommandOpcode::AccessRegister => {
                cmdq_entry.set_input_inline_data_0(NetworkPortRegisters::PMTU as u32);

                let register_data = input_mailbox_buffers[0]
                    .mp
                    .as_type_mut::<[U32<BigEndian>; 3]>(0)
                    .map_err(|_e| CommandQueueError::InvalidMailboxOffset)?;
                register_data[0] = U32::new(1 << 16);

                if parameters.opmod.ok_or(CommandQueueError::MissingInput)?
                    == AccessRegisterOpMod::Write as u16
                {
                    register_data[2] = U32::new(
                        (parameters.mtu.ok_or(CommandQueueError::MissingInput)? as u32) << 16,
                    );
                } else {
            ...
            CommandOpcode::CreateEq => {
                ...
                Self::write_event_queue_context_to_mailbox(
                    ...
                    parameters
                        .user_access_region
                        .ok_or(CommandQueueError::MissingInput)?,
                    parameters
                        .queue_size
                        .ok_or(CommandQueueError::MissingInput)?,
                )?;
            ...
            CommandOpcode::ModifySq => {
                let sq_state = 0 << 28;
                cmdq_entry.set_input_inline_data_0(
                    sq_state
                        | parameters
                            .send_queue_num
                            .ok_or(CommandQueueError::MissingInput)?
                ...
                Self::modify_sq_state(&mut input_mailbox_buffers[SQ_CONTEXT_MAILBOX_INDEX])?;
            }
            ...
            CommandOpcode::ModifyRq => {
                ...
                Self::modify_rq_state(&mut input_mailbox_buffers[RQ_CONTEXT_MAILBOX_INDEX])?;
            }
            ...
            CommandOpcode::SetFlowTableEntry => {
                Self::write_flow_entry_info_to_mailbox(
                    ...
                    parameters
                        ...
                        .0,
                    parameters
                        .transport_interface_receive_num
                        .ok_or(CommandQueueError::MissingInput)?
                        .0,
                )?;
            }
        ...
        let initialized_entry = Command::new(
            entry_num,
            cmdq_entry,
            input_mailbox_buffers,
            output_mailbox_buffers,
        ...
        #[cfg(mlx_verbose_log)]
        {
            ...
            for (i, mb) in initialized_entry.input_mailbox_buffers.iter().enumerate() {
                mb.mp
                    .as_type::<CommandInterfaceMailbox>(0)
                    .map_err(|_e| CommandQueueError::InvalidMailboxOffset)?
                    .dump_mailbox(i)
            }
        }
    ...
    fn initialize_mailboxes(
        &mut self,
        num_mailboxes: usize,
    ) -> Result<Box<[MailboxBuffer]>, CommandQueueError> {
        ...
        for block_num in 0..num_mailboxes {
            ...
            let mailbox = command_mailbox_buffers[block_num]
                .mp
                .as_type_mut::<CommandInterfaceMailbox>(DEFAULT_MAILBOX_OFFSET_IN_PAGE)
                .map_err(|_e| CommandQueueError::InvalidMailboxOffset)?;
    ...
    fn write_event_queue_context_to_mailbox(
        ...
        input_mailbox_buffers[0]
            .write_to_mailbox(eq_context, EventQueueContext::mailbox_offset())?;
    ...
    fn write_completion_queue_context_to_mailbox(
        ...
        let cq_context: CompletionQueueContext =
            CompletionQueueContext::init(uar, cq_size, c_eqn, doorbell_pa, collapsed);
        input_mailbox_buffers[0]
            .write_to_mailbox(cq_context, CompletionQueueContext::mailbox_offset())?;
    ...
    fn write_transport_interface_send_context_to_mailbox(
        input_mailbox_buffer: &mut MailboxBuffer,
        td: u32,
    ) -> Result<(), CommandQueueError> {
        // initialize the TIS context
        let tis_context = TransportInterfaceSendContext::init(td);
        input_mailbox_buffer
            .write_to_mailbox(tis_context, TransportInterfaceSendContext::mailbox_offset())?;
        Ok(())
    }
    ...
    fn write_send_queue_context_to_mailbox(
        ...
        input_mailbox_buffers[0]
            .write_to_mailbox(sq_context, SendQueueContext::mailbox_offset())?;
    ...
    fn write_receive_queue_context_to_mailbox(
        ...
        input_mailbox_buffers[0]
            .write_to_mailbox(rq_context, ReceiveQueueContext::mailbox_offset())?;
    ...
    fn write_flow_table_context_to_mailbox(
        input_mailbox_buffer: &mut MailboxBuffer,
        num_ft_entries: u32,
    ) -> Result<(), CommandQueueError> {
    ...
    fn write_flow_group_info_to_mailbox(
        input_mailbox_buffer: &mut MailboxBuffer,
        ft_id: u32,
    ) -> Result<(), CommandQueueError> {
        let flow_group_input =
            FlowGroupInput::init(FlowTableType::NicRx, ft_id, 0, 0, MatchCriteriaEnable::None);
        input_mailbox_buffer
            .write_to_mailbox(flow_group_input, FlowGroupInput::mailbox_offset())?;
    ...
    fn write_transport_interface_receive_context_to_mailbox(
        input_mailbox_buffer: &mut MailboxBuffer,
        rqn: u32,
        td: u32,
    ) -> Result<(), CommandQueueError> {
        ...
        input_mailbox_buffer.write_to_mailbox(
            tir_context,
            TransportInterfaceReceiveContext::mailbox_offset(),
        )?;
        Ok(())
    }
    ...
    fn write_flow_entry_info_to_mailbox(
        input_mailbox_buffers: &mut [MailboxBuffer],
        ft_id: u32,
        fg_id: u32,
        tirn: u32,
        ...
        input_mailbox_buffers[0]
            .write_to_mailbox(flow_entry_input, FlowEntryInput::mailbox_offset())?;
        ...
        let dest_list_mb: usize =
            libm::ceilf((0x30 + 0x300) as f32 / MAILBOX_DATA_SIZE_IN_BYTES as f32) as usize;
    ...
    fn write_flow_table_root_to_mailbox(
        input_mailbox_buffer: &mut MailboxBuffer,
        ft_id: u32,
    ) -> Result<(), CommandQueueError> {
        let ft_root: [U32<BigEndian>; 2] = [
            U32::new((FlowTableType::NicRx as u32) << 24),
            U32::new(ft_id & 0xFF_FFFF),
        ];
    ...
    ) -> Result<CommandCompletionStatus, CommandQueueError> {
        #[cfg(mlx_verbose_log)]
        {
    ...
    fn check_command_output_validity(
        ...
        if self.entries[entry_num].get_command_opcode()? != cmd_opcode {
            error!(
                "Incorrect Command!: {:?}",
                self.entries[entry_num].get_command_opcode()?
            );
    ...
    pub fn get_port_type(
        ...
        let port_type = (mailbox
            .as_type::<U32<BigEndian>>(DATA_OFFSET_IN_MAILBOX)
            .map_err(|_e| CommandQueueError::InvalidMailboxOffset)?)
        .get();
    ...
    pub fn get_device_capabilities(
        ...
        let mailbox = &command.output_mailbox_buffers[0].mp;
        let capabilities = mailbox
            .as_type::<HCACapabilitiesLayout>(DATA_OFFSET_IN_MAILBOX)
            .map_err(|_e| CommandQueueError::InvalidMailboxOffset)?;
    ...
    pub fn get_max_mtu(
        ...
        let mailbox = &command.output_mailbox_buffers[0].mp;
        let mtu_data = mailbox
            .as_type::<[U32<BigEndian>; 2]>(DATA_OFFSET_IN_MAILBOX)
            .map_err(|_e| CommandQueueError::InvalidMailboxOffset)?;
    ...
    pub fn get_vport_mac_address(
        ...
        const DATA_OFFSET_IN_MAILBOX: usize = 0x0;
        let mailbox = &command.output_mailbox_buffers[0].mp;
        let nic_vport_context = mailbox
            .as_type::<NicVportContext>(DATA_OFFSET_IN_MAILBOX)
            .map_err(|_e| CommandQueueError::InvalidMailboxOffset)?;
    ...
    pub fn get_eq_number(
        &mut self,
        command: Command<{ CmdState::Completed }>,
    ) -> Result<(Eqn, CommandCompletionStatus), CommandQueueError> {
    ...
    pub fn get_cq_number(
        &mut self,
        command: Command<{ CmdState::Completed }>,
    ) -> Result<(Cqn, CommandCompletionStatus), CommandQueueError> {
        ...
        Ok((
            Cqn(cq_number & 0xFF_FFFF),
            self.get_command_status(command)?,
        ))
    }

    pub fn get_tis_context_number(
        &mut self,
        command: Command<{ CmdState::Completed }>,
    ) -> Result<(Tisn, CommandCompletionStatus), CommandQueueError> {
    ...
    pub fn get_send_queue_number(
        &mut self,
        command: Command<{ CmdState::Completed }>,
    ) -> Result<(Sqn, CommandCompletionStatus), CommandQueueError> {
    ...
    pub fn get_receive_queue_number(
        &mut self,
        command: Command<{ CmdState::Completed }>,
    ) -> Result<(Rqn, CommandCompletionStatus), CommandQueueError> {
    ...
    pub fn get_sq_state(
        &mut self,
        command: Command<{ CmdState::Completed }>,
    ) -> Result<(SendQueueState, CommandCompletionStatus), CommandQueueError> {
        self.check_command_output_validity(command.entry_num, CommandOpcode::QuerySq)?;

        let mailbox = &command.output_mailbox_buffers[0];
        let sq_context = mailbox
            .mp
            .as_type::<SendQueueContext>(0x10)
            .map_err(|_e| CommandQueueError::InvalidMailboxOffset)?;
        let state = sq_context
            .get_state()
            .map_err(|_e| CommandQueueError::InvalidSQState)?;
        Ok((state, self.get_command_status(command)?))
    }

    pub fn get_flow_table_id(
        &mut self,
        command: Command<{ CmdState::Completed }>,
    ) -> Result<(FtId, CommandCompletionStatus), CommandQueueError> {
    ...
    pub fn get_flow_group_id(
        &mut self,
        command: Command<{ CmdState::Completed }>,
    ) -> Result<(FgId, CommandCompletionStatus), CommandQueueError> {
    ...
    pub fn get_tir_context_number(
        &mut self,
        command: Command<{ CmdState::Completed }>,
    ) -> Result<(Tirn, CommandCompletionStatus), CommandQueueError> {
...
impl fmt::Debug for CommandQueueEntry {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("CommandQueueEntry")
            ...
            .field(
                "command_input_inline_data_0",
                &self.command_input_inline_data_0.read().get(),
            )
            .field(
                "command_input_inline_data_1",
...
impl CommandQueueEntry {
    ...
    fn init(
        ...
        cmdq_entry
            .type_of_transport
            .write(U32::new(CommandTransportType::PCIe as u32));
        ...
        cmdq_entry
            .token_signature_status_own
            .write(U32::new(val | ((token as u32) << 24)));
        ...
        cmdq_entry
            .input_length
            .write(U32::new(opcode.input_bytes(num_pages)?));
        ...
        cmdq_entry
            .output_length
            .write(U32::new(opcode.output_bytes()?));

        cmdq_entry
            .command_input_opcode
            .write(U32::new((opcode as u32) << 16));
        cmdq_entry
            .command_input_opmod
            .write(U32::new(opmod.unwrap_or(0) as u32));
    ...
    fn set_input_mailbox_pointer(&mut self, mailbox_ptr: PhysicalAddress) {
        self.input_mailbox_pointer_h
            .write(U32::new((mailbox_ptr.value() >> 32) as u32));
        self.input_mailbox_pointer_l
            .write(U32::new((mailbox_ptr.value() & 0xFFFF_FFFF) as u32));
    }

    /// Sets the mailbox pointer in a command entry with the physical address of the first mailbox.
    fn set_output_mailbox_pointer(&mut self, mailbox_ptr: PhysicalAddress) {
        self.output_mailbox_pointer_h
            .write(U32::new((mailbox_ptr.value() >> 32) as u32));
        self.output_mailbox_pointer_l
            .write(U32::new((mailbox_ptr.value() & 0xFFFF_FFFF) as u32));
    }
    ...
    pub fn get_return_status(&self) -> Result<CommandReturnStatus, CommandQueueError> {
        ...
        CommandReturnStatus::try_from(status)
            .map_err(|_e| CommandQueueError::InvalidCommandReturnStatus)
    }
...
const_assert_eq!(
    core::mem::size_of::<CommandInterfaceMailbox>(),
    MAILBOX_SIZE_IN_BYTES
);
...
impl CommandInterfaceMailbox {
    ...
    fn init(&mut self, block_num: u32, token: u8, next_mb_addr: usize) {
        ...
        self.token_ctrl_signature
            .write(U32::new((token as u32) << 16));
        self.next_pointer_h
            .write(U32::new((next_mb_addr >> 32) as u32));
        self.next_pointer_l
            .write(U32::new((next_mb_addr & 0xFFFF_FFFF) as u32));
    }
    ...
    fn dump_mailbox(&self, block_num: usize) {
        ...
        unsafe {
            ...
            for i in 0..MAILBOX_SIZE_IN_BYTES / 16 {
                let x = (i * 4) as isize;
                debug!(
                    "{:#05x}: {:#010x} {:#010x} {:#010x} {:#010x}",
                    i * 16 + 0x40 + (block_num * MAILBOX_SIZE_IN_BYTES),
                    (*ptr.offset(x)).to_be(),
                    (*ptr.offset(x + 1)).to_be(),
                    (*ptr.offset(x + 2)).to_be(),
                    (*ptr.offset(x + 3)).to_be()
                );
            }
            debug!("")
        }
...
impl HCACapabilitiesLayout {
    fn get_capabilities(&self) -> HCACapabilities {
        HCACapabilities {
            ...
            log_max_current_mc_list: ((self.log_max_current_uc_list.read().get() >> 8) & 0x1F)
                as u8,
