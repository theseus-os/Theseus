...
pub struct HttpResponse {
    /// The actual array of raw bytes received from the server, including all of
...
impl HttpResponse {
    ...
    /// Returns the content of this `HttpResponse` as a `Result`, in which
    /// `Ok(content)` is returned if the status code is 200 (Ok), and
    /// `Err((status_code, reason))` is returned otherwise.
