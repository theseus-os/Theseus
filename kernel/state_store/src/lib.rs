...
pub fn init() {
    SYSTEM_STATE.call_once(|| SystemStateList::new());
}
...
pub fn insert_state<S: Any>(state: S) -> Option<S> {
    let old_val: Option<Box<dyn Any>> =
        SYSTEM_STATE.get().expect("SYSTEM_STATE uninited").0.insert(
            TypeId::of::<S>(),
...
impl<S: Any> SSCached<S> {
    ...
    pub fn get(&self) -> Option<Arc<S>> {
        ...
        let old_ptr = self
            .0
            .swap(Box::into_raw(Box::new(new_cached_val)), Ordering::Release);
...
pub fn get_state<S: Any>() -> SSCached<S> {
    SSCached(AtomicPtr::new(Box::into_raw(Box::new(
        get_state_internal::<S>(),
    ))))
}
