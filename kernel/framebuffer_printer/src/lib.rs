...
pub fn print_string<P: Pixel>(
    ...
    for byte in slice.bytes() {
        if byte == b'\n' {
            ...
            fill_blank(framebuffer, &mut blank, bg_pixel);
            curr_column = 0;
            curr_line += 1;
    ...
    blank = Rectangle {
        top_left: Coord::new(x, y + ((curr_line + 1) * CHARACTER_HEIGHT) as isize),
        bottom_right: Coord::new(x + width as isize, y + height as isize),
    };
    fill_blank(framebuffer, &mut blank, bg_pixel);
...
pub fn fill_blank<P: Pixel>(framebuffer: &mut Framebuffer<P>, blank: &mut Rectangle, pixel: P) {
