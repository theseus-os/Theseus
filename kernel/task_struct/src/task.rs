...
use crate::{
    ExitValue, FailureCleanupFunction, InheritedStates, KillHandler, RestartInfo, RunState,
    TASK_LIST,
};
...
pub struct Task {
    ...
    pub(crate) inner: IrqSafeMutex<TaskInner>,
    ...
    pub(crate) running_on_cpu: AtomicCell<OptionalCpuId>,
    /// The runnability of this task, i.e., whether it's eligible to be scheduled in.
    ///
    /// This is not public because it permits interior mutability.
    pub(crate) runstate: AtomicCell<RunState>,
    ...
    pub(crate) failure_cleanup_function: FailureCleanupFunction,
    /// A mailbox for exchanging a task's exit value with another task waiting to join on it.
    pub(crate) exit_value_mailbox: Mutex<Option<ExitValue>>,
