...
impl Line {
    ...
    fn replace_unit(
        ...
        if let Some(unit_to_replace) = self.units.get_mut(index.0) {
            ...
            (new_unit_index, new_width as i32)
        }
    }
...
impl<'term, Backend: TerminalBackend> Perform for TerminalActionHandler<'term, Backend> {
    ...
    fn csi_dispatch(&mut self, params: &vte::Params, _intermediates: &[u8], _ignore: bool, action: char) {
        ...
        match (action, first_param_value) {
            ...
            ('~', END_KEY_PARAM) => {
                // End key was pressed, move to the end of the current row.
                self.move_right(screen_size.num_columns.0 as usize, Wrap::No);
            }
            ('A', num_rows) => {
                // Down arrow was pressed, move the cursor up.
                // TODO: self.move_up(Row(1));
                trace!("Cursor up by {} is unimplemented", num_rows);
            }
            ('B', num_rows) => {
                // Down arrow was pressed, move the cursor down.
                // TODO: self.move_down(Row(1));
                trace!("Cursor down by {} is unimplemented", num_rows);
            }
            ('C', num_units) => {
...
fn increment_both_cursors(
    ...
    while num_units > 0 {
        ...
        scrollback_position.unit_idx.0 = min(new_unit_idx, scrollback_buffer[scrollback_position.line_idx].len() - 1);
        ...
        if num_units > 0 {
            if scrollback_position.line_idx.0 >= scrollback_buffer.len() - 1 {
...
impl ScreenPoint {
    ...
    /// this returns a `ScrollbackBufferPoint` based on the assumption that sufficient 
    /// empty lines (that occupy one row each) and empty units (that occupy one column each) 
    /// would be inserted into the scrollback buffer.
    fn to_scrollback_point(
        &self,
        origin_point: ScrollbackBufferPoint, 
        scrollback_buffer: &ScrollbackBuffer,
        screen_width: Column,
        ...
        for line in scrollback_buffer.get(line_idx.0 ..) {
            ...
            if row >= target_row {
                ...
                unit_idx = UnitIndex(line.len().saturating_sub(row_overshoot * screen_width));
                break;
            }
        ...
        if row < target_row {
            ...
            ScrollbackBufferPoint {
                line_idx,
                unit_idx: UnitIndex(min(target_column, unit_idx.0)),
            }
        }
    }
}
        ...
        while num_units > 0 {
            ...
            if num_units > unit_idx.0 {
                ...
                if scrollback_cursor.line_idx > LineIndex(0) {
                    ...
                    scrollback_cursor.unit_idx.0 = scrollback_buffer[scrollback_cursor.line_idx].len() - 1;
                } else {
                    // we're at the first line, so there's nowhere to wrap backwards to.
                    break;
                }
