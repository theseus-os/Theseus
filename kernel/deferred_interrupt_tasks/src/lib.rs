    ...
    S: Into<String>,
{
    ...
    tb.spawn()
        .map_err(|e| InterruptRegistrationError::SpawnError(e))
}
...
where
    DIA: Fn(&Arg) -> Result<Success, Failure>,
    Arg: Send + 'static,
{
    let curr_task =
        get_my_current_task().expect("BUG: deferred_task_entry_point: couldn't get current task.");
    // trace!("Entered {:?}:\n\t action: {:?}\n\t arg:    {:?}",
    ...
    loop {
        ...
        match _res {
            ...
            Err(failure) => error!(
                "Deferred interrupt action returned failure: {:?}",
                debugit!(failure)
            ),
        }
