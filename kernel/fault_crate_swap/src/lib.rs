...
use crate_swap::{swap_crates, SwapRequest};
use fault_log::{log_handled_fault, remove_unhandled_exceptions, FaultEntry, RecoveryAction};
...
pub fn do_self_swap(
    ...
    let kernel_mmi_ref =
        memory::get_kernel_mmi_ref().ok_or_else(|| "couldn't get kernel_mmi_ref".to_string())?;
    ...
    let swap_requests = {
        ...
        let (into_new_crate_file, new_namespace) = {
            if let Some(f) = override_namespace_crate_dir
                .as_ref()
                .and_then(|ns_dir| ns_dir.get_file_starting_with(crate_name))
            {
                (IntoCrateObjectFile::File(f), None)
            } else if let Some(FileOrDir::File(f)) =
                Path::new(String::from(crate_name)).get(curr_dir)
            {
                (IntoCrateObjectFile::File(f), None)
            } else {
...

    {
        ...
        return_struct.old_text = old_crate
            .text_pages
            .as_ref()
            .map(|(_mp, addr_range)| addr_range.clone());
        return_struct.old_rodata = old_crate
            .rodata_pages
            .as_ref()
            .map(|(_mp, addr_range)| addr_range.clone());
        return_struct.old_data = old_crate
            .data_pages
            .as_ref()
            .map(|(_mp, addr_range)| addr_range.clone());
    }
    ...
    let new_crate_ref =
        match CrateNamespace::get_crate_and_namespace(&real_new_namespace, &new_crate_full_name) {
            Some((ocr, _ns)) => ocr,
            _ => {
    ...
    // Find the address range of the newly loaded crate
    {
        ...
        return_struct.new_text = new_crate
            .text_pages
            .as_ref()
            .map(|(_mp, addr_range)| addr_range.clone());
        return_struct.new_rodata = new_crate
            .rodata_pages
            .as_ref()
            .map(|(_mp, addr_range)| addr_range.clone());
        return_struct.new_data = new_crate
            .data_pages
            .as_ref()
            .map(|(_mp, addr_range)| addr_range.clone());
        ...
        #[cfg(not(downtime_eval))]
        {
            if let (Some(old_text), Some(new_text)) = (
                return_struct.old_text.clone(),
                return_struct.new_text.clone(),
            ) {
            ...
            if let (Some(old_data), Some(new_data)) = (
                return_struct.old_data.clone(),
                return_struct.new_data.clone(),
            ) {
                debug!("Data Range was {:X} - {:X}", old_data.start, old_data.end);
                debug!("Data Range is {:X} - {:X}", new_data.start, new_data.end);
            }

            if let (Some(old_rodata), Some(new_rodata)) = (
                return_struct.old_rodata.clone(),
                return_struct.new_rodata.clone(),
            ) {
                debug!(
                    "Rodata Range was {:X} - {:X}",
                    old_rodata.start, old_rodata.end
                );
                debug!(
                    "Rodata Range is {:X} - {:X}",
                    new_rodata.start, new_rodata.end
                );
            }
        }
    }
...
pub fn constant_offset_fix(
    ...
    loop {
        ...
        if let (Some(old_text), Some(new_text)) =
            (swap_ranges.old_text.clone(), swap_ranges.new_text.clone())
        {
        ...
        if let (Some(old_rodata), Some(new_rodata)) = (
            swap_ranges.old_rodata.clone(),
            swap_ranges.new_rodata.clone(),
        ) {
        ...
        if let (Some(old_data), Some(new_data)) =
            (swap_ranges.old_data.clone(), swap_ranges.new_data.clone())
        {
...
pub fn self_swap_handler(crate_name: &str) -> Result<SwapRanges, String> {
    let taskref =
        task::get_my_current_task().ok_or_else(|| format!("failed to get current task"))?;
    ...
    for (_id, taskref) in task::TASKLIST.lock().iter() {
        let (bottom, top) =
            taskref.with_kstack(|kstack| (kstack.bottom().value(), kstack.top_usable().value()));
                ...
                match fe_last_fault {
                    ...
                    Some(fault_entry) => {
                        if fault_entry.action_taken == RecoveryAction::FaultCrateReplaced {
                            ...
                            || fault_entry.action_taken == RecoveryAction::MultipleFaultRecovery
                        {
