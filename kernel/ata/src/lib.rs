...
impl AtaBus {
    ...
    fn read_pio(
        ...
        self.wait_for_data_done()
            .map_err(|_| "error before issuing read pio command")?;
        ...
        for _lba in lba_start..(lba_start + sector_count) {
            ...
            self.wait_for_data_ready()
                .map_err(|_| "error during data read")?;

            for chunk in
                buffer[buffer_offset..(buffer_offset + SECTOR_SIZE_IN_BYTES)].chunks_exact_mut(2)
            {
        ...
        self.wait_for_data_done()
            .map_err(|_| "error after data read")?;
        Ok(sector_count)
    }
    ...
    fn write_pio(
        ...
        self.wait_for_data_done()
            .map_err(|_| "error before issuing write command")?;
        ...
        for _lba in lba_start..(lba_start + sector_count) {
            // Before transferring each sector, we have to wait for the drive to be ready for data
            self.wait_for_data_ready()
                .map_err(|_| "error during data write")?;

            for chunk in
                buffer[buffer_offset..(buffer_offset + SECTOR_SIZE_IN_BYTES)].chunks_exact(2)
            {
        ...
        self.wait_for_data_done()
            .map_err(|_| "error after data write")?;

        // Flush the drive's cache after each write command
        let cache_flush_cmd = if using_lba_28 {
        ...
        self.wait_for_data_done()
            .map_err(|_| "error after cache flush after data write")?;
        Ok(sector_count)
    }
    ...
    fn identify_drive(&mut self, which: BusDriveSelect) -> Result<AtaIdentifyData, &'static str> {
        self.wait_for_data_done()
            .map_err(|_| "error before issuing identify command")?;
        ...
        self.wait_for_data_ready()
            .map_err(|_| "error before identify data read")?;
        ...
        self.wait_for_data_done()
            .map_err(|_| "error after identify data read")?;
    ...
    fn software_reset(&mut self) {
        ...
        unsafe {
            self.control.write(AtaControl::SRST.bits());
        }
...
impl AtaDrive {
    ...
    pub fn read_pio(
        ...
        self.bus
            .lock()
            .read_pio(buffer, self.master_slave, lba_start, sector_count)
    }
    ...
    pub fn write_pio(
        ...
        self.bus
            .lock()
            .write_pio(buffer, self.master_slave, lba_start, sector_count)
    }
...
impl IdeController {
    ...
    pub fn new(pci_device: &PciDevice) -> Result<IdeController, &'static str> {
        let primary_bus_data_port = match pci_device.bars[0] {
            ...
            other => {
                warn!(
                    "Untested rare condition: ATA drive PCI BAR0 was special address value: {:#X}",
                    other
                );
                other as u16
            }
        ...
        let primary_bus_control_port = match pci_device.bars[1] {
            ...
            other => {
                warn!(
                    "Untested rare condition: ATA drive PCI BAR1 was special address value: {:#X}",
                    other
                );
                other as u16
            }
        ...
        let secondary_bus_data_port = match pci_device.bars[2] {
            ...
            other => {
                warn!(
                    "Untested rare condition: ATA drive PCI BAR2 was special address value: {:#X}",
                    other
                );
                other as u16
            }
        ...
        let secondary_bus_control_port = match pci_device.bars[3] {
            ...
            other => {
                warn!(
                    "Untested rare condition: ATA drive PCI BAR3 was special address value: {:#X}",
                    other
                );
                other as u16
            }
        ...
        let primary_bus = Arc::new(Mutex::new(AtaBus::new(
            primary_bus_data_port,
            primary_bus_control_port,
        )));
        let secondary_bus = Arc::new(Mutex::new(AtaBus::new(
            secondary_bus_data_port,
            secondary_bus_control_port,
        )));
...
impl StorageController for IdeController {
    fn devices<'c>(&'c self) -> Box<(dyn Iterator<Item = StorageDeviceRef> + 'c)> {
        Box::new(
            self.iter()
                .map(|ata_drive_ref| Arc::clone(ata_drive_ref) as StorageDeviceRef),
        )
    }
}
