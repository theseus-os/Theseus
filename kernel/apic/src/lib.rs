...
pub fn init(_page_table: &mut PageTable) -> Result<(), &'static str> {
    let x2 = has_x2apic();
    debug!(
        "is x2apic? {}.  IA32_APIC_BASE (phys addr): {:X?}",
        x2,
        PhysicalAddress::new(rdmsr(IA32_APIC_BASE) as usize)
    );
    ...
    if !x2 {
        ...
        unsafe {
            wrmsr(
                IA32_APIC_BASE,
                rdmsr(IA32_APIC_BASE) | IA32_APIC_XAPIC_ENABLE,
            );
        }
    }
...
fn map_apic(page_table: &mut PageTable) -> Result<MappedPages, &'static str> {
    if has_x2apic() {
        return Err("map_apic() is only for use in apic/xapic systems, not x2apic.");
    }
...
impl fmt::Debug for LocalApic {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "LocalApic {{processor: {}, apic_id: {}, is_bsp: {}}}",
...
impl LocalApic {
    ...
    pub fn new(
        ...
        if has_x2apic() {
            ...
            let apic_regs = BoxRefMut::new(Box::new(map_apic(page_table)?))
    ...
    fn enable_apic(&mut self) -> Result<(), &'static str> {
        assert!(
            !has_x2apic(),
            "an x2apic system must not use enable_apic(), it should use enable_x2apic() instead."
        );
        ...
        if let Some(ref mut regs) = self.regs {
            ...
            regs.spurious_interrupt_vector
                .write(APIC_SPURIOUS_INTERRUPT_VECTOR | APIC_SW_ENABLE);
            Ok(())
        } else {
    ...
    fn enable_x2apic(&mut self) {
        ...
        unsafe {
            wrmsr(
                IA32_APIC_BASE,
                rdmsr(IA32_APIC_BASE) | IA32_APIC_XAPIC_ENABLE | IA32_APIC_X2APIC_ENABLE,
            );
        }
        info!(
            "LAPIC x2 ID {:#x}, version: {:#x}, is_bsp: {}",
            self.id(),
            self.version(),
            is_bsp
        );
        ...
        info!(
            "x2LAPIC ID {:#x}, version {:#X}, (cluster {:#X} logical {:#X}), is_bsp: {}",
            self.id(),
            self.version(),
            cluster_id,
            logical_id,
        ...
        unsafe {
            ...
            wrmsr(
                IA32_X2APIC_SIVR,
                (APIC_SPURIOUS_INTERRUPT_VECTOR | APIC_SW_ENABLE) as u64,
            );
        }
    }
    ...
    pub fn id(&self) -> u8 {
        ...
        assert!(
            id == self.apic_id,
            "LocalApic::id() {} wasn't the same as given apic_id {}!",
            id,
            self.apic_id
        );
        id
    ...
    pub fn set_icr(&mut self, value: u64) {
        if has_x2apic() {
            ...
            self.regs
                ...
                .expect("ApicRegisters")
                .interrupt_command_high
                .write(high); // sets part of ICR register, but doesn't yet issue the IPI
            let low = value as u32;
            self.regs
                .as_mut()
                .expect("ApicRegisters")
                .interrupt_command_low
                .write(low); // this actually issues the IPI
            while self
                .regs
                .as_ref()
                .expect("ApicRegisters")
    ...
    pub fn set_ldr(&mut self, value: u32) {
        assert!(
            !has_x2apic(),
            "set_ldr(): Setting LDR MSR for x2apic is forbidden! (causes GPF)"
        );
        let old_ldr = self
            .regs
            .as_ref()
        ...
        self.regs
            ...
            .expect("ApicRegisters")
            .destination_format
            .write(old_ldr & 0x00FF_FFFF | value);
    }
        ...
        if has_x2apic() {
            match lint {
                0 => unsafe {
                    wrmsr(
                        ...
                        (flags << 12) as u64 | APIC_NMI as u64,
                    )
                }, // or APIC_NMI | 0x2000 ??
                1 => unsafe {
                    wrmsr(
            ...
            match lint {
                0 => self
                    ...
                    .expect("ApicRegisters")
                    .lvt_lint0
                    .write((flags << 12) as u32 | APIC_NMI), // or APIC_NMI | 0x2000 ??
                1 => self
                    .regs
                    .as_mut()
                    .expect("ApicRegisters")
                    .lvt_lint1
                    .write((flags << 12) as u32 | APIC_NMI),
                _ => return Err("set_nmi(): invalid lint {}, must be 0 or 1!"),
            }
        }
