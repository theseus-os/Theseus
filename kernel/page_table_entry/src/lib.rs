...
impl PageTableEntry {
    ...
    pub fn set_unmapped(&mut self) -> UnmapResult {
        ...
        if flags.is_exclusive() {
            UnmapResult::Exclusive(UnmappedFramesInfo(frame_range))
        } else {
...
///
/// If exclusive, the contained `UnmappedFramesInfo` can be used to deallocate frames.
///
...
///
/// `UnmappedFramesInfo` can be used to create an `UnmappedFrames`
/// and then safely deallocated within the `frame_allocator`.
///
/// See the `PageTableEntry::set_unmapped()` function.
pub struct UnmappedFramesInfo(FrameRange);
impl Deref for UnmappedFramesInfo {
