...
use serial_port::{take_serial_port_basic, SerialPortAddress};
...
pub fn init(key_producer: Queue<Event>, mouse_producer: Queue<Event>) -> Result<(), &'static str> {
    ...
    let init_serial_port = |spa: SerialPortAddress| {
        if let Some(sp) = take_serial_port_basic(spa) {
            ...
            info!(
                "Ignoring input on {:?} because it is being used for logging.",
                spa
            );
        }
    };
    ...
    for dev in pci::pci_device_iter() {
        ...
        match storage_manager::init_device(dev) {
            ...
            Err(e) => {
                error!(
                    "Failed to initialize storage device, it will be unavailable.\n{:?}\nError: {}",
                    dev, e
                );
                continue;
            }
        ...
        if dev.class == 0x02 && dev.subclass == 0x00 {
            if dev.vendor_id == e1000::INTEL_VEND && dev.device_id == e1000::E1000_DEV {
                ...
                let e1000_interface = EthernetNetworkInterface::new_ipv4_interface(
                    e1000_nic_ref,
                    DEFAULT_LOCAL_IP,
                    &DEFAULT_GATEWAY_IP,
                )?;
    ...
    if false {
        for storage_device in storage_manager::storage_devices() {
            let disk = FatFsAdapter(ReaderWriter::new(ByteReaderWriterWrapper::from(
            ...
            if let Ok(filesystem) = fatfs::FileSystem::new(disk, fatfs::FsOptions::new()) {
                ...
                for f in root.iter() {
                    debug!(
                        "\t {:X?}",
                        f.map(|entry| (entry.file_name(), entry.attributes(), entry.len()))
                    );
                }
            }
        }
...
impl<IO> FatFsAdapter<IO> {
    pub fn new(io: IO) -> FatFsAdapter<IO> {
        FatFsAdapter(io)
    }
}
...
impl<IO> fatfs::Read for FatFsAdapter<IO>
where
    IO: core2::io::Read,
{
...
impl<IO> fatfs::Write for FatFsAdapter<IO>
where
    IO: core2::io::Write,
{
...
impl<IO> fatfs::Seek for FatFsAdapter<IO>
where
    IO: core2::io::Seek,
{
