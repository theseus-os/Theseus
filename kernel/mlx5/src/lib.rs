...
impl ConnectX5Nic {
    ...
    pub fn init(mlx5_pci_dev: &PciDevice) -> Result</*&'static MutexIrqSafe<ConnectX5Nic>*/ (), &'static str> {
        ...
        assert_eq!(sq_size_in_bytes % PAGE_SIZE, 0);
        
        let rq_size_in_bytes = rq_size * 64;

        assert_eq!(rq_size_in_bytes % PAGE_SIZE, 0);
        ...
        let (rq_mp, sq_mp) = q_mp.split(memory_structs::Page::containing_address(vaddr + sq_size_in_bytes))
            .map_err(|_e| "Could not split mp")?;
        
        let sq_pa = q_pa + sq_size_in_bytes;
        
        debug!("rq_pa: {:#X}, sq_pa: {:#X}", q_pa, sq_pa);
        debug!("rq_pa: {:?}, sq_pa: {:?}", rq_mp.start_address(), sq_mp.start_address());
        ...
        send_queue.send(sq_number, tisn, rlkey, pa, buffer)?;
