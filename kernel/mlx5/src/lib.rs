...
use mlx_ethernet::{
    command_queue::{
        AccessRegisterOpMod, CommandBuilder, CommandOpcode, CommandQueue, CommandQueueEntry,
        HCACapabilities, ManagePagesOpMod, QueryHcaCapCurrentOpMod, QueryHcaCapMaxOpMod,
        QueryPagesOpMod,
    },
    completion_queue::{CompletionQueue, CompletionQueueDoorbellRecord, CompletionQueueEntry},
    event_queue::{EventQueue, EventQueueEntry},
    initialization_segment::InitializationSegment,
    receive_queue::ReceiveQueue,
    send_queue::SendQueue,
    work_queue::{DoorbellRecord, WorkQueueEntryReceive, WorkQueueEntrySend},
};
use nic_buffers::{ReceiveBuffer, TransmitBuffer};
use nic_initialization::{allocate_memory, init_rx_buf_pool, NIC_MAPPING_FLAGS};
...
impl ConnectX5Nic {
    ...
    pub fn init(
        ...
        let (cmdq_mapped_pages, cmdq_starting_phys_addr) =
            create_contiguous_mapping(size_in_bytes_of_cmdq, NIC_MAPPING_FLAGS)?;
        ...
        let mut cmdq = CommandQueue::create(
            BoxRefMut::new(Box::new(cmdq_mapped_pages))
                .try_map_mut(|mp| mp.as_slice_mut::<CommandQueueEntry>(0, num_cmdq_entries))?,
            num_cmdq_entries,
        )?;
        ...
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::EnableHca),
            &mut init_segment,
        )?;
        ...
        // execute QUERY_ISSI
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::QueryIssi),
            &mut init_segment,
        )?;
        let (current_issi, available_issi, status) =
            cmdq.get_query_issi_command_output(completed_cmd)?;
        trace!(
            "QueryISSI: {:?}, issi version :{}, available: {:#X}",
            status,
            current_issi,
            available_issi
        );
        ...
        if available_issi & ISSI_VERSION_1 == ISSI_VERSION_1 {
            let completed_cmd = cmdq.create_and_execute_command(
                CommandBuilder::new(CommandOpcode::SetIssi),
                &mut init_segment,
            )?;
        ...
        trace!(
            "ManagePages boot: {:?}",
            cmdq.get_command_status(completed_cmd)?
        );
        ...
        trace!(
            "QueryHCACap:{:?}, current capabilities: {:?}",
            status,
        ...
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::QueryPages).opmod(QueryPagesOpMod::InitPages as u16),
            &mut init_segment,
        )?;
        let (num_init_pages, status) = cmdq.get_query_pages_command_output(completed_cmd)?;
        trace!(
            "QueryPages: {:?}, num init pages: {:?}",
            status,
            num_init_pages
        );
        ...
        trace!(
            "QueryNicVportContext: {:?}, mac address: {:#X?}",
            status,
            mac_addr
        );
        ...
        trace!(
            "QueryVportState: {:?}, tx_speed: {:#X}, admin_state:{:#X}, state: {:#X}",
            status,
            tx_speed,
            admin_state,
            state
        ...
        if mtu > max_mtu {
            return Err("Required MTU is greater than maximum MTU!");
        }
        ...
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::ModifyNicVportContext).mtu(mtu),
            &mut init_segment,
        )?;
        let status = cmdq.get_command_status(completed_cmd)?;
        ...
        let (eq_mp, eq_pa) = create_contiguous_mapping(
            num_eq_entries * core::mem::size_of::<EventQueueEntry>(),
            NIC_MAPPING_FLAGS,
        )?;
        ...
        trace!(
            "AllocTransportDomain: {:?}, transport domain: {:?}",
            status,
            td
        );
        ...
        let (cq_mp, cq_pa) = create_contiguous_mapping(
            NUM_CQ_ENTRIES_SEND * core::mem::size_of::<CompletionQueueEntry>(),
            NIC_MAPPING_FLAGS,
        )?;

        // 2. Allocate page for doorbell
        let (db_page, db_pa) = create_contiguous_mapping(
            core::mem::size_of::<CompletionQueueDoorbellRecord>(),
            NIC_MAPPING_FLAGS,
        )?;
        ...
        // 3. Initialize the CQ
        let send_completion_queue =
            CompletionQueue::init(cq_mp, NUM_CQ_ENTRIES_SEND, db_page, cqn_s)?;
        ...
        let (cq_mp, cq_pa) = create_contiguous_mapping(
            cq_entries_r * core::mem::size_of::<CompletionQueueEntry>(),
            NIC_MAPPING_FLAGS,
        )?;
        // 2. Allocate page for doorbell
        let (db_page, db_pa) = create_contiguous_mapping(
        ...
        // execute CREATE_TIS
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::CreateTis).td(td),
            &mut init_segment,
        )?;
        ...
        let (q_mp, q_pa) =
            create_contiguous_mapping(rq_size_in_bytes + sq_size_in_bytes, NIC_MAPPING_FLAGS)?;
        let vaddr = q_mp.start_address();
        let (rq_mp, sq_mp) = q_mp
            .split(memory_structs::Page::containing_address(
                vaddr + rq_size_in_bytes,
            ))
            .map_err(|_e| "Could not split MappedPages")?;
        let sq_pa = q_pa + rq_size_in_bytes;
        debug!(
            "RQ paddr: {:?}, SQ paddr: {:?}, RQ vaddr: {:?}, SQ vaddr: {:?}",
            q_pa,
            sq_pa,
            rq_mp.start_address(),
            sq_mp.start_address()
        ...
        let (db_page, db_pa) =
            create_contiguous_mapping(core::mem::size_of::<DoorbellRecord>(), NIC_MAPPING_FLAGS)?;
        ...
        let send_queue =
            SendQueue::create(sq_mp, num_tx_descs, db_page, uar_page, sqn, tisn, rlkey)?;
        ...
        // MODIFY_SQ
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::ModifySq).sqn(sqn),
            &mut init_segment,
        )?;
        trace!(
            "Modify SQ status: {:?}",
        ...
        // MODIFY_RQ
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::ModifyRq).rqn(rqn),
            &mut init_segment,
        )?;
        trace!(
            "Modify RQ status: {:?}",
        ...
        let (tx_speed, admin_state, state, status) = cmdq.get_vport_state(completed_cmd)?;
        trace!(
            "Query Vport State status: {:?}, tx_speed: {:#X}, admin_state:{:#X}, state: {:#X}",
            status,
            tx_speed,
            admin_state,
            state
        ...
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::CreateFlowTable).queue_size(NUM_RULES),
            &mut init_segment,
        )?;
        ...
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::CreateFlowGroup).flow_table_id(ft_id),
            &mut init_segment,
        )?;
        ...
        let completed_cmd = cmdq.create_and_execute_command(
            CommandBuilder::new(CommandOpcode::SetFlowTableRoot).flow_table_id(ft_id),
            &mut init_segment,
        )?;
        trace!(
            "Set FT root status: {:?}",
    ...
    fn map_init_segment(
        mem_base: PhysicalAddress,
    ) -> Result<BoxRefMut<MappedPages, InitializationSegment>, &'static str> {
    ...
    fn allocate_pages_for_nic(
        num_pages: usize,
    ) -> Result<(Vec<MappedPages>, Vec<PhysicalAddress>), &'static str> {
    ...
    pub fn send(&mut self, buffer: TransmitBuffer) -> Result<(), &'static str> {
        ...
        let wqe_counter = self.send_queue.send(
            buffer.phys_addr,
            buffer.as_slice(0, packet_length as usize)?,
        );
        // self.send_completion_queue.wqe_posted(wqe_counter);
        self.send_completion_queue
            .check_packet_transmission(0, wqe_counter);
