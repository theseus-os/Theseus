...
pub fn init(double_fault_stack_top_unusable: Option<memory::VirtualAddress>) {
    println_raw!(
        "exceptions_early(): double_fault_stack_top_unusable: {:X?}",
    ...
    if let Some(df_stack_top) = double_fault_stack_top_unusable {
        ...
        tss.interrupt_stack_table[tss::DOUBLE_FAULT_IST_INDEX] =
            x86_64::VirtAddr::new(df_stack_top.value() as u64);
...
extern "x86-interrupt" fn early_page_fault_handler(
    ...
    println_raw!(
        "Exception IP {:#X} is at {:?}",
        stack_frame.instruction_pointer,
        mod_mgmt::get_initial_kernel_namespace().and_then(|ns| {
            ns.get_section_containing_address(
                memory::VirtualAddress::new_canonical(
    ...
    println_raw!(
        "Faulted access address {:#X} is at {:?}",
        accessed_address,
