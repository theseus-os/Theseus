...
pub extern "C" fn nano_core_start(
    ...
    try_exit!(
        logger::early_init(None, IntoIterator::into_iter(logger_ports).flatten())
            .map_err(|_a| "logger::early_init() failed.")
    );
    info!("Logger initialized.");
    println_raw!("nano_core_start(): initialized logger.");

    // initialize basic exception handlers
    exceptions_early::init(Some(VirtualAddress::new_canonical(
        early_double_fault_stack_top,
    )));
    ...
    println_raw!(
        "nano_core_start(): booted via multiboot2 with info at {:#X}.",
        multiboot_information_virtual_address
    );
    ...
    let default_namespace =
        match mod_mgmt::init(bootloader_modules, kernel_mmi_ref.lock().deref_mut()) {
    ...
    let (nano_core_crate_ref, ap_realmode_begin, ap_realmode_end) =
        match mod_mgmt::parse_nano_core::parse_nano_core(
            ...
            Ok((nano_core_crate_ref, init_symbols, _num_new_syms)) => {
                ...
                let ap_realmode_begin = try_exit!(init_symbols
    ...
    {
        try_exit!(
            mod_mgmt::replace_nano_core_crates::replace_nano_core_crates(
                &default_namespace,
                nano_core_crate_ref,
                &kernel_mmi_ref
            )
    ...
    #[cfg(loadable)]
    {
        ...
        let (captain_file, _ns) = try_exit!(CrateNamespace::get_crate_object_file_starting_with(
            default_namespace,
            "captain-"
        )
        .ok_or("couldn't find the singular \"captain\" crate object file"));
        let (_captain_crate, _num_captain_syms) =
            try_exit!(default_namespace.load_crate(&captain_file, None, &kernel_mmi_ref, false));
        println_raw!("nano_core_start(): loading the panic handling crate(s)...");
        let (panic_wrapper_file, _ns) =
            try_exit!(CrateNamespace::get_crate_object_file_starting_with(
                default_namespace,
                "panic_wrapper-"
            )
            .ok_or("couldn't find the singular \"panic_wrapper\" crate object file"));
        let (_pw_crate, _num_pw_syms) = try_exit!(default_namespace.load_crate(
            &panic_wrapper_file,
            None,
            &kernel_mmi_ref,
            false
    ...
    try_exit!(Err(
        "captain::init returned unexpectedly... it should be an infinite loop (diverging function)"
    ));
}
