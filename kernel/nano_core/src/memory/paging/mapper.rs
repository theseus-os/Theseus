...
impl Mapper {
        ...
        where A: FrameAllocator
    {
        ...
        assert!(pages_count == frames_count, "map_to_internal(): page count {} must equal frame count {}!", pages_count, frames_count);
    ...
    pub fn unmap<A>(&mut self, pages: PageIter, _allocator: &mut A) -> Result<(), &'static str> 
        where A: FrameAllocator
    {
        for page in pages {
