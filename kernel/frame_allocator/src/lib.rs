...
static FREE_RESERVED_FRAMES_LIST: Mutex<StaticArrayRBTree<Chunk>> =
    Mutex::new(StaticArrayRBTree::empty());
    ...
    R: IntoIterator<Item = P> + Clone,
{
    ...
    let all_areas = free_list[..free_list_idx]
        .iter()
        .flatten()
    ...
    fn from(alloc_err: AllocationError) -> &'static str {
        match alloc_err {
            AllocationError::AddressNotFree(..) => {
                "address was in use or outside of this frame allocator's range"
            }
...
fn frame_is_in_list(list: &StaticArrayRBTree<Chunk>, frame: &Frame) -> bool {
    match &list.0 {
        Inner::Array(ref arr) => {
...
pub fn allocate_frames_deferred(
    ...
    if let Some(paddr) = requested_paddr {
            ...
            find_specific_chunk(&mut free_reserved_frames_list, start_frame, num_frames)
        {
            ...
            if let Ok(result) = find_specific_chunk(
                &mut FREE_GENERAL_FRAMES_LIST.lock(),
                start_frame,
                num_frames,
            ) {
                Ok(result)
            ...
            // but ONLY if those frames are *NOT* in the general-purpose region.
                ...
                let new_reserved_frames =
                    add_reserved_region(&mut RESERVED_REGIONS.lock(), frames.clone())?;
                // If we successfully added a new reserved region,
                // then add those frames to the actual list of *available* reserved regions.
                let _new_free_reserved_frames = add_reserved_region(
                    &mut free_reserved_frames_list,
                    new_reserved_frames.clone(),
                )?;
...
pub fn allocate_frames_by_bytes_at(
    ...
    allocate_frames_by_bytes_deferred(Some(paddr), num_bytes).map(|(af, _action)| af)
}
...
pub fn allocate_frames_at(
    ...
    allocate_frames_deferred(Some(paddr), num_frames).map(|(af, _action)| af)
}
