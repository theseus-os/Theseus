//! This crate defines a `WindowProfile` trait.
//!
//! A `Window` object contains a refrence to an object which implements the `WindowProfile` trait. It profiles the basic information of the window and contains a framebuffer in which it can display itself. A `Window` object can use the methods of its profile to get these information or display.
//!
//! A window manager maintains a list of existing `Window` objects. It defines methods to manipulate them.
...
/// Trait for window. A window manager holds a list of objects who implement the `Window` trait.
...
pub trait WindowProfile {
    ...
    /// Gets a refrence to the framebuffer of the window.
    fn framebuffer(&self) -> &dyn FrameBuffer;

    /// Gets a mutable refrence to the framebuffer of the window.
