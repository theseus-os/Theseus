...
pub unsafe fn register_unwind_info(
    ...
    let text_end =
        VirtualAddress::new_canonical(text_section_base_address as usize + text_section_len);
...
/// for the given `text_section_base_address`.
pub unsafe fn deregister_unwind_info(text_section_base_address: *mut u8) -> Result<(), ()> {
    EXTERNAL_UNWIND_INFO
        .lock()
        .remove(&VirtualAddress::new_canonical(
            text_section_base_address as usize,
        ))
        .map(|_| ())
        .ok_or(())
}
...
pub fn get_unwind_info(text_section_address: VirtualAddress) -> Option<ExternalUnwindInfo> {
    // iterate over the entries in sorted order, up to the given `text_section_address` inclusively.
    for (_text_base_addr, uw_info) in EXTERNAL_UNWIND_INFO
        .lock()
        .range((Unbounded, Included(text_section_address)))
    {
