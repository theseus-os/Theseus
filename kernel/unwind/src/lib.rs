...
use gimli::{
    BaseAddresses, CfaRule, EhFrame, EndianSlice, FrameDescriptionEntry, NativeEndian, Pointer,
    RegisterRule, UninitializedUnwindContext, UnwindSection, UnwindTableRow, X86_64,
};
use memory::VirtualAddress;
use mod_mgmt::{CrateNamespace, SectionType, StrongCrateRef, StrongSectionRef};
...
impl FallibleIterator for StackFrameIter {
    ...
    fn next(&mut self) -> Result<Option<Self::Item>, Self::Error> {
        ...
        let caller_virt_addr =
            VirtualAddress::new(caller as usize).ok_or("caller wasn't a valid virtual address")?;

        // Get unwind info for the call site ("caller") address.
        let (eh_frame_sec, base_addrs) = self
            .namespace
            ...
            .and_then(|crate_ref| {
                get_eh_frame_info(&crate_ref).map(|(eh_frame_sec, base_addrs)| {
                    (EhFrameReference::Section(eh_frame_sec), base_addrs)
                })
            })
            // Second: search externally-registered unwind info for the caller address.
            .or_else(|| {
                external_unwind_info::get_unwind_info(caller_virt_addr).map(|uw_info| {
            ...
            .ok_or_else(|| {
                #[cfg(not(downtime_eval))]
                error!(
                    "StackTraceIter::next(): couldn't get unwind info for call site address: {:#X}",
                    caller
                );
        ...
        let row_ref = UnwindRowReference {
            caller,
            eh_frame_sec,
            base_addrs,
        };
    ...
    F: FuncWithRegisters,
{
    ...
    #[naked]
    unsafe extern "C" fn unwind_trampoline(
        _func: *mut FuncWithRegistersRefMut,
...
unsafe fn land(regs: &Registers, landing_pad_address: u64) -> Result<(), &'static str> {
    let mut landing_regs = LandingRegisters {
        ...
        rsp: regs[X86_64::RSP].ok_or(
            "unwind::land(): RSP was None, \
...
impl UnwindRowReference {
    ...
    where
        F: FnMut(
            &FrameDescriptionEntry<NativeEndianSliceReader, usize>,
            &UnwindTableRow<NativeEndianSliceReader>,
        ) -> Result<O, &'static str>,
    {
        ...
        let mut invoke_f_with_eh_frame_slice = |eh_frame_slice: &[u8]| {
            ...
            let fde = eh_frame
                .fde_for_address(&self.base_addrs, self.caller, EhFrame::cie_from_offset)
                .map_err(|_e| {
                    error!("gimli error: {:?}", _e);
                    "gimli error while finding FDE for address"
                })?;
            let unwind_table_row = fde
                .unwind_info_for_address(&eh_frame, &self.base_addrs, &mut unwind_ctx, self.caller)
                .map_err(|_e| {
                    error!("gimli error: {:?}", _e);
        ...
        match &self.eh_frame_sec {
            EhFrameReference::Section(sec) => {
                ...
                let eh_frame_slice: &[u8] =
                    sec_pages.as_slice(sec.mapped_pages_offset, sec.size())?;
            ...
            EhFrameReference::External(uw_info) => {
                ...
                let eh_frame_size =
                    uw_info.unwind_info.end.value() - uw_info.unwind_info.start.value();
                let eh_frame_slice =
                    unsafe { core::slice::from_raw_parts(eh_frame_ptr, eh_frame_size) };
                invoke_f_with_eh_frame_slice(eh_frame_slice)
            }
        }
    }
...
///
#[doc(hidden)]
pub fn start_unwinding(
    reason: KillReason,
    stack_frames_to_skip: usize,
) -> Result<(), &'static str> {
    ...
    let unwinding_context_ptr = {
        ...
        Box::into_raw(Box::new(UnwindingContext {
    ...
    match &res {
        ...
        &Err(e) => {
            error!(
                "BUG: unwinding the first stack frame returned unexpectedly. Error: {}",
                e
            );
        }
    }
...
fn continue_unwinding(unwinding_context_ptr: *mut UnwindingContext) -> Result<(), &'static str> {
    ...
    #[cfg(not(downtime_eval))]
    trace!(
        "continue_unwinding(): stack_frame_iter: {:#X?}",
        stack_frame_iter
    );

    let (mut regs, landing_pad_address) = if let Some(frame) =
        stack_frame_iter.next().map_err(|e| {
            error!(
        ...
        #[cfg(not(downtime_eval))]
        {
            ...
            info!(
                "  In func: {:?}",
                stack_frame_iter.namespace().get_section_containing_address(
                    VirtualAddress::new_canonical(frame.initial_address() as usize),
                    false
                )
            );
        ...
        if let Some(lsda) = frame.lsda() {
            ...
            if let Some((sec, _)) = stack_frame_iter
                .namespace()
                .get_section_containing_address(lsda, true)
            {
                #[cfg(not(downtime_eval))]
                info!("  parsing LSDA section: {:?}", sec);

                let starting_offset =
                    sec.mapped_pages_offset + (lsda.value() - sec.address_range.start.value());
                let length_til_end_of_mp = sec.address_range.end.value() - lsda.value();
                let sec_mp = sec.mapped_pages.lock();
                let lsda_slice = sec_mp
                    .as_slice::<u8>(starting_offset, length_til_end_of_mp)
                ...
                let entry = match table.call_site_table_entry_for_address(frame.call_site_address())
                {
                    ...
                    Err(e) => {
                        ...
                        let mut iter = table
                            ...
                            .map_err(|_e| "Couldn't find call_site_table_entries")?;

                        let mut closest_entry = None;
                        while let Some(entry) = iter
                            .next()
                            .map_err(|_e| "Couldn't iterate through the entries")?
                        {
                ...
                #[cfg(not(downtime_eval))]
                debug!(
                    "Found call site entry for address {:#X}: {:#X?}",
                    frame.call_site_address(),
                    entry
                );
                (
                    stack_frame_iter.registers().clone(),
                    entry.landing_pad_address(),
                )
            } else {
                error!(
                    "  BUG: couldn't find LSDA section (.gcc_except_table) for LSDA address: {:#X}",
                    lsda
                );
    ...
    #[cfg(not(downtime_eval))]
    debug!(
        "Jumping to landing pad (cleanup function) at {:#X}",
        landing_pad_address
    );
...
pub fn unwind_resume(unwinding_context_ptr: usize) -> ! {
    ...
    match continue_unwinding(unwinding_context_ptr) {
        ...
        Err(e) => {
            error!(
                "BUG: in unwind_resume(): continue_unwinding() returned an error: {}",
                e
            );
        }
    }
...
fn cleanup_unwinding_context(unwinding_context_ptr: *mut UnwindingContext) -> ! {
    ...
    #[cfg(not(downtime_eval))]
    warn!(
        "cleanup_unwinding_context(): invoking the task_cleanup_failure function for task {:?}",
        current_task
    );
