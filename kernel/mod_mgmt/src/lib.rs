...
pub use crate_name_utils::{
    crate_name_from_path, get_containing_crate_name, replace_containing_crate_name,
};
use fs_node::{DirRef, File, FileOrDir, FileRef};
use hashbrown::HashMap;
use memfs::MemFile;
use memory::{
    allocate_frames_by_bytes_at, allocate_pages_by_bytes, EntryFlags, MappedPages,
    MemoryManagementInfo, MmiRef, VirtualAddress,
};
use path::Path;
...
///
pub fn create_application_namespace(
...
pub fn init(
    ...
    let (_namespaces_dir, default_kernel_namespace_dir) =
        parse_bootloader_modules_into_files(bootloader_modules, kernel_mmi)?;
...
fn parse_bootloader_modules_into_files(
    ...
    let namespaces_dir =
        VFSDirectory::new(NAMESPACES_DIRECTORY_NAME.to_string(), root::get_root())?;
    // create the top-level directory to hold all extra files
    let extra_files_dir =
        VFSDirectory::new(EXTRA_FILES_DIRECTORY_NAME.to_string(), root::get_root())?;
    ...
    for m in bootloader_modules {
        ...
        let (crate_type, prefix, file_name) =
            if let Ok((c, p, f)) = CrateType::from_module_name(m.name().as_str()) {
                (c, p, f)
            } else {
    ...
    Ok((
        namespaces_dir,
        prefix_map
            .remove(CrateType::Kernel.default_namespace_name())
            .ok_or("BUG: no default namespace found")?,
    ))
}
...
fn parse_extra_file(
    ...
    let mut iter = extra_file
        .name()
        .as_str()
        .split(EXTRA_FILES_DIRECTORY_DELIMITER)
        .peekable();
...
impl NamespaceDir {
    ...
    pub fn get_crate_object_file(&self, crate_module_file_name: &str) -> Option<FileRef> {
        let (_crate_type, _prefix, objfilename) =
            CrateType::from_module_name(crate_module_file_name).ok()?;
    ...
    pub fn write_crate_object_file(
        &self,
        crate_object_file_name: &str,
        content: &[u8],
    ) -> Result<FileRef, &'static str> {
        let (_crate_type, _prefix, objfilename) =
            CrateType::from_module_name(crate_object_file_name)?;
...
impl fmt::Debug for IntoCrateObjectFile {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            Self::File(object_file) => dbg.field(
                "File",
                &object_file
                    .try_lock()
...
impl Drop for AppCrateRef {
    fn drop(&mut self) {
        ...
        if let Some(_removed_app_crate) = self
            .namespace
            .crate_tree()
            .lock()
            .remove_str(&crate_locked.crate_name)
        {
            ...
            error!(
                "BUG: the dropped AppCrateRef {:?} could not be removed from namespace {:?}",
                self.crate_ref,
                self.namespace.name()
            );
        }
...
impl CrateNamespace {
    ...
    pub fn crate_names(&self, recursive: bool) -> Vec<String> {
        let mut crates: Vec<String> = self
            ...
            .lock()
            .keys()
            .map(|bstring| String::from(bstring.as_str()))
            .collect();

        if recursive {
            if let Some(mut crates_recursive) = self
                .recursive_namespace
                .as_ref()
                .map(|r_ns| r_ns.crate_names(recursive))
            {
    ...
    pub fn for_each_crate<F>(&self, recursive: bool, mut f: F)
    where
    ...
    pub fn get_crate(&self, crate_name: &str) -> Option<StrongCrateRef> {
        self.crate_tree
            ...
            .or_else(|| {
                self.recursive_namespace
                    .as_ref()
                    .and_then(|r_ns| r_ns.get_crate(crate_name))
            })
    }
        ...
        namespace
            ...
            .or_else(|| {
                namespace
                    .recursive_namespace
                    .as_ref()
                    .and_then(|r_ns| Self::get_crate_and_namespace(r_ns, crate_name))
            })
    }
    ...
    pub fn get_crates_starting_with<'n>(
        ...
        let mut crates_in_this_namespace = crates
            .iter_prefix_str(crate_name_prefix)
            .map(|(key, val)| (key.clone().into(), val.clone_shallow(), namespace))
            .collect::<Vec<_>>();

        // Second, we make a similar list for the recursive namespace.
        let mut crates_in_recursive_namespace = namespace
            .recursive_namespace
            .as_ref()
    ...
    pub fn get_crate_starting_with<'n>(
        ...
        let mut crates_iter =
            Self::get_crates_starting_with(namespace, crate_name_prefix).into_iter();
    ...
    pub fn get_crate_object_files_starting_with<'n>(
        ...
        // Second, we make a similar list for the recursive namespace.
        let mut files_in_recursive_namespace = namespace
            .recursive_namespace
            .as_ref()
    ...
    pub fn get_crate_object_file_starting_with<'n>(
        ...
        let mut files_iter =
            Self::get_crate_object_files_starting_with(namespace, file_name_prefix).into_iter();
    ...
    pub fn method_get_crate_object_files_starting_with(
        ...
        // Second, we make a similar list for the recursive namespace.
        let mut files_in_recursive_namespace = self
            .recursive_namespace
            .as_ref()
    ...
    pub fn method_get_crate_object_file_starting_with(
        ...
        let mut files_iter = self
            .method_get_crate_object_files_starting_with(file_name_prefix)
            .into_iter();
        files_iter.next().filter(|_| files_iter.next().is_none()) // ensure single element
    }
    ...
    pub fn load_crate_as_application(
        ...
        debug!(
            "load_crate_as_application(): trying to load application crate at {:?}",
            crate_object_file.lock().get_absolute_path()
        );
        ...
        let new_crate_ref =
            namespace.load_crate_internal(crate_object_file, None, kernel_mmi_ref, verbose_log)?;
        {
            ...
            namespace
                .crate_tree
                .lock()
                .insert_str(&new_crate.crate_name, CowArc::clone_shallow(&new_crate_ref));
            info!(
                "loaded new application crate: {:?}, num sections: {}, added {} new symbols",
                new_crate.crate_name,
                new_crate.sections.len(),
                _new_syms
            );
        }
        Ok(AppCrateRef {
    ...
    pub fn load_crate(
        ...
        #[cfg(not(loscd_eval))]
        debug!(
            "load_crate: trying to load crate at {:?}",
            crate_object_file.lock().get_absolute_path()
        );
        let new_crate_ref = self.load_crate_internal(
            crate_object_file,
        ...
        #[cfg(not(loscd_eval))]
        info!(
            "loaded new crate {:?}, num sections: {}, added {} new symbols.",
            new_crate_name, _num_sections, new_syms
        );
        self.crate_tree
            .lock()
            .insert(new_crate_name.into(), new_crate_ref.clone_shallow());
    ...
    fn load_crate_internal(
        ...
        let (new_crate_ref, elf_file) =
            self.load_crate_sections(cf.deref(), kernel_mmi_ref, verbose_log)?;
        ...
        I: Iterator<Item = &'f FileRef>,
    {
        ...
        let mut partially_loaded_crates: Vec<(StrongCrateRef, ElfFile)> =
            Vec::with_capacity(locked_crate_files.len());
        for locked_crate_file in &locked_crate_files {
            let (new_crate_ref, elf_file) =
                self.load_crate_sections(locked_crate_file.deref(), kernel_mmi_ref, verbose_log)?;
            let _new_syms =
                self.add_symbols(new_crate_ref.lock_as_ref().sections.values(), verbose_log);
    ...
    pub fn rewrite_section_dependents(
        ...
        for weak_dep in &old_section.inner.read().sections_dependent_on_me {
            let target_sec = weak_dep
                .section
                .upgrade()
                .ok_or_else(|| "couldn't upgrade WeakDependent.section")?;
            let relocation_entry = weak_dep.relocation;

            debug!(
                "rewrite_section_dependents(): target_sec: {:?}, old_sec: {:?}, new_sec: {:?}",
                target_sec, old_section, new_section
            );
            ...
            // then we need to temporarily remap them as writable here so we can fix up the target_sec's new relocation entry.
            {
                ...
                if !target_sec_initial_flags.is_writable() {
                    target_sec_mapped_pages.remap(
                        &mut kernel_mmi_ref.lock().page_table,
                        target_sec_initial_flags | EntryFlags::WRITABLE,
                    )?;
                }
            ...
            for mut strong_dep in target_sec.inner.write().sections_i_depend_on.iter_mut() {
                if Arc::ptr_eq(&strong_dep.section, old_section)
    ...
    fn load_crate_sections<'f>(
        ...
        if self.get_crate(&crate_name).is_some() {
            return Err(
                "the crate has already been loaded, cannot load it again in the same namespace",
            );
        }
        ...
        if typ != Type::Relocatable {
            error!(
                "load_crate_sections(): crate \"{}\" was a {:?} Elf File, must be Relocatable!",
                &crate_name, typ
            );
        ...
        #[cfg(not(loscd_eval))]
        debug!(
            "Parsing Elf kernel crate: {:?}, size {:#x}({})",
            abs_path, size_in_bytes, size_in_bytes
        );
            ...
            for entry in symtab.iter() {
                ...
                if entry.get_binding() == Ok(xmas_elf::symbol_table::Binding::Global) {
                    match entry.get_type() {
                        Ok(
                            xmas_elf::symbol_table::Type::Func
        ...
        if let Some((ref tp, ref tp_range)) = text_pages {
            ...
            let text_source = elf_file.input.get(..text_size).ok_or(
                "BUG: end of last .text section was miscalculated to be beyond ELF file bounds",
            )?;
        ...
        let mut read_only_pages_locked =
            rodata_pages.as_ref().map(|(rp, _)| (rp.clone(), rp.lock()));
        let mut read_write_pages_locked =
            data_pages.as_ref().map(|(dp, _)| (dp.clone(), dp.lock()));
        ...
        for (shndx, sec) in elf_file.section_iter().enumerate() {
            ...
            if is_exec && !is_write {
                ...
                let name = if is_global && name.starts_with(UNLIKELY_PREFIX) {
                    name.get(UNLIKELY_PREFIX.len()..).ok_or_else(|| {
                        error!(
                            "Failed to get the .text.unlikely. section's name: {:?}",
                            sec_name
                        );
            ...
            // because they contain thread-local storage initializer data that is only read from.
                ...
                if let Some((ref rp_ref, ref mut rp)) = read_only_pages_locked {
                    ...
                    let (_tls_offset, new_tls_section) = self
                        .tls_initializer
                        .lock()
            ...
            // Fourth, if neither executable nor TLS nor writable, handle .rodata sections.
                ...
                if let Some((ref rp_ref, ref mut rp)) = read_only_pages_locked {
                    ...
                    let dest_vaddr = rp
                        .address_at_offset(rodata_offset)
        ...
        {
            let mut new_crate_mut = new_crate.lock_as_mut().ok_or_else(|| {
                "BUG: load_crate_sections(): couldn't get exclusive mutable access to new_crate"
            })?;
    ...
    fn perform_relocations(
        ...
        let mut new_crate = new_crate_ref.lock_as_mut().ok_or(
            "BUG: perform_relocations(): couldn't get exclusive mutable access to new_crate",
        )?;
        if verbose_log {
            debug!(
                "=========== moving on to the relocations for crate {} =========",
                new_crate.crate_name
            );
        }
        let symtab = find_symbol_table(&elf_file)?;
        ...
        for sec in elf_file
            .section_iter()
            .filter(|sec| sec.get_type() == Ok(ShType::Rela) && sec.size() != 0)
        {
            use xmas_elf::sections::SectionData::Rela64;
            if verbose_log {
                trace!(
                    "Found Rela section name: {:?}, type: {:?}, target_sec_index: {:?}",
                    sec.get_name(&elf_file),
                    sec.get_type(),
                    sec.info()
                );
            }
            ...
            if let Ok(name) = sec.get_name(&elf_file) {
                ...
                // ignore debug special sections for now
                {
                    continue;
                }
            }
            ...
            let rela_array = match sec.get_data(&elf_file) {
                ...
                _ => {
                    error!(
                        "Found Rela section that wasn't able to be parsed as Rela64: {:?}",
                        sec
                    );
            ...
            let target_sec = new_crate.sections.get(&target_sec_shndx).ok_or_else(|| {
                error!(
                    "ELF file error: target section was not loaded for Rela section {:?}!",
                    sec.get_name(&elf_file)
                );
            ...
            let mut target_sec_internal_dependencies: Vec<InternalDependency> = Vec::new();
            {
                ...
                for rela_entry in rela_array {
                    ...
                    if verbose_log {
                        let source_sec_header_name = source_sec_entry
                            .get_section_header(
                                &elf_file,
                                rela_entry.get_symbol_table_index() as usize,
                            )
                            .and_then(|s| s.get_name(&elf_file));
                        trace!(
                            "             relevant section [{}]: {:?}",
                            source_sec_shndx,
                            source_sec_header_name
                        );
                    ...
                    if source_and_target_in_same_crate {
                        ...
                        #[cfg(internal_deps)]
                        target_sec_internal_dependencies
                            .push(InternalDependency::new(relocation_entry, source_sec_shndx))
                    } else {
            ...
            {
                debug!(
                    "Invalidating TlsInitializer due to relocation written to section {:?}",
                    &*target_sec
                );
            ...
            // add the target section's dependencies and relocation details all at once
            {
                ...
                target_sec_inner
                    ...
                    .append(&mut target_sec_dependencies);
                #[cfg(internal_deps)]
                target_sec_inner
                    .internal_dependencies
                    .append(&mut target_sec_internal_dependencies);
            }
        }
        ...
        if let Some(ref tp) = new_crate.text_pages {
            tp.0.lock()
                .remap(&mut kernel_mmi_ref.lock().page_table, TEXT_SECTION_FLAGS)?;
        }
        if let Some(ref rp) = new_crate.rodata_pages {
            rp.0.lock()
                .remap(&mut kernel_mmi_ref.lock().page_table, RODATA_SECTION_FLAGS)?;
        }
    ...
    fn add_symbol(
        ...
        match existing_symbol_map.entry(new_section_key.into()) {
                ...
                if log_replacements {
                    if let Some(old_sec) = old_val.get().upgrade() {
                        ...
                        if new_section.size() != old_sec.size() {
                            ...
                            warn!(
                                "Replacing new symbol already present: old {:?}, new: {:?}",
                                old_sec.name, new_section.name
                            );
                        }
                    }
                }
            ...
            qp_trie::Entry::Vacant(new_entry) => {
                if log_replacements {
                    debug!(
                        "         add_symbol(): Adding brand new symbol: new: {:?}",
                        new_section
                    );
                }
    ...
    pub fn add_symbols<'a, I>(&self, sections: I, _log_replacements: bool) -> usize
    where
        ...
        F: Fn(&LoadedSection) -> bool,
    {
        ...
        for sec in sections.into_iter() {
            ...
            if condition {
                ...
                let added = CrateNamespace::add_symbol(
                    &mut existing_map,
                    sec.name.clone(),
                    sec,
                    log_replacements,
    ...
    pub fn get_section_containing_address(
        ...
        let containing_crate =
            self.get_crate_containing_address(virt_addr, search_all_section_types)?;
    ...
    pub fn get_symbol_and_namespace(
        ...
        let weak_symbol = self
            .symbol_map
            .lock()
            .get_str(demangled_full_symbol)
            .cloned();
        weak_symbol
            .map(|sym| (sym, self))
            ...
            .or_else(|| {
                self.recursive_namespace
                    .as_ref()
                    .and_then(|rns| rns.get_symbol_and_namespace(demangled_full_symbol))
            })
    }
    ...
    fn get_symbol_internal(&self, demangled_full_symbol: &str) -> Option<WeakSectionRef> {
        self.get_symbol_and_namespace(demangled_full_symbol)
            .map(|(sym, _ns)| sym)
    }
    ...
    pub fn get_symbol_or_load(
        ...
        if let Some(backup) = temp_backup_namespace {
            ...
            if let Some(sec) = self.get_symbol_from_backup_namespace(
        ...
        if let Some(weak_sec) = self.load_crate_for_missing_symbol(
            demangled_full_symbol,
            temp_backup_namespace,
            kernel_mmi_ref,
            verbose_log,
            ...
            #[cfg(not(loscd_eval))]
            debug!(
                "Symbol \"{}\" not found. Try loading the specific crate manually first.",
                demangled_full_symbol
            );
    ...
    fn get_symbol_from_backup_namespace(
        ...
        let (weak_sec, _found_in_ns) = if !fuzzy_matching {
            ...
            let fuzzy_matches = temp_backup_namespace.find_symbols_starting_with_and_namespace(
                LoadedSection::section_name_without_hash(demangled_full_symbol),
            );
        ...
        self.crate_tree
            .lock()
            .insert(parent_crate_name.into(), parent_crate_ref);
        return Some(sec);
    }
    ...
    fn load_crate_for_missing_symbol(
        ...
        for potential_crate_name in get_containing_crate_name(demangled_full_symbol) {
            ...
            let (potential_crate_file, ns_of_crate_file) = match self
                .method_get_crate_object_file_starting_with(&potential_crate_name)
            {
                Some(found) => found,
                // TODO: should we be blindly recursively searching the backup namespace's files?
                None => match temp_backup_namespace.and_then(|backup| {
                    backup.method_get_crate_object_file_starting_with(&potential_crate_name)
                }) {
            ...
            let potential_crate_file_path =
                Path::new(potential_crate_file.lock().get_absolute_path());
            // Check to make sure this crate is not already loaded into this namespace (or its recursive namespace).
            if self
                .get_crate(crate_name_from_path(&potential_crate_file_path))
                .is_some()
            {
                trace!(
                    "  (skipping already-loaded crate {:?})",
                    potential_crate_file_path
                );
                continue;
            }
            #[cfg(not(loscd_eval))]
            ...
            match ns_of_crate_file.load_crate(
                &potential_crate_file,
                temp_backup_namespace,
                kernel_mmi_ref,
                verbose_log,
            ) {
    ...
    pub fn find_symbols_starting_with(&self, symbol_prefix: &str) -> Vec<(String, WeakSectionRef)> {
        let mut syms: Vec<(String, WeakSectionRef)> = self
            .symbol_map
            .lock()
        ...
        if let Some(mut syms_recursive) = self
            .recursive_namespace
            .as_ref()
            .map(|r_ns| r_ns.find_symbols_starting_with(symbol_prefix))
        {
    ...
    pub fn find_symbols_starting_with_and_namespace(
        ...
        let mut syms: Vec<(String, WeakSectionRef, &CrateNamespace)> = self
            .symbol_map
            .lock()
            .iter_prefix_str(symbol_prefix)
        ...
        if let Some(mut syms_recursive) = self
            .recursive_namespace
            .as_ref()
            .map(|r_ns| r_ns.find_symbols_starting_with_and_namespace(symbol_prefix))
        {
            syms.append(&mut syms_recursive);
        }
    ...
    fn get_symbol_starting_with_internal(&self, symbol_prefix: &str) -> Option<WeakSectionRef> {
        ...
        let symbol_in_this_namespace = iter
            .next()
        ...
        let symbol_in_recursive_namespace = self
            .recursive_namespace
            .as_ref()
            .and_then(|r_ns| r_ns.get_symbol_starting_with_internal(symbol_prefix));
...
fn allocate_section_pages(
    ...
    let executable_pages = if exec_bytes > 0 {
        Some(allocate_and_map_as_writable(
            exec_bytes,
            TEXT_SECTION_FLAGS,
            kernel_mmi_ref,
        )?)
...
fn dump_weak_dependents(sec: &LoadedSection, prefix: String) {
    ...
    if !sec_inner.sections_dependent_on_me.is_empty() {
        debug!(
            "{}Section \"{}\": sections dependent on me (weak dependents):",
            prefix, sec.name
        );
...
pub fn find_symbol_table<'e>(
    ...
    match symtab_data {
        ...
        _ => Err("no symbol table found. Was file stripped?"),
    }
}
...
impl TlsInitializer {
    ...
    pub(crate) fn add_existing_static_tls_section(
        ...
        self.static_section_offsets
            .insert(range, StrongSectionRefWrapper(tls_section));
    ...
    pub(crate) fn add_new_dynamic_tls_section(
        ...
        for gap in self
            .dynamic_section_offsets
            .gaps(&range_after_tls_self_pointer)
        {
        ...
        if let Some(start) = start_index {
            ...
            section.address_range = VirtualAddress::new_canonical(range.start)
                ..VirtualAddress::new_canonical(range.end);
            let section_ref = Arc::new(section);
            self.end_of_dynamic_sections = max(self.end_of_dynamic_sections, range.end);
            self.dynamic_section_offsets
                .insert(range, StrongSectionRefWrapper(Arc::clone(&section_ref)));
    ...
    pub(crate) fn get_data(&mut self) -> TlsDataImage {
        ...
        let required_capacity = if total_section_size > 0 {
            total_section_size + POINTER_SIZE
        } else {
            0
        };
        ...
        fn copy_tls_section_data(
            ...
            for (range, sec) in section_offsets.iter() {
                ...
                if sec.get_type() == SectionType::TlsData {
                    ...
                    let sec_data: &[u8] = sec_mp
                        .as_slice(sec.mapped_pages_offset, sec.size())
                        .unwrap();
        ...
        if self.cache_status == CacheStatus::Invalidated {
            ...
            copy_tls_section_data(
                &mut new_data,
                &self.static_section_offsets,
                &mut end_of_previous_range,
            );
        ...
        if let Some(dest_slice) =
            data_copy.get_mut(self_ptr_offset..(self_ptr_offset + POINTER_SIZE))
        {
            ...
            panic!(
                "BUG: offset of TLS self pointer was out of bounds in the TLS data image:\n{:02X?}",
                data_copy
            );
        }
    }
