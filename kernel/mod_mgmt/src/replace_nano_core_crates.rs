...
use super::{CrateNamespace, LoadedCrate, MmiRef, StrongCrateRef};
...
pub fn replace_nano_core_crates(
    ...
    let (crate_object_file, _ns) =
        CrateNamespace::get_crate_object_file_starting_with(namespace, "memory-")
...
fn load_crate_using_nano_core_data_sections(
    ...
    debug!(
        "Replacing nano_core's constituent crate {:?}",
        cf.get_name()
    );
    ...
    let (new_crate_ref, elf_file) =
        namespace.load_crate_sections(&*cf, kernel_mmi_ref, verbose_log)?;
    ...
    {
        let mut new_crate = new_crate_ref
            .lock_as_mut()
            .ok_or_else(|| "BUG: could not get exclusive mutable access to newly-loaded crate")?;
