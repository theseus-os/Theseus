...
use crate_metadata::{
    LoadedCrate, LoadedSection, SectionType, Shndx, StrongCrateRef, StrongSectionRef,
};
use cstr_core::CStr;
...
> {
    ...
    let parse_result = match nano_core_file_path.extension() {
        Some("sym") => parse_nano_core_symbol_file(
            real_namespace,
            &nano_core_crate_ref,
            &text_pages,
            &rodata_pages,
        ...
        Some("bin") => parse_nano_core_binary(
            real_namespace,
            &nano_core_crate_ref,
            &text_pages,
            &rodata_pages,
    ...
    // Access and propertly set the new_crate's sections list and other items.
    {
        let mut new_crate_mut = try_mp!(
            nano_core_crate_ref.lock_as_mut().ok_or_else(|| {
                "BUG: parse_nano_core(): couldn't get exclusive mutable access to new_crate"
            }),
            text_pages,
            rodata_pages,
            data_pages
    ...
    real_namespace
        .crate_tree
        .lock()
        .insert(crate_name.into(), nano_core_crate_ref.clone_shallow());
    info!(
...
fn add_new_section(
        ...
        .map_or(false, |(shndx, _)| sec_ndx == shndx)
    {
        ...
        namespace
            .tls_initializer
            .lock()
            .add_existing_static_tls_section(tls_offset, Arc::clone(&tls_section))
            .unwrap();
        Some(tls_section)
    } else if shndxs
        .tls_bss_shndx
