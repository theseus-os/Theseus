...
pub fn connect(
    ...
    debug!(
        "smoltcp_helper: connecting from {}:{} to {} ...",
        iface
            .lock()
            .ip_addrs()
            .get(0)
    ...
    loop {
        ...
        if millis_since(start)? > timeout_millis {
            error!(
                "smoltcp_helper: failed to connect to socket, timed out after {} ms",
                timeout_millis
            );
    ...
    debug!(
        "smoltcp_helper: connected!  (took {} ms)",
        millis_since(start)?
    );
    Ok(())
}
...
pub fn poll_iface(
    ...
    let packets_were_sent_or_received =
        match iface.lock().poll(sockets, Instant::from_millis(timestamp)) {
            Ok(b) => b,
            Err(err) => {
