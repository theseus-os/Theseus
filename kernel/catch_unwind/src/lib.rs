...
fn panic_callback<F, A, R>(data_ptr: *mut u8, exception_object: *mut u8)
where
    F: FnOnce(A) -> R,
{
    let data = unsafe { &mut *(data_ptr as *mut TryIntrinsicArg<F, A, R>) };
    let unwinding_context_boxed =
        unsafe { Box::from_raw(exception_object as *mut unwind::UnwindingContext) };
