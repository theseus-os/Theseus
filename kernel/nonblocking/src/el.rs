...
use alloc::sync::Arc;
use irq_safety::{MutexIrqSafe, MutexIrqSafeGuard, hold_interrupts, enable_interrupts};
...
impl EventListener {
    ...
    // TODO("Uncomment after Instant implement")
    ...
    // TODO("Uncomment deadline code after Instant implement")
    fn wait_internal(mut self/*, deadline: Option<Instant> */ ) -> bool {
        ...
        // Set this listener's state to `Waiting`.
        {
            ...
            match e.state.replace(State::Notified(false)) {
                ...
                _ => e.state.set(State::Waiting(task::get_my_current_task().expect("couldn't get current task").clone())),
            }
        }
...
struct ListGuard<'a> {
    ...
    guard: MutexIrqSafeGuard<'a, List>,
}
