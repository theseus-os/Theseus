...
pub fn init_rx_queue<T: RxDescriptor>(num_desc: usize, rx_buffer_pool: &'static mpmc::Queue<ReceiveBuffer>, buffer_size: usize, 
            rdbal: &mut Rdbal, rdbah: &mut Rdbah, rdlen: &mut Rdlen, rdh: &mut Rdh, rdt: &mut Rdt)
    ...
    *rdbal = rx_desc_phys_addr_lower;
    *rdbah = rx_desc_phys_addr_higher;

    // write the length (in total bytes) of the rx descs array
    *rdlen = size_in_bytes_of_all_rx_descs_per_queue as u32; // should be 128 byte aligned, minimum 8 descriptors
...
pub fn init_tx_queue<T: TxDescriptor>(num_desc: usize, tdbal: &mut Tdbal, tdbah: &mut Tdbah, tdlen: &mut Tdlen, 
            tdh: &mut Tdh, tdt: &mut Tdt) 
    ...
    *tdbal = tx_desc_phys_addr_lower; 
    *tdbah = tx_desc_phys_addr_higher; 

    // write the length (in total bytes) of the tx descs array
    *tdlen = size_in_bytes_of_all_tx_descs as u32;               
