...
cd results
grep -rl --null --include '*.txt' "detected faults recovered" . | xargs -0r cp -t ../results_recovered

cd ..
rm -rf results_temp
mkdir results_temp
cd results
grep -rL --null --include '*.txt' "detected faults recovered" . | xargs -0r cp -t ../results_temp
