...
pub struct AtomicLinkedListIter<'a, T: 'static> {
    curr: &'a AtomicPtr<Node<T>>,
    _phantom: PhantomData<&'a T>, // we don't need this with the &'a above
}
impl<'a, T: 'static> Iterator for AtomicLinkedListIter<'a, T> {
    type Item = &'a T; 
...
pub struct AtomicLinkedListIterMut<'a, T: 'static> {
    curr: &'a AtomicPtr<Node<T>>,
    _phantom: PhantomData<&'a T>, // we don't need this with the &'a above
}
impl<'a, T: 'static> Iterator for AtomicLinkedListIterMut<'a, T> {
    type Item = &'a mut T; 
