...
use memory::{EntryFlags, MappedPages, Page, VirtualAddress};
use mod_mgmt::{
    find_symbol_table, write_relocation, CrateNamespace, RelocationEntry, StrongDependency,
};
use path::Path;
use rustc_demangle::demangle;
use xmas_elf::{program::SegmentData, sections::ShType, ElfFile};
...
fn rmain(matches: Matches) -> Result<c_int, String> {
    ...
    let path = matches
        .free
        .get(0)
        .ok_or_else(|| format!("Missing path to ELF executable"))?;
    ...
    let (mut segments, entry_point, _vaddr_offset, elf_file) =
        parse_and_load_elf_executable(byte_slice)?;
...
fn parse_and_load_elf_executable<'f>(
    ...
    let mut all_pages = memory::allocate_pages_by_bytes_at(
        VirtualAddress::new(start_vaddr)
            .ok_or_else(|| format!("Segment had invalid virtual address {:#X}", start_vaddr))?,
        total_size_in_bytes,
    )
    .map_err(|_| {
        format!(
            "Failed to allocate {} bytes at {}",
            total_size_in_bytes, start_vaddr
        )
    })?;
    ...
    for (segment_ndx, prog_hdr) in elf_file.program_iter().enumerate() {
        ...
        let mut start_vaddr =
            VirtualAddress::new(prog_hdr.virtual_addr() as usize).ok_or_else(|| {
        ...
        let (this_ap, remaining_pages) = all_pages.split(end_page + 1).map_err(|_ap| {
            format!(
                "Failed to split allocated pages {:?} at page {:#X}",
                _ap, start_vaddr
            )
        })?;
        ...
        let offset_into_mp = mp.offset_of_address(start_vaddr).ok_or_else(|| {
            format!(
                "BUG: destination address {:#X} wasn't within segment's {:?}",
                start_vaddr, mp
            )
        })?;
    ...
    let mut entry_point_vaddr = VirtualAddress::new(entry_point).ok_or_else(|| {
        format!(
            "ELF entry point was invalid virtual address: {:#X}",
            entry_point
        )
    })?;
        ...
        .filter(|sec| sec.get_type() == Ok(ShType::Rela) && sec.size() != 0)
    {
        ...
        for rela_entry in rela_array {
                ...
                .upgrade()
            {
                ...
                let offset_into_target_segment = target_segment
                    .mp
                    .offset_of_address(VirtualAddress::new(relocation_entry.offset).ok_or_else(
                        || {
                            format!(
                                "relocation_entry.offset {:#X} was not a valid virtual address",
                                relocation_entry.offset
                            )
                        },
                    )?)
                    .ok_or(format!(
                        "target segment {:X?} did not contain relocation_entry.offset {:#X}",
                        target_segment, relocation_entry.offset
                    ))?;
                ...
                if verbose_log {
                    debug!(
                        "                 Performing relocation target {} + {:#X} <-- source {}",
                        target_segment.mp.start_address(),
                        offset_into_target_segment,
                        existing_source_sec.name
                    );
                }
                write_relocation(
                    relocation_entry,
                ...
                trace!(
                    "Skipping relocation that points to non-Theseus section: {:?}",
                    demangled
                );
            }
        }
