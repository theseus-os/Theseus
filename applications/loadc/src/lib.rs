...
extern crate task;
extern crate xmas_elf; // for basic C types/typedefs used in libc
...
fn rmain(matches: Matches) -> Result<c_int, String> {
    let (curr_wd, namespace, mmi) = task::with_current_task(|curr_task| {
        (curr_task.get_env().lock().working_dir.clone(), curr_task.get_namespace().clone(), curr_task.mmi.clone())
    })
    ...
    // Now, overwrite (recalculate) the relocations that refer to symbols that already exist in
    // Theseus, most important of which are static data sections,
    // as it is logically incorrect to have duplicates of data that are supposed to be global
    // system-wide singletons. We should throw a warning here if there are no relocations in the
    // file, as it was probably built/linked with the wrong arguments.
    overwrite_relocations(&namespace, &mut segments, &elf_file, &mmi, false)?;

    // Remap each segment's mapped pages using the correct flags; they were previously mapped as
    // always writable.
    {
...
impl Offset {
    ...
    fn new(first: usize, second: usize) -> Offset {
        if first < second { Offset::Negative(second - first) } else { Offset::Positive(first - second) }
    }
...
/// 2. The virtual address of the executable's entry point, e.g., the `_start` function. This is the
///    function that we should call to start running the executable.
/// 3. The `Offset` by which all virtual addresses in the loaded executable should be shifted by.
///    This is the difference between where the program is *actually* loaded in memory and where the
///    program *expected* to be loaded into memory.
/// 4. A reference to the parsed `ElfFile`, whose lifetime is tied to the given `file_contents`
///    parameter.
fn parse_and_load_elf_executable(
    ...
    // so we have to load the C executable at the exact virtual address it specifies (since it's
    // non-relocatable).

    // TODO FIXME: remove this old approach of invalidly loading non-PIE executables at other
    // virtual addresses than what they expect.             Also remove the whole idea of the
    // "Offset", since that will be built into position-independent executables.
    // This is because this only works for SUPER SIMPLE C programs, in which we can just maintain
    // the *relative* position of each segment             in memory with respect to other
    // segments to ensure they're consistent.
    //
    // Not really necessary to do this, but we iterate over all segments first to find the total
    // range of virtual pages we must allocate.
    ...
    let mut all_pages = memory::allocate_pages_by_bytes_at(
        VirtualAddress::new(start_vaddr)
            .ok_or_else(|| format!("Segment had invalid virtual address {start_vaddr:#X}"))?,
        total_size_in_bytes,
    )
    ...
    for (segment_ndx, prog_hdr) in elf_file.program_iter().enumerate() {
        ...
        // 1) memory size: the size in memory that the segment, when loaded, will actually consume. This is
        //    how much virtual memory space we have to allocate for it.
        // 2) file size: the size of the segment's actual data from the ELF file itself. This is how much
        //    data we will actually copy from the file's segment into our allocated memory.
        // The difference is primarily due to .bss sections, in which the file size will be less
        // than the memory size. If memory size > file size, the difference should be filled
        // with zeros.
        ...
        let (this_ap, remaining_pages) = all_pages
            .split(end_page + 1)
            .map_err(|_ap| format!("Failed to split allocated pages {_ap:?} at page {start_vaddr:#X}"))?;
        all_pages = remaining_pages;
        // debug!("Successfully split pages into {:?} and {:?}", this_ap, all_pages);
        // debug!("Adjusted segment vaddr: {:#X}, size: {:#X}, {:?}", start_vaddr,
        // memory_size_in_bytes, this_ap.start_address());
        ...
        let offset_into_mp = mp
            .offset_of_address(start_vaddr)
            .ok_or_else(|| format!("BUG: destination address {start_vaddr:#X} wasn't within segment's {mp:?}"))?;
        match prog_hdr.get_data(&elf_file).map_err(String::from)? {
            SegmentData::Undefined(segment_data) => {
                ...
                //     segment_data.len(), segment_data.len(), file_size_in_bytes,
                // file_size_in_bytes);
...
/// rewrite relocations that depend on source sections already existing and currently loaded in
/// Theseus.
///
...
fn overwrite_relocations(
        ...
        .filter(|sec| sec.get_type() == Ok(ShType::Rela) && sec.size() != 0)
    {
        ...
        // There is one target section per rela section (`rela_array`), and one source section per
        // `rela_entry` in each `rela_array`. The "info" field in the Rela section specifies
        // which section is the target of the relocation.
        ...
        let target_segment = segments
            ...
            .ok_or_else(|| {
                let err = format!(
                    "ELF file error: couldn't find loaded segment that contained section for Rela section {:?}!",
                    sec.get_name(elf_file)
                );
                error!("{}", err);
                err
        ...
        let target_segment_slice: &mut [u8] = target_segment
            .mp
            .as_slice_mut(0, target_segment.bounds.end.value() - target_segment.bounds.start.value())?;
        ...
        for rela_entry in rela_array {
            ...
            // Ignore relocations that refer/point to irrelevant things: sections, files, notypes,
            // or nothing.
            ...
            if verbose_log {
                trace!(
                    "      Rela64 entry has offset: {:#X}, addend: {:#X}, symtab_index: {}, type: {:#X}",
            ...
            if verbose_log {
                ...
                trace!(
                    "                 Entry name {} {:?} vis {:?} bind {:?} type {:?} shndx {} value {} size {}",
                    source_sec_entry.name(),
                    source_sec_entry.get_name(elf_file),
                    source_sec_entry.get_other(),
                    source_sec_entry.get_binding(),
                    source_sec_entry.get_type(),
                    source_sec_entry.shndx(),
                    source_sec_entry.value(),
                    source_sec_entry.size()
                );
            }

            let demangled = demangle(source_sec_name).to_string();

            // If the source section exists in this namespace already, rewrite the relocation entry
            // to point to the existing section instead.
            if let Some(existing_source_sec) = namespace
                .get_symbol_or_load(&demangled, None, mmi, verbose_log)
                .upgrade()
            {
                ...
                // Here, in an executable ELF file, the relocation entry's "offset" represents an
                // absolute virtual address rather than an offset from the beginning
                // of the section/segment (I think). Therefore, we need to adjust
                // that value before we invoke `write_relocation()`, which expects a
                // regular `offset` + an offset into the target segment's mapped pages.
                ...
                // Now that we have incorporated the relocation_entry's actual offset into the
                // target_segment offset, we set it to zero for the duration of this
                // call. TODO: this is hacky as hell, we should just create a new
                // `write_relocation()` function instead.
                relocation_entry.offset = 0;

                if verbose_log {
                    debug!(
                        "                 Performing relocation target {:#X} + {:#X} <-- source {}",
                ...
                // Here, we typically tell the existing_source_sec that the target_segment is
                // dependent upon it. However, the `WeakDependent` entry type only
                // accepts a weak section reference at the moment, and we don't have
                // that -- we only have a target segment. TODO: if/when we wish to
                // track weak dependencies from a section to a target segment, we should add it
                // below.
                //
                ...
                let strong_dep =
                    StrongDependency { section: Arc::clone(&existing_source_sec), relocation: relocation_entry };
        ...
        target_segment
            ...
            .append(&mut target_segment_dependencies);
    }
