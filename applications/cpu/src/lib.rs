...
pub fn main(args: Vec<String>) -> isize {
    ...
    for lapic in all_lapics.iter() {
        ...
        println!("APIC ID: {}\nPROCESSOR ID: {}\nIS BSP: {}", apic_id, processor, is_bsp); 
        if let Some(runqueue) = get_runqueue(apic_id) {
            ...
            on_runqueue.pop();
            currently_running.pop();
            println!("CURRENTLY RUNNING TASKS:\n{}",currently_running);
            println!("ON RUNQUEUE:\n{}", on_runqueue);
        }
        
        else {
            println!("Can't retrieve runqueue of core");
            return -1;
        }
...
fn print_usage(opts: Options) -> isize {
    ...
    brief.push_str("For each core, prints apic_id, processor, whether it is the bootstrap processor (the first processor to boot up), which tasks that is currently running on that core and which tasks are present in that core's runqueue");
