...
use crate_swap::{SwapRequest, SwapRequestList};
use fs_node::{DirRef, FileOrDir};
use getopts::{Matches, Options};
use memfs::MemFile;
use mod_mgmt::{CrateNamespace, IntoCrateObjectFile, NamespaceDir};
...
fn list(remote_endpoint: IpEndpoint, update_build: Option<&String>) -> Result<(), String> {
    ...
    if let Some(ub) = update_build {
        ...
        let update_builds =
            ota_update_client::download_available_update_builds(&iface, remote_endpoint)
                .map_err(|e| e.to_string())?;
...
fn download(
    ...
    let crate_list = if crate_list == Some(&[]) {
        None
    } else {
        crate_list
    };
    ...
    let crates = if let Some(crate_list) = crate_list {
        ...
        let diff_lines = ota_update_client::download_diff(&iface, remote_endpoint, update_build)
            .map_err(|e| {
                format!(
                    "failed to download diff file for {}, error: {}",
                    update_build, e
                )
            })?;
        ...
        let new_crates_to_download: BTreeSet<String> =
            diff.pairs.iter().map(|(_old, new)| new.clone()).collect();
        let crates = ota_update_client::download_crates(
            &iface,
            remote_endpoint,
            update_build,
            new_crates_to_download,
        )
        .map_err(|e| e.to_string())?;
    ...
    let curr_dir = task::get_my_current_task()
        ...
        .ok_or_else(|| format!("couldn't get my current working directory"))?;
    ...
    for df in crates.into_iter() {
        ...
        println!(
            "Downloaded crate: {:?}, size {}",
            cfile.lock().get_absolute_path(),
            size
        );
    }
...
fn apply(base_dir_path: &Path) -> Result<(), String> {
    if cfg!(not(loadable)) {
        return Err(format!(
            "Evolutionary updates can only be applied when Theseus is built in loadable mode."
        ));
    }

    let kernel_mmi_ref =
        memory::get_kernel_mmi_ref().ok_or_else(|| format!("couldn't get kernel MMI"))?;
    let curr_dir = task::get_my_current_task()
        .map(|t| t.get_env().lock().working_dir.clone())
        .ok_or_else(|| format!("couldn't get my current working directory"))?;
    let new_namespace_dir = match base_dir_path.get(&curr_dir) {
        ...
        _ => {
            return Err(format!(
                "cannot find an update base directory at path {}",
                base_dir_path
            ))
        }
    };
    let diff_file = match new_namespace_dir.lock().get(DIFF_FILE_NAME) {
        ...
        _ => {
            return Err(format!(
                "cannot find diff file expected at {}/{}",
                base_dir_path, DIFF_FILE_NAME
            ))
        }
    };
    ...
    let diffs = ota_update_client::as_lines(&diff_content)
        .map_err(|e| e.to_string())
        .and_then(|diff_lines| {
            ota_update_client::parse_diff_lines(&diff_lines).map_err(|e| e.to_string())
        })?;
    ...
    for (old_crate_module_file_name, new_crate_module_file_name) in diffs.pairs.into_iter() {
        println!(
            "Looking at diff {} -> {}",
            old_crate_module_file_name, new_crate_module_file_name
        );
        let old_crate_name = if old_crate_module_file_name == "" {
            ...
            let old_crate_name =
                mod_mgmt::crate_name_from_path(&Path::new(old_crate_module_file_name)).to_string();
        ...
        let new_crate_file = new_namespace_dir
            ...
            .ok_or_else(|| {
                format!(
                    "cannot find new crate file {:?} in new namespace dir {}",
                    new_crate_module_file_name, base_dir_path
                )
            })?;
