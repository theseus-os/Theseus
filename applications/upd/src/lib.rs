...
pub fn main(args: Vec<String>) -> isize {
    ...
    opts.optopt(
        "d",
        "destination",
        "specify the IP address (and optionally, the port) of the update server",
        "IP_ADDR[:PORT]",
    );
...
fn rmain(matches: Matches) -> Result<(), String> {
    let mut remote_endpoint = if let Some(ip_str) = matches.opt_str("d") {
        IpEndpoint::from_str(&ip_str).map_err(|_e| "couldn't parse destination IP address/port".to_string())?
    } else {
    ...
    match &*matches.free[0] {
        "list" | "ls" => list(remote_endpoint, matches.free.get(1)),
        "list-diff" | "ls-diff" => {
            let update_build = matches
                .free
                .get(1)
                .ok_or_else(|| String::from("missing UPDATE_BUILD argument"))?;
        ...
        "apply" | "ap" => {
            let base_dir_path = matches
                .free
                .get(1)
                .ok_or_else(|| String::from("missing BASE_DIR path argument"))?;
...
fn list(remote_endpoint: IpEndpoint, update_build: Option<&String>) -> Result<(), String> {
    ...
    if let Some(ub) = update_build {
        let listing = ota_update_client::download_listing(&iface, remote_endpoint, ub).map_err(|e| e.to_string())?;
        println!("{}", listing.join("\n"));
    } else {
        let update_builds =
            ota_update_client::download_available_update_builds(&iface, remote_endpoint).map_err(|e| e.to_string())?;
...
fn diff(remote_endpoint: IpEndpoint, update_build: &str) -> Result<(), String> {
    ...
    let file_str =
        ota_update_client::download_diff(&iface, remote_endpoint, update_build).map_err(|e| e.to_string())?;
...
fn download(remote_endpoint: IpEndpoint, update_build: &str, crate_list: Option<&[String]>) -> Result<(), String> {
    ...
    let crates = if let Some(crate_list) = crate_list {
        ...
        ota_update_client::download_crates(&iface, remote_endpoint, update_build, crate_set)
        ...
        let crates = ota_update_client::download_crates(&iface, remote_endpoint, update_build, new_crates_to_download)
            .map_err(|e| e.to_string())?;
    ...
    for df in crates.into_iter() {
        ...
        // The name of the crate file that we downloaded is something like:
        // "/keyboard_log/k#keyboard-36be916209949cef.o". We need to get just the basename
        // of the file, then remove the crate type prefix ("k#").
        let file_name = Path::new(&df.name)
            .file_name()
            .ok_or("crate file path did not have file name")?;
...
fn apply(base_dir_path: &Path) -> Result<(), String> {
    ...
    let diff_file = match new_namespace_dir.lock().get(DIFF_FILE_NAME) {
        ...
        _ => {
            return Err(format!("cannot find diff file expected at {base_dir_path}/{DIFF_FILE_NAME}"));
        }
    };
    ...
    let diffs = ota_update_client::as_lines(&diff_content)
        .map_err(|e| e.to_string())
    ...
    for (old_crate_module_file_name, new_crate_module_file_name) in diffs.pairs.into_iter() {
        ...
        let old_crate_name = if old_crate_module_file_name.is_empty() {
            // An empty old_crate_name indicates that there is no old crate or object file to
            // remove, we are just loading a new crate (or inserting its object file)
            None
        } else {
            ...
            if curr_namespace.get_crate(&old_crate_name).is_none() {
                println!(
                    "\t Note: old crate {:?} was not currently loaded into namespace {:?}.",
                    old_crate_name,
                    curr_namespace.name()
                );
            }
        ...
        let new_crate_file = new_namespace_dir
            .get_crate_object_file(&new_crate_module_file_name)
            .ok_or_else(|| {
                format!(
                    "cannot find new crate file {new_crate_module_file_name:?} in new namespace dir {base_dir_path}"
                )
            })?;
...
fn get_my_current_namespace() -> Arc<CrateNamespace> {
    task::with_current_task(|t| t.get_namespace().clone()).unwrap_or_else(|_| {
