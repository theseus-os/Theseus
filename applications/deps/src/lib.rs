...
fn section_containing_address(addr: &str) -> Result<(), String> {
    let addr = if addr.starts_with("0x") || addr.starts_with("0X") { &addr[2..] } else { addr };
...
fn num_deps_all() -> Result<(), String> {
    ...
    println!(
        "Total Dependency Count for all {} crates ({} sections):\nStrong: {}\nWeak:   {}\nIntrnl: {}",
        crate_count, section_count, s_total, w_total, i_total
    );
    Ok(())
}
...
fn count_private_rodata_sections() -> Result<(), String> {
    ...
    namespace.for_each_crate(true, |_crate_name, crate_ref| {
        ...
        for sec in crate_ref
            ...
            .sections
            .values()
            .filter(|sec| sec.typ == mod_mgmt::SectionType::Rodata)
        {
            section_count += 1;
...
/// Returns the given section's count of `(strong dependencies, weak dependents, internal
/// dependencies)`.
...
///
/// # Arguments
/// * `all_sections`: If `true`, then all sections will be printed. If `false`, then only public
///   (global) sections will be printed.
///
/// If there are multiple matches, this returns an Error containing
/// all of the matching section names separated by the newline character `'\n'`.
...
fn find_section(section_name: &str) -> Result<StrongSectionRef, String> {
    ...
    let containing_crate_ref = get_containing_crate_name(section_name)
        ...
        .ok_or_else(|| {
            format!("Couldn't find section {section_name} in symbol map, and couldn't get its containing crate")
        })?;
    ...
    let mut matching_sections: Vec<StrongSectionRef> = containing_crate_ref
        ...
        .sections
        .values()
        .filter_map(|sec| if sec.name.starts_with(section_name) { Some(sec.clone()) } else { None })
        .collect();
