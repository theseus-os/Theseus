...
fn rmain(matches: Matches) -> Result<(), String> {
    let (namespace, curr_wd) =
        task::with_current_task(|t| (t.get_namespace().clone(), t.get_env().lock().working_dir.clone()))
...
fn load_crate(output: &mut String, crate_file_ref: FileRef, namespace: &Arc<CrateNamespace>) -> Result<(), String> {
    ...
    let (_new_crate_ref, _new_syms) = namespace
        ...
        .map_err(String::from)?;
    writeln!(output, "Loaded crate with {} new symbols from {}", _new_syms, crate_file_ref.lock().get_absolute_path())
        .unwrap();
    Ok(())
}
