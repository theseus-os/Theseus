...
pub fn main(args: Vec<String>) -> isize {
    ...
    options
        ...
        .optflag("l", "least-busy", "Get the least busy core's run queue")
        .optopt("c", "core", "Get <core>'s run queue", "<core>")
    ...
    let duration = if least_busy {
        run(
            |_| {
                scheduler::get_least_busy_run_queue();
            },
            num,
        )
        ...
        run(
            |count| {
                scheduler::get_run_queue((count % cpu_count) as u8);
            },
            num,
        )
    };
