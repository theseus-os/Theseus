...
use smoltcp::{
    ...
    socket::{IcmpEndpoint, IcmpPacketMetadata, IcmpSocket, IcmpSocketBuffer, SocketSet},
...
pub fn main(args: Vec<String>) -> isize {
    ...
    opts.optflag(
        "v",
        "verbose",
        "a more detailed view of packets sent and received",
    );
    opts.optopt(
        "c",
        "count",
    ...
    opts.optopt(
        "i",
        "interval",
        "interval between packets being sent in miliseconds (default: 1000)",
        "N",
    );
    opts.optopt(
        "t",
        "timeout",
        "maximum time between echo request and echo reply in milliseconds (default: 5000)",
        "N",
    );
    opts.optopt(
        "s",
        "buffer size",
        "size of packet to send to target address, (min: 8, max: 120, default: 40)",
        "N",
    );
...
pub fn rmain(matches: &Matches, _opts: Options, address: IpAddress) -> Result<(), &'static str> {
    ...
    if let Some(i) = matches.opt_default("c", "4") {
        count = i
            .parse::<usize>()
            .map_err(|_e| "couldn't parse number of packets")?;
    }
    ...
    if let Some(i) = matches.opt_default("t", "5000") {
        timeout = i
            .parse::<u64>()
            .map_err(|_e| "couldn't parse timeout length")?;
    }
    if let Some(i) = matches.opt_default("s", "40") {
        buffer_size = i
            .parse::<usize>()
            .map_err(|_e| "couldn't parse packet size")?;
        if buffer_size > 120 {
...
fn get_icmp_pong(
    waiting_queue: &mut HashMap<u16, u64>,
    times: &mut Vec<u64>,
    total_time: &mut u64,
    repr: Icmpv4Repr,
    received: &mut u16,
        ...
        }
        {
            ...
            if socket.can_send() && seq_no < count as u16 && send_at <= timestamp {
                ...
                if verbose {
                    ...
                    println!(
                        "checking checksum of packet, should be 0: {:?}",
                        icmp_packet.checksum()
                    );
                    println!(
                        "checking echo_ident of packet, should be a value: {:?}",
                        icmp_packet.echo_ident()
                    );
                    println!(
                        "checking msg_type of packet, should be an echo_request: {:?}",
            ...
            if socket.can_recv() {
                ...
                if verbose {
                    println!("buffer length: {}", icmp_repr.buffer_len());
                    println!(
                        "checking checksum of packet, should be above 0: {:?}",
                        icmp_packet.checksum()
                    );
                    println!(
                        "checking echo_ident of packet, should be a value: {:?}",
                    ...
                    println!(
                        "checking msg_type of packet, should be an echo_reply: {:?}",
                        icmp_packet.msg_type()
                    );
                }
            }
            ...
            let unflushed_txbuffer =
                timeout_loop == true && poll_status == false && seq_no != count as u16;
