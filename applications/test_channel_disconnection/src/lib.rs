...
pub fn main(args: Vec<String>) -> isize {
    ...
    opts.optopt("s", "send_count", "number of messages expected to be sent (default 100)", "SEND_C");
    opts.optopt("l", "receive_count", "number of messages expected to be sreceived (default 100)", "REC_C");
...
fn rmain(matches: Matches) -> Result<(), &'static str> {
    ...
    if matches.opt_present("r") {
        did_something = true;
        println!("Running rendezvous channel test");
        rendezvous_test(send_count!(), receive_count!())?;
    }

    if matches.opt_present("a") {
        did_something = true;
        println!("Running asynchronous channel test.");
        asynchronous_test(send_count!(), receive_count!())?;
    }
...
fn rendezvous_test(send_count: usize, receive_count: usize) -> Result<(), &'static str> {
    ...
    t2.join()?;
    t1.join()?;
    warn!("rendezvous_test(): Joined the sender and receiver tasks.");
...
fn asynchronous_test(send_count: usize, receive_count: usize) -> Result<(), &'static str> {
    ...
    t2.join()?;
    t1.join()?;
    warn!("asynchronous_test(): Joined the sender and receiver tasks.");
