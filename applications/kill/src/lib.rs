...
pub fn main(args: Vec<String>) -> isize {
    ...
    for task_id_str in matches.free.iter() {
        match task_id_str.parse::<usize>() {
            Ok(task_id) => match kill_task(task_id, reap) {
                Ok(_) => {}
                Err(e) => {
                    println!("{}", e);
                    return -1;
            ...
            _ => {
                println!(
                    "Invalid argument {}, not a valid task ID (usize)",
                    task_id_str
                );
                return -1;
            }
...
fn kill_task(task_id: usize, reap: bool) -> Result<(), String> {
    if let Some(task_ref) = task::get_task(task_id) {
        if task_ref
            .kill(task::KillReason::Requested)
            ...
            .is_ok()
        {
            ...
            if reap {
                match task_ref.take_exit_value() {
                    Some(exit_val) => {
                        println!(
                            "Reaped task {}, got exit value {}",
                            task_id,
                            debugit!(exit_val)
                        );
                        Ok(())
                    }
                    _ => Err(format!("Failed to reap task {}", task_id)),
                }
            } else {
            ...
            if reap {
                ...
                Err(format!(
                    "Failed to kill task {}, it was already exited.",
                    task_id
                ))
            }
        }
