...
impl Terminal {
    ...
    fn display_text(&mut self) -> Result<(), &'static str>{
        ...
        debug!("pre area_to_render");
        let area_to_render = self.text_display.display(coord, &mut self.window.framebuffer_mut().lock())?;
        debug!("post area_to_render");
...
impl Terminal {
    ...
    pub fn new(final_framebuffer: Framebuffer<AlphaPixel>, key_consumer: Queue<KeyEvent>, mouse_consumer: Queue<MouseEvent>) -> Result<Terminal, &'static str> {
        debug!("pre window");
    ...
    pub fn display_cursor(&mut self) -> Result<(), &'static str> {
        ...
        let bounding_box = {
            ...
            let bounding_box = self.cursor.display(
                ...
                cursor_col,
                cursor_line,
                &mut self.window.framebuffer_mut().lock(),
            )?;
            bounding_box
        };   
        //debug!("cursor");
