...
use hpet::get_hpet;
use mod_mgmt::{IntoCrateObjectFile, NamespaceDir};
use path::Path;
...
pub fn main(args: Vec<String>) -> isize {
    ...
    opts.optflag(
        "v",
        "verbose",
        "enable verbose logging of crate swapping actions",
    );
    opts.optflag(
        "c",
        "cache",
        "enable caching of the old crate(s) removed by the swapping action",
    );
    opts.optopt(
        "d",
        "directory-crates",
...
fn rmain(matches: Matches) -> Result<(), String> {
    ...
    let override_namespace_crate_dir = if let Some(path) = matches.opt_str("d") {
        ...
        let dir = match path.get(&curr_dir) {
            ...
            _ => {
                return Err(format!(
                    "Error: could not find specified namespace crate directory: {}.",
                    path
                ))
            }
        };
        ...
        for (old_crate_name, new_crate_str, reexport) in tuples {
            ...
            let (into_new_crate_file, new_namespace) = {
                    ...
                    .and_then(|ns_dir| ns_dir.get_file_starting_with(new_crate_str))
                {
                    ...
                    Path::new(String::from(new_crate_str)).get(curr_dir)
                {
                    (IntoCrateObjectFile::File(f), None)
                } else {
                    (
                        IntoCrateObjectFile::Prefix(String::from(new_crate_str)),
                        None,
                    )
                }
            };
