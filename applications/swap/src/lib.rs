...
pub fn main(args: Vec<String>) -> isize {
    ...
    opts.optopt(
        "d",
        "directory-crates",
        "the absolute path of the base directory where new crates will be loaded from",
        "PATH",
    );
    opts.optmulti(
        "t",
        "state-transfer",
        "the fully-qualified symbol names of state transfer functions, to be run in the order given",
        "SYMBOL",
    );
...
fn rmain(matches: Matches) -> Result<(), String> {
    ...
    let override_namespace_crate_dir = if let Some(path) = matches.opt_str("d") {
        ...
        let dir = match path.get(&curr_dir) {
            ...
            _ => {
                return Err(format!("Error: could not find specified namespace crate directory: {path}."));
            }
        };
    ...
    do_swap(tuples, &curr_dir, override_namespace_crate_dir, state_transfer_functions, verbose, cache_old_crates)
}
...
fn parse_input_tuples(args: &str) -> Result<Vec<(&str, &str, bool)>, String> {
    ...
    for (paren_start, _) in open_paren_iter {
        let the_rest = args
            .get((paren_start + 1)..)
            .ok_or_else(|| "unmatched open parenthesis.".to_string())?;
        ...
        let parsed = the_rest
            ...
            .and_then(|inside_paren| {
                ...
                token_iter.next().and_then(|first| {
                    token_iter
                        .next()
                        .map(|second| (first, second, token_iter.next()))
                })
            });
        match parsed {
            Some((o, n, reexport)) => {
    ...
    if v.is_empty() { Err("no crate tuples specified.".to_string()) } else { Ok(v) }
}
...
fn do_swap(
    ...
    let namespace = task::with_current_task(|t| t.get_namespace().clone()).map_err(|_| "Couldn't get current task")?;

    let swap_requests = {
        let mut requests: Vec<SwapRequest> = Vec::with_capacity(tuples.len());
        for (old_crate_name, new_crate_str, reexport) in tuples {
            // Check that the new crate file exists. It could be a regular path, or a prefix for a
            // file in the namespace's dir. If it's a full path, then we just check that
            // the path points to a valid crate object file. Otherwise, we treat it as a
            // prefix for a crate object file name that may be found
            let (into_new_crate_file, new_namespace) = {
                if let Some(f) = override_namespace_crate_dir
                    .as_ref()
                    .and_then(|ns_dir| ns_dir.get_file_starting_with(new_crate_str))
                {
    ...
    match swap_result {
        Ok(()) => {
            println!(
                "Swap operation complete. Elapsed HPET ticks: {}, (HPET Period: {} femtoseconds)",
