...
fn main() -> Result<(), String> {
    ...
    let mut kernel_prefix = matches
        .opt_str("kernel-prefix")
    ...
    if kernel_prefix.matches(MODULE_PREFIX_DELIMITER).count() > 1 {
        return Err(format!(
            "kernel-prefix {:?} must only contain one '#' character at the end!",
            kernel_prefix
        ));
    }
    let mut app_prefix = matches.opt_str("app-prefix").unwrap_or_else(|| "a".into());
    ...
    if app_prefix.matches(MODULE_PREFIX_DELIMITER).count() > 1 {
        return Err(format!(
            "app-prefix {:?} must only contain one '#' character at the end!",
            app_prefix
        ));
    }
    ...
    let output_objects_dir = matches
        .opt_str("output-objects")
        .expect("no --output-objects arg provided");
    ...
    let kernel_crates_set = if kernel_arg_path.is_file() {
        ...
        return Err(format!(
            "Couldn't access -k/--kernel argument {:?} as a file or directory",
            kernel_arg_path
        ));
    };
    ...
    let mut app_crates_set = if app_arg_path.is_file() {
        ...
        return Err(format!(
            "Couldn't access -k/--app argument {:?} as a file or directory",
            app_arg_path
        ));
    };

    let extra_app_names = matches.opt_strs("e");
    app_crates_set.extend(
        extra_app_names
            .iter()
    ...
    let (app_object_files, kernel_objects_and_deps_files, other_objects_and_deps_files) =
        parse_input_dir(app_crates_set, kernel_crates_set, input_dir).unwrap();
    ...
    fs::create_dir_all(&output_deps_dir).map_err(|e| {
        format!(
            "Error creating output deps directory {:?}, {:?}",
            output_deps_dir, e
        )
    })?;
    copy_files(
    ...
    if let Some(output_sysroot_dir) = matches.opt_str("output-sysroot") {
        fs::create_dir_all(&output_sysroot_dir).map_err(|e| {
            format!(
                "Error creating output sysroot directory {:?}, {:?}",
                output_sysroot_dir, e
            )
        })?;
        let sysroot_files = other_objects_and_deps_files
            .iter()
        ...
        copy_files(&output_sysroot_dir, sysroot_files, "").unwrap();
    }
    ...
    for dir_entry in dir_iter {
        if dir_entry.file_type().is_file() && dir_entry.file_name() == "Cargo.toml" {
            ...
            let parent_crate_name = parent_crate_dir.file_name().ok_or_else(|| {
                let err_str = format!(
                    "Error getting the name of crate directory {:?}",
                    parent_crate_dir
                );
                io::Error::new(io::ErrorKind::NotFound, err_str)
            })?;
...
fn parse_input_dir(
    ...
    for dir_entry in fs::read_dir(input_dir)? {
        ...
        let file_stem = file_name
            .split(".o")
            .next()
            .expect("object file name didn't have the .o extension");
        let prefix = file_name
            .split("-")
            .next()
            .expect("object file name didn't have the crate/hash '-' delimiter");
    ...
    Ok((app_objects, kernel_files, other_files))
}
...
fn copy_files<'p, O, P, I>(output_dir: O, files: I, prefix: &str) -> io::Result<()>
where
    O: AsRef<Path>,
    P: AsRef<Path>,
    I: Iterator<Item = P>,
