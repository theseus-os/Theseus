//! string implementation from relibc
...
pub unsafe extern "C" fn strndup(s1: *const c_char, size: size_t) -> *mut c_char {
    ...
    let buffer = crate::stdlib::malloc(len + 1) as *mut c_char;
    if buffer.is_null() {
        errno = ENOMEM;
    } else {
...
pub unsafe extern "C" fn strerror(errnum: c_int) -> *mut c_char {
    ...
    let mut w = crate::io::StringWriter(strerror_buf.as_mut_ptr(), strerror_buf.len());
...
// TODO: add back in when we have support for signals
// #[no_mangle]
